...
int cri_nas_rules_pref_mode_rule_check_handler(void *rule_data)
{
    ...
    if ( pref_mode_rule_data->pref_mode_valid &&
         cri_nas_sys_sel_pref_info.mode_pref_valid )
    {
    ...
    if ( pref_mode_rule_data->roaming_pref_valid &&
         cri_nas_sys_sel_pref_info.roam_pref_valid )

    {
        current_roaming_pref = cri_nas_sys_sel_pref_info.roam_pref;
        if ( current_roaming_pref == pref_mode_rule_data->roaming_pref )
        {
            ret_code = TRUE;
        }
    }
