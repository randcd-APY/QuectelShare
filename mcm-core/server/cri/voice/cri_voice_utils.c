...
)
{
   cri_core_error_type result = TRUE;
       ...
       {
       case CRI_VOICE_CALL_TYPE_ECALL:
          *qmi_call_type = CALL_TYPE_ECALL_V02;
          break;

       case CRI_VOICE_CALL_TYPE_EMERGENCY:
          if( cri_call_domain == CRI_VOICE_CALL_DOMAIN_PS )
          {
             *qmi_call_type = CALL_TYPE_EMERGENCY_IP_V02;
          }
          else
          {
             *qmi_call_type = CALL_TYPE_EMERGENCY_V02;
          }
...
uint8_t cri_voice_util_get_qmi_emergency_cat(unsigned emergency_cat)
{
    uint8_t ret = 0;

    UTIL_LOG_MSG("\n cri_voice_util_get_qmi_emergency_cat, emergency_cat - %d\n",emergency_cat);
    if (emergency_cat & (1 << CRI_VOICE_EMERGENCY_CATEGORY_POLICE))
    {
        ret |= (1 << VOICE_EMER_CAT_POLICE_BIT_V02);
    }
    if (emergency_cat & (1 << CRI_VOICE_EMERGENCY_CATEGORY_AMBULANCE))
    {
        ret |= (1 << VOICE_EMER_CAT_AMBULANCE_BIT_V02);
    }
    if (emergency_cat & (1 << CRI_VOICE_EMERGENCY_CATEGORY_FIRE_BRIGADE))
    {
        ret |= (1 << VOICE_EMER_CAT_FIRE_BRIGADE_BIT_V02);
    }
    if (emergency_cat & (1 << CRI_VOICE_EMERGENCY_CATEGORY_MARINE_GUARD))
    {
        ret |= (1 << VOICE_EMER_CAT_MARINE_GUARD_BIT_V02);
    }
    if (emergency_cat & (1 << CRI_VOICE_EMERGENCY_CATEGORY_MOUNTAIN_RESCUE))
    {
        ret |= (1 << VOICE_EMER_CAT_MOUNTAIN_RESCUE_BIT_V02);
    }
    if (emergency_cat & (1 << CRI_VOICE_EMERGENCY_CATEGORY_ECALL_AUTO))
    {
        ret |= (1 << VOICE_EMER_CAT_AUTO_ECALL_BIT_V02);
    }
    if (emergency_cat & (1 << CRI_VOICE_EMERGENCY_CATEGORY_ECALL_MANUAL))
    {
        ret |= (1 << VOICE_EMER_CAT_MANUAL_ECALL_BIT_V02);
    }

    UTIL_LOG_MSG("\n cri_voice_util_get_qmi_emergency_cat, ret - %d\n",ret);
    return ret;
}
