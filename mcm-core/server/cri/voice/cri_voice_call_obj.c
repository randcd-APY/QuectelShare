...
//        QCRIL_LOG_FUNC_RETURN_WITH_RET((int) err);
    ...
    {
        //Initialize with type voice
        call_obj_ptr->cri_call_type = CRI_VOICE_CALL_TYPE_VOICE;
        boolean call_domain_set = FALSE;

        if(qmi_call_type == CALL_TYPE_EMERGENCY_V02)
        {
            call_obj_ptr->cri_call_type = CRI_VOICE_CALL_TYPE_EMERGENCY;
        }
        else if(qmi_call_type == CALL_TYPE_ECALL_V02)
        {
            call_obj_ptr->cri_call_type = CRI_VOICE_CALL_TYPE_ECALL;
        }
