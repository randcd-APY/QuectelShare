...
cri_core_error_type cri_voice_dial_req_handler(cri_core_context_type cri_core_context, const cri_voice_dial_request_type *req_message, const void *user_data, cri_voice_request_dial_cb_type dial_cb)
{
  ...
  char * address = req_message->address;
  boolean is_conf_uri = req_message->is_conference_uri;

  cri_voice_clir_type clir;
  // If 'default' CLIR is specified, get its value from the CLIR variable in cri_voice_ss.c
  if (req_message->clir == CRI_VOICE_CLIR_DEFAULT || req_message->clir == NULL)
  {
      clir = cri_voice_obtain_stored_clir();
      QCRIL_LOG_INFO("Dial request has picked up the default CLIR value from CRF.");
  }
  // Otherwise, pass the request specified CLIR setting forward.
  else
  {
      clir = req_message->clir;
  }
  ...
  unsigned emergency_cat = req_message->emergency_cat;
  size_t msd_length = req_message->msd_length;
  if (!is_conf_uri)
  {
...
//        unsigned int old_call_radio_tech_family = qcril_qmi_voice_nas_control_convert_radio_tech_to_radio_tech_family(old_call_radio_tech);
        ...
        if (!is_conf_uri)
        {
            ...
            {
               memcpy(dial_call_req_msg.calling_number,
                      address,
                      (addr_len < QMI_VOICE_NUMBER_MAX_V02) ? addr_len : QMI_VOICE_NUMBER_MAX_V02);
             }
             else
             {
               // address
        ...
        if( addr_len > QMI_VOICE_NUMBER_MAX_V02 )
        {
           memcpy(dial_call_req_msg.sip_uri_overflow,
                  address + QMI_VOICE_NUMBER_MAX_V02,
                  (((addr_len - QMI_VOICE_NUMBER_MAX_V02) < QMI_VOICE_SIP_URI_OVERFLOW_MAX_V02) ?
                   (addr_len - QMI_VOICE_NUMBER_MAX_V02):QMI_VOICE_SIP_URI_OVERFLOW_MAX_V02));
        ...
        if(emergency_cat != 0)
        {
           ...
           dial_call_req_msg.emer_cat = cri_voice_util_get_qmi_emergency_cat(emergency_cat);
           dial_call_req_msg.ecall_variant_valid = TRUE;
           dial_call_req_msg.ecall_variant = ECALL_EMERGENCY_V02;
        }

        if( msd_length && msd_length <= CRI_MAX_ECAL_MSD_LENGTH)
        {
           dial_call_req_msg.ecall_msd_valid = TRUE;
           dial_call_req_msg.ecall_msd_len = msd_length;
           memcpy(dial_call_req_msg.ecall_msd, req_message->msd, msd_length);
        }
...
//        int is_non_std_otasp = qmi_ril_phone_number_is_non_std_otasp( address ) && (RADIO_TECH_3GPP2 == old_call_radio_tech_family);
        ...
        user_rule_info.rule_data = (void*)((int)call_obj_ptr->cri_call_id);
        ...
        user_rule_info.cri_resp_util_data = (void*)((int)call_obj_ptr->cri_call_id);
    ...
    if ( QMI_ERR_NONE_V01 != call_setup_result )
    { // rollback
...
)
{
    ...
    {
        err = cri_core_retrieve_err_code(QMI_NO_ERR, &qmi_dial_resp_msg_ptr->resp);
    }
...
static void* cri_voice_dial_get_resp_data(cri_core_error_type cri_core_error, void *cri_resp_util_data)
{
    ...
    if (CRI_ERR_NONE_V01 == cri_core_error)
    {
        ...
        if (dial_resp_ptr)
        {
            dial_resp_ptr->dial_call_id = (int) cri_resp_util_data;
...
static void cri_voice_dial_free_resp_data(void *cri_resp_data)
{
    ...
    if (dial_resp_ptr)
    {
        ...
        util_memory_free(&dial_resp_ptr);
    }
