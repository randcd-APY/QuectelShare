...
cri_core_error_type cri_voice_core_send_end_call_request(cri_core_context_type cri_core_context, const void *user_data, hlos_resp_cb_type cb, uint8_t qmi_id)
{
    ...
    user_rule_info.rule_data = (void*)(int)qmi_id;
...
static cri_core_error_type cri_voice_hangup_all_calls
(
...
{
    cri_core_error_type err = 0;
    cri_core_error_type err_ip = 0;
    cri_core_error_type err_3gpp = 0;
    ...
    do
    {
        if (call_summary.nof_ps_calls > 0)
        {
            err_ip = cri_voice_core_send_manage_ip_call_request(
                    cri_core_context,
                    user_data,
                    cri_voice_hangup_resp_wapper,
                    VOIP_SUPS_TYPE_END_ALL_CALLS_V02);

            QCRIL_LOG_DEBUG("VOIP_SUPS_TYPE_END_ALL_CALLS result: %d", err_ip);
        }
        if (call_summary.nof_cs_3gpp_calls > 0)
        {
            err_3gpp = cri_voice_core_send_manage_call_request(
                    cri_core_context,
                    user_data,
                    cri_voice_hangup_resp_wapper,
                    SUPS_TYPE_END_ALL_CALLS_V02);

            QCRIL_LOG_DEBUG("SUPS_TYPE_END_ALL_CALLS result: %d", err_3gpp);
        }

        if (call_summary.nof_cs_3gpp2_calls > 0)
        {
            cri_voice_call_list_get_filtered_call_objects(
                    cri_voice_core_get_call_list(),
                    cri_voice_call_obj_is_3gpp2, &num_of_calls, &call_obj_dptr);

            if (num_of_calls && call_obj_dptr)
            {
                if (num_of_calls >= 1)
                {
                    err_3gpp2 = cri_voice_core_send_end_call_request(
                            cri_core_context,
                            user_data,
                            cri_voice_hangup_resp_wapper,
                            call_obj_dptr[0]->qmi_call_id);
                    QCRIL_LOG_DEBUG("END_CALL_REQUEST result: %d", err_3gpp2);
                    if(num_of_calls > 1)
                    {
                        QCRIL_LOG_DEBUG("END_CALL_REQUEST Have more than 1 3gpp2 calls!");
                    }
                }
            }
        }
...
cri_core_error_type cri_voice_hangup_req_handler(cri_core_context_type cri_core_context, const cri_voice_call_hangup_request_type *req_message, const void *user_data, cri_voice_request_hangup_cb_type hangup_cb)
{
    ...
    else
    {
        ...
        case CRI_VOICE_HANGUP_ALL_CALLS:
            err = cri_voice_hangup_all_calls(cri_core_context, hlos_cb_data_wrapper_ptr);
            break;
        ...
        case CRI_VOICE_HANGUP_WITH_CALL_HLOS_ID:
            ...
            {
                UTIL_LOG_MSG("call_hlos_id_valid: %d",call_hlos_id_valid);
