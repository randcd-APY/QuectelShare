...
#define HOSTAPD_CFG_PATH_SSID2 "/etc/misc/wifi/hostapd-eth1.conf"
...
boolean MCM_QCMAP_ConnectionManager::EnableDNS()
{
  ...
  if (qcmap_cm_get_ipv4_net_conf(this->qcmap_cm_handle, &public_ip, &pri_dns_addr, &sec_dns_addr, &qcmap_cm_errno) == QCMAP_CM_SUCCESS)
  {
    if (pri_dns_addr)
    {
      ...
      snprintf(command, MAX_COMMAND_STR_LEN, "echo 'nameserver %s' > %s", inet_ntoa(addr), DNSMASQ_RESOLV_FILE);
