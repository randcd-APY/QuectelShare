...
struct gcs_config_rsp {
    /* Pointer to copy the response payload */
    void *rsp_buf;

    /*
     * Size of the rsp_buf allocated by caller
     * should be size of all pararams + the header
     * size for each param.
     */
    uint32_t rsp_size_requested;

    /* Size of the response that was actually returned */
    uint32_t rsp_size_returned;
};
