...
int skip_wave_header(FILE *fp)
{
    ...
    fseek(fp, 0 ,SEEK_SET);
    if (sizeof(wave_hdr) != fread(&wave_hdr, sizeof(wave_hdr), 1, fp)) {
        print("fread() failed");
    }
    ...
    do {
        if (sizeof(subchunk) != fread(&subchunk, sizeof(subchunk), 1, fp)) {
            print("fread() failed");
        }
...
void read_bytes_timestamps_from_file(qap_audio_buffer_t *buffer) {
     if (NULL != fp_input_file) {
         char tempstr[100];
         int seek_offset = 0;
         if (fgets(tempstr, sizeof(tempstr), fp_input_file) != NULL) {
         ...
         if (token != NULL) {
             ...
             if(token!= NULL) {
                 ...
                 if (token!= NULL) {
                     ...
                     printf("%s and timestamp to be pushed to queue is %jd \n", __FUNCTION__, buffer->common_params.timestamp);
                 }
                 token = strtok(NULL, ",");
                 if (token != NULL) {
            ...
            if (( argv[i][0] == '-') &&( argv[i][1] < '0' || argv[i][1] > '9')) {
                ...
                if (strcmp(key,"im") == 0) {
                    ...
                    if (value == NULL) {
                        ...
                        print("%s: Input Sampling Rate = %s", __func__, value);
                    ...
                    if (value == NULL) {
                        ...
                        print("%s: System Input Sampling Rate = %s", __func__, value);
...
void secondary_file_playback(void *ptr)
{
    ...
    if (has_main2_input && !session_broadcast) {
        ret = qap_session_set_callback(second_input->session_handle, &qap_session_callback_secondary, NULL);
...
int main(int argc, const char *argv[])
{
    ...
    ret = qap_session_set_callback(qap_session_handle, &qap_session_callback, NULL);
    if (ret != QAP_STATUS_OK) {
        print("!!!! Please specify appropriate Session");
        return -1;
    }
                  ...
                  if (bytes_consumed < 0) {
                      while (!is_buffer_available) {
                          ...
                          if (reply_data)
                           ...
                           print("%s : %d, dts clip reply_data is %p buffer availabale is %d",
