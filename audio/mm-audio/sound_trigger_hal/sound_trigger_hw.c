...
static void handle_audio_concurrency(audio_event_type_t event_type, audio_event_info_t* config)
{
    ...
    */
    pthread_mutex_lock(&stdev->lock);
    /*
       To Do: Club platform_stdev_check_concurrency_supported and
       platform_stdev_check_and_update_concurrency to have concurrency
       handling at one place
    */
    conc_supported = platform_stdev_check_concurrency_supported(stdev->platform, config);
    num_sessions = get_num_sessions();
    platform_stdev_check_and_update_concurrency(stdev->platform, event_type,
        num_sessions);
    ...
    if (event_type == AUDIO_EVENT_CAPTURE_DEVICE_ACTIVE) {
        /*
         * SVA session is paused when audio usecase starts independent of
         * concurrency is active or not. AHAL will go ahead and enable
         * device as part of audio session start. SVA session will be restarted
         * as part of CAPTURE_STREAM_ACTIVE event if conc is enabled.
         */
        ALOGD("%s: Audio session started, pause SVA session", __func__);
        list_for_each(p_ses_node, &stdev->sound_model_list) {
            p_ses = node_to_item(p_ses_node, st_session_t, list_node);
            st_session_pause(p_ses);
        }
    } else if (event_type == AUDIO_EVENT_CAPTURE_DEVICE_INACTIVE) {
        /*
         * Restart SVA session when audio usecase ends with and without
         * concurrency enabled. Restart of SVA session will ensure that
         * SVA device mixer controls will be triggered.
         * Check for tx_concurrency_active becomes zero to make sure that
         * sva gets restarted only after all audio sessions are closed.
         */
        if (!stdev->tx_concurrency_active) {
            stdev->audio_concurrency_active = false;
            ALOGD("%s: AHAL session has ended, restart STHAL usecase", __func__);
            list_for_each(p_ses_node, &stdev->sound_model_list) {
                p_ses = node_to_item(p_ses_node, st_session_t, list_node);
                st_session_pause(p_ses);
            }
        ...
        /*
         * Resume SVA session when concurrency is enabled so that audio and SVA
         * usecases run concurrently.
         */
        if (conc_supported) {
            ALOGD("%s: AHAL has a capture session ON, do not disable the device", __func__);
    ...
    /*
     * else if (event_type == AUDIO_EVENT_CAPTURE_STREAM_INACTIVE)
     * Nothing happens as part of stream inactive event as session restart
     * decision is taken as part of DEVICE_INACTIVE event.
     */
    pthread_mutex_unlock(&stdev->lock);
                                  ...
                                  sound_model_handle_t *handle)
{
    ...
    concurrency_allowed = platform_stdev_is_session_allowed(stdev->platform,
                                                            get_num_sessions());
...
static int stdev_reconfig_backend_on_stop(st_session_t *stopped_ses)
{
    ...
    st_session_t *ses;
    struct st_vendor_info *ses_v_info;
    struct listnode *ses_node;
    int status = 0;
    bool reconfig = true;
    ...
    bool is_stopped = (st_session_is_detected(stopped_ses) == false) &&
    ...
    stopped_ses_channel_count = platform_stdev_get_channel_count(stdev->platform,
                                                                 stopped_v_info);
    list_for_each(ses_node, &stdev->sound_model_list) {
        ses = node_to_item(ses_node, st_session_t, list_node);
        ses_channel_count = platform_stdev_get_channel_count(stdev->platform,
                                                             ses->vendor_uuid_info);
        ALOGV("%s:[%d] check ses_v_info %p", __func__, ses->sm_handle, ses_v_info);
        ...
        if (ses_channel_count >= stopped_ses_channel_count) {
            ALOGV("Better config exists w/ ch count %d v/s stopped %d, skip reconfig",
                  ses_channel_count, stopped_ses_channel_count);
            /*
             * there exists another session with better or same config.
             * we don't have to reconfig
             */
            reconfig = false;
            break;
        } else if ((best_ses == NULL) || (ses_channel_count > best_channel_count)) {
            /*
             * if best_ses == NULL, this if check will be reached only if
             * channel count of ses is < stopped
             */
            best_ses = ses;
    ...
    if (best_ses) {
        ...
        ALOGI("%s:[%d] force changed backend to %d", __func__,
              best_ses->sm_handle,
              best_ses->vendor_uuid_info->channel_count);
                               ...
                               audio_event_info_t* config)
{
    ...
    case AUDIO_EVENT_DEVICE_CONNECT:
        ...
        if (config->u.value == AUDIO_DEVICE_OUT_LINE) {
            pthread_mutex_lock(&stdev->lock);
            /* New EC ref device would take effect after restarting the usecase */
            stdev->ec_ref_dev = config->u.value;
            pthread_mutex_unlock(&stdev->lock);
            ALOGD("%s: set ec ref device to %d", __func__, config->u.value);
        } else {
    ...
    case AUDIO_EVENT_DEVICE_DISCONNECT:
        ...
        if (config->u.value == AUDIO_DEVICE_OUT_LINE) {
            pthread_mutex_lock(&stdev->lock);
            /*
             * Currently spkr and lineout are the supported ec ref devices
             * Set default EC ref device (spkr) if lineout is disconnected
             */
            stdev->ec_ref_dev = AUDIO_DEVICE_OUT_SPEAKER;
            pthread_mutex_unlock(&stdev->lock);
            ALOGD("%s: set default ec ref device %d", __func__, config->u.value);
        } else {
