...
static int allocate_lab_buffers_ape(st_hw_session_lsm_t* p_lsm_ses)
{
    ...
    pthread_condattr_init(&attr);
    pthread_condattr_setclock(&attr, CLOCK_MONOTONIC);
    pthread_cond_init(&p_lsm_ses->pcm_out_buf_cond, &attr);
...
)
{
    ...
    if (enable) {
        ...
        if (1 == ref_cnt) {
            ...
            if (!status) {
                ALOGD("%s: enable device (%x) = %s", __func__, st_device,
                      st_device_name);
                /*
                 * Make sure to disable device first and then enable with conc enabled
                 ...
                 * mixer value in audio route is not reset, so detections wont work
                 * 4. To fix this call audio reset then enable device
                 */
                if (p_ses->stdev->conc_capture)
        ...
        if (0 == ref_cnt) {
            if (!p_ses->stdev->audio_concurrency_active) {
                ...
                ALOGD("%s: Concurrent capture on, do not disable", __func__);
            }
        }
        pthread_mutex_unlock(&p_ses->stdev->ref_cnt_lock);
