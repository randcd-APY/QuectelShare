...
#define QCAMERA_MSG_HIGH printf
...
{
	int ret;
    if ( 0 > g_muxer_param.vi )
    {
       QCAMERA_MSG_HIGH("vi less than 0");
        return ;
    }
    ...
    AVStream *pst = m_pOc->streams[ g_muxer_param.vi ];
    // Init packet
    AVPacket pkt;
    // 我的添加，为了计算pts
    av_init_packet( &pkt );
...
void qcamera_close_mp4()
{
    ...
    if (m_pOc)
	{
		printf("write trailer start\n");
        av_write_trailer(m_pOc);
		printf("write trailer end\n");
	}
    if (m_pOc && !(m_pOc->oformat->flags & AVFMT_NOFILE))
	{
		printf("avio close start\n");
        avio_close(m_pOc->pb);
		printf("avio close end\n");
	}

    if (m_pOc)
    {
		printf("free context start\n");
        avformat_free_context(m_pOc);
		printf("free context end\n");
        m_pOc = NULL;
    }
}
