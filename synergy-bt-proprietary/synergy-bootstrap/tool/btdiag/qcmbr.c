...
unsigned short Calc_CRC_16_l(BYTE pBufPtr[], int bufLen)
{
     ...
     */
    static const unsigned short _aCRC_16_L_Table[] =
    {
            ...
            if ((inputDIAGMsg[10] == 1) && (inputDIAGMsg[1] == 11) && (inputDIAGMsg[2] == 4) && (inputDIAGMsg[3] == 0))//check 4B packet
            {
                ...
                switch (connectionTypeOptions)
                {
                    case SERIAL:
                        ...
                        ret = QCA_SendHciCommand(msgToDUT, lengthOfPayload);

                        printf("%s: write %d success\n", __FUNCTION__, ret);

                        if (ret != lengthOfPayload)
                        {
                            printf("%s: Send failed with ret value: %d\n", __FUNCTION__, ret);
                            return 0;
                        }
