...
start_synergy_app_for_carkit()
{
    ...
    if [ "$enable_log" = "0" ]; then
        $SYNERGY_APP_FULL_NAME --bc-transport $TRANSPORT --bc-port $PORT --bc-baud $INIT_BAUDRATE --bc-bs-baud $NEW_BAUDRATE --platform $PLATFORM --bc-bs-device-name $IMX_NAME --bt-dev carkit --bt-av-alsa-device hw:0,0  --bt-remote-address 2424:0E:CA7E44 --ibs $IBS --lpm $LPM --use-key $KEY_INPUT
    else
        $SYNERGY_APP_FULL_NAME --bc-transport $TRANSPORT --bc-port $PORT --bc-baud $INIT_BAUDRATE --bc-bs-baud $NEW_BAUDRATE --platform $PLATFORM --bc-bs-device-name $IMX_NAME --bt-dev carkit --bt-av-alsa-device hw:0,0 --bt-remote-address 2424:0E:CA7E44 --ibs $IBS --lpm $LPM --use-key $KEY_INPUT --log-cleartext-stdout --log-pcap-file $WIRESHARE_LOG --log-btsnoop-file $HCI_LOG --tui-nocls
    fi
}
...
start_synergy_app_for_phone()
{
    ...
    if [ "$enable_log" = "0" ]; then
        $SYNERGY_APP_FULL_NAME --bc-transport $TRANSPORT --bc-port $PORT --bc-baud $INIT_BAUDRATE --bc-bs-baud $NEW_BAUDRATE --platform $PLATFORM --bc-bs-device-name $IMX_NAME --bt-dev phone --bt-av-wav-file-in /data/csr.wav --bt-remote-address 0002:5B:00FF07 --ibs $IBS --lpm $LPM --use-key $KEY_INPUT
    else
        $SYNERGY_APP_FULL_NAME --bc-transport $TRANSPORT --bc-port $PORT --bc-baud $INIT_BAUDRATE --bc-bs-baud $NEW_BAUDRATE --platform $PLATFORM --bc-bs-device-name $IMX_NAME --bt-dev phone --bt-av-wav-file-in /data/csr.wav --bt-remote-address 0002:5B:00FF07 --ibs $IBS --lpm $LPM --use-key $KEY_INPUT --log-cleartext-stdout --log-pcap-file $WIRESHARE_LOG --log-btsnoop-file $HCI_LOG --tui-nocls
    fi
}
