...
typedef struct
{
    ...
    CsrUint8 own_address_type;         /* Own Address Type used in LE connection
...
#define DM_SM_INIT_FAIL_WRITE_SC_HOST_SUPPORT               ((CsrUint16)0x0020)
typedef struct
{
...
typedef struct
{
    ...
    CsrUint8                 *oob_hash_c;
    CsrUint8                 *oob_rand_r;
    CsrUint16                key_distribution;   /* SMP only */
...
typedef struct
{
    ...
    CsrSchedQid               phandle;   /* destination phandle */
    CsrUint8                 status;    /* Generation success or failed */
    PHYSICAL_TRANSPORT_T    tp_type;
...
/*----------------------------------------------------------------------------*
 * PURPOSE
 *      This API generates a random address of type requested and then sets the
 *      value to controller. If address requested is RPA, it triggers a TGAP
 *      timer equal to rpa_timeout. Upon expiry a new rpa is generated and set.
 *      If address generation is offloaded to the controller then controller
 *      will generate a new local rpa post expiry of the rpa_timeout.
 ...
 *      address is generated and set with controller.
 *
 *      Note:
 ...
 *      configured address.
 *      -Usage of DM_SM_CONFIGURE_LOCAL_ADDRESS_REQ API shall be disallowed
 *      once the application calls this API.
 *----------------------------------------------------------------------------*/
typedef struct
{
    ...
    CsrUint16                rpa_timeout;                /* TGAP or rpa timeout in seconds */
 ...
 *      When connected and if application supplies peer Identity Address then
 ...
 *      Identity Adrress is present in resolving list.
 *
 *      If peer Identity address supplied is all zero then flags field shall be
 *      ignored and current local configured address shall be returned in the
 *      confirmation message.
 * NOTE:
 *      Valid connection address(notified to application) shall be passed as
 *      tp_peer_addrt
 *-----------------------------------------------------------------------------*/
typedef struct
{
    ...
    CsrUint16            flags;         /*  See definition */
...
typedef struct
{
    ...
    CsrUint8                 page_num;
    CsrUint8                 max_page_num;
    CsrUint8                 lmp_ext_features[8]; /* As per the specification */
...
typedef struct
{
    ...
    CsrUint16 supervision_timeout;   /*!< link timeout value as number 0f 10 ms */
...
typedef struct
{
    ...
    CsrUint16 conn_interval_max;     /*!< Maximum allowed connection interval
                                         as number of 1.25 ms */
    CsrUint16 conn_latency;          /*!< connection slave latency */
    CsrUint16 supervision_timeout;   /*!< link timeout value as number of 10 ms */
    CsrUint16 result;                /*!< L2CAP_CONNECTION_PARAMETER_UPDATE_ACCEPT or HCI_SUCCESS - if parameters are acceptable,
...
typedef struct
{
    dm_prim_t           type;             /* Prim ID */
    CsrSchedQid           phandle;          /* destination phandle */
    CsrUint8             status;           /* Success or failure */
    CsrUint8             lt_addr;
    CsrUint16            interval;
} DM_HCI_SET_CSB_CFM_T;
...
typedef struct
{
    ...
    CsrUint8             lt_addr;            /* Reserved LT_ADDR */
} DM_HCI_SET_CSB_RECEIVE_CFM_T;
...
typedef struct
{
    ...
    CsrUint8             status;             /* Success or failure - See Note1 */
    BD_ADDR_T           bd_addr;            /* Remote device BD Address */
    CsrUint32            clock_offset;       /* Difference with master clock */
    CsrUint8             map[10];            /* AFH map data */
    CsrUint8             lt_addr;            /* Reserved LT_ADRR */
    CsrUint32            next_broadcast_instant; /* Clock of a future broadcast */
...
typedef struct
{
    ...
    BD_ADDR_T           bd_addr;            /* Remote device BD address */
    CsrUint8             lt_addr;
    CsrUint32            clock;              /* clock when data is received */
    CsrUint32            offset;
...
typedef DM_COMMON_T DM_CRYPTO_GENERATE_PUBLIC_PRIVATE_KEY_REQ_T;
typedef DM_COMMON_T DM_CRYPTO_GENERATE_SHARED_SECRET_KEY_REQ_T;
typedef DM_COMMON_T DM_CRYPTO_ENCRYPT_REQ_T;
typedef DM_COMMON_T DM_CRYPTO_HASH_REQ_T;
typedef DM_COMMON_T DM_ULP_ENABLE_ZERO_SLAVE_LATENCY_REQ_T;

typedef DM_UPSTREAM_COMMON_T DM_CRYPTO_GENERATE_PUBLIC_PRIVATE_KEY_CFM_T;
typedef DM_UPSTREAM_COMMON_T DM_CRYPTO_GENERATE_SHARED_SECRET_KEY_CFM_T;
typedef DM_UPSTREAM_COMMON_T DM_CRYPTO_ENCRYPT_CFM_T;
typedef DM_UPSTREAM_COMMON_T DM_CRYPTO_HASH_CFM_T;
typedef DM_UPSTREAM_COMMON_T DM_ULP_ENABLE_ZERO_SLAVE_LATENCY_CFM_T;
/*------------------------------------------------------------------------
 *
 *      UNION OF       PRIMITIVES
 *
