...
/*******************************************************************************
    ...
    DESCRIPTION
        Copies the null terminated UTF-8 string pointed at by source into the
        memory pointed at by target, and gaurentees a null terminated string.
        The copy limits to the minimum of (source length and count - 1) that is passed,
        hence assuring no array over bound writes.

        This function copies max up to count - 1 from null terminated source to 
        target and guarantees to NULL terminate the target string. Returning source length
        from this function makes possible to check whether any truncation happened.

    PARAMETERS
        target - Pointer to the target memory where the content is to be copied.
        source - UTF-8 string to be copied.
        count  - Size of the target string (not length).

    RETURNS
        Returns - Length of the source string that function tried to create
