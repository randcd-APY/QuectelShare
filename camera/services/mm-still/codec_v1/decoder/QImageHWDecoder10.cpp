 ...
 * Notes: none
 *==========================================================================*/
bool QImageHW10Decoder::IsAvailable(QIDecodeParams& aParams __unused)
{
  int lrc = QI_SUCCESS;
...
QImageHW10Decoder::~QImageHW10Decoder()
{
  /*Stop and join the decoder thread*/
...
int QImageHW10Decoder::setScaleFactor()
{
  ...
  size_t count = sizeof(gScaleFactor)/sizeof(gScaleFactor[0]);
...
int QImageHW10Decoder::setPaddedDimensions()
{
  ...
  if (MONO_CNT == mOutputImage->PlaneCount()) {
    ...
    mPaddedSize[IY] = lPaddedSize;
    mPaddedSize[IC] = QISize((uint32_t)((float)lPaddedSize.Width() * lHSS * 2),
      (uint32_t)((float)lPaddedSize.Height() * lVSS));
  } else if (P_CNT == mOutputImage->PlaneCount()) {
    mPaddedSize[IY] = lPaddedSize;
    mPaddedSize[IC1] = mPaddedSize[IC2] =
      QISize((uint32_t)((float)lPaddedSize.Width() * lHSS),
      (uint32_t)((float)lPaddedSize.Height() * lVSS));
  }
  return QI_SUCCESS;
}
