...
{
  if (NULL == mQuantTable) {
    mQuantTable = new QIQuantTable[QIQuantTable::QTABLE_MAX];
    if (NULL == mQuantTable) {
...
/*===========================================================================
 * Function: setDefaultHuffTables
 *
 * Description: set the standard huffman tables
 *
 * Input parameters:
 *
 * Return values:
 ...
 * Notes: none
 *==========================================================================*/
int QIEncodeParams::setDefaultHuffTables()
{
  if (NULL == mHuffTable) {
    mHuffTable = new QIHuffTable[4];
    if (NULL == mHuffTable) {
 ...
 *
 * Input parameters:
 *   aQuantTable - pointer to QIQuantTable object
 *
 * Return values:
 *   QI_SUCCESS
 *   QI_NO_MEMORY
 *
  * Notes: none
 *==========================================================================*/
int QIEncodeParams::setQuantTables(QIQuantTable *aQuantTable)
{
  ...
  for (int i = 0; i < QIQuantTable::QTABLE_MAX; i++) {
    mQuantTable[i].setType(aQuantTable[i].Type());
    mQuantTable[i].setTableValue(aQuantTable[i].Table());
    /* custom tables are already adjusted based on quality. Hence no need
       to set them */
  }
...
{
  mQuality = aOther.mQuality;
  mCrop = aOther.mCrop;
  mOutputSize = aOther.mOutputSize;
  mInputSize = aOther.mInputSize;
  mRestartInterval = aOther.mRestartInterval;
  mRotation = aOther.mRotation;
  mNumOfComponents = aOther.mNumOfComponents;
  mUseHiSpeed = aOther.mUseHiSpeed;
