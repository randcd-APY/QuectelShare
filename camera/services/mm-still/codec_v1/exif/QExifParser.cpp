 ...
 *==========================================================================*/
int QExifParser::Start(int aSync __unused)
{
  int lrc = QI_SUCCESS;
  if (mBuffer == NULL) {
...
int QExifParser::ParseSOF(jpeg_frame_info_t *p_frame_info)
{
  ...
  for (;;) {
    ...
    default: {
      ...
      if (bytesToSkip >= 2)
        bytesToSkip =(uint16_t)(bytesToSkip - 2);
  ...
  jpeg_marker_t marker)
{
  ...
  for (i = 0; i < comp_infos_len; i++) {
    ...
    p_info->sampling_h = (uint8_t)(temp_byte >> 4);
  ...
  p_frame_info->num_comps = (uint8_t)comp_infos_len;
  ...
  if (comp_infos_len == 1) {
    ...
    if (p_frame_info->p_comp_infos[0].sampling_h == 2) {
      subsampling = (uint8_t)(subsampling - 2);
    }
    if (p_frame_info->p_comp_infos[0].sampling_v == 2) {
      subsampling = (uint8_t)(subsampling - 1);
    }
...
{
  int rc = QI_SUCCESS;
  uint8_t temp_byte, comp_sels_len, j;
  uint32_t len, i;
  ...
  if (len != (uint32_t)(6 + comp_sels_len * 2)) {
  ...
  for (i = 0; i < comp_sels_len; i++) {
    ...
    temp_byte = FetchBytes();
    p_scan_info->p_selectors[i].dc_selector = (uint8_t)(temp_byte >> 4);
  ...
  if (QI_SUCCEEDED(rc)) {
    ...
    temp_byte = FetchBytes();
    p_scan_info->succ_approx_h = (uint8_t)(temp_byte >> 4);
...
int QExifParser::ProcessDQT(jpeg_frame_info_t *p_frame_info)
{
  ...
  for (i = 0; i < length / 64; i++) {
    ...
    p_frame_info->quant_precision = (uint8_t)(temp_byte >> 4);
    dest = temp_byte & 0x0F;
    ...
    p_frame_info->qtable_present_flag |= (uint8_t)(1 << dest);
    ...
    for (j = 0; j < 64; j++) {
      p_frame_info->p_qtables[dest][j] = (p_frame_info->quant_precision) ?
        (uint16_t)Fetch2Bytes() : (uint16_t)FetchBytes();
    }
...
int QExifParser::ProcessDHT(jpeg_frame_info_t *p_frame_info)
{
  ...
  while (size > 16) {
    ...
    // Set the bit flag to indicate which table are read
    p_frame_info->htable_present_flag |= (uint8_t)(1 << index);
...
int QExifParser::ProcessApp0()
{
  ...
  if (QI_SUCCEEDED (rc)) {
    ...
    p_new_tag->entry.data._short = (uint16_t)(FetchBytes() + 1);
