...
#define PARSE_F_A1(S,F) (parseValArr<float>("%f", #F, &((S)->F[0]), \
    (int32_t)(sizeof((S)->F)/sizeof((S)->F[0]))))
/**
...
#define PARSE_F_A2(S,F) (parseValArr<float>("%f", #F, &((S)->F[0][0]), \
    (int32_t)(sizeof((S)->F)/sizeof((S)->F[0][0]))))
  ...
  uint32_t aSize)
{
  ...
  PARSE_I(&lAwbData, ccm_update.awb_ccm_enable);

  PARSE_F_A2(&lAwbData, ccm_update.ccm);

  // AEC DATA
  PARSE_F_A1(&lAecData, aec_get.real_gain);
  PARSE_U_A1(&lAecData, aec_get.linecount);
  PARSE_F(&lAecData, aec_get.led_off_real_gain);
  ...
  return (int32_t)((uint8_t *)mDstPtr - aTuningInfo);
}
    ...
    const char *aTag, T *aValPtr, int aLen)
{
  ...
  *(mDstPtr-1) = '\0';
  ...
  return (int32_t)(mDstPtr - lStartDst);
}
