...
* Description:
==============================================================================*/
void OMXJpegDecoder::ReadError(ErrorType aErrorType __unused)
{
...
* Description:
==============================================================================*/
void OMXJpegDecoder::ReadComplete(QIBuffer &aBuffer __unused)
{
  QIDBG_MED("%s:%d] ", __func__, __LINE__);
}
...
* Description:
==============================================================================*/
void OMXJpegDecoder::ReadFragmentDone(QIBuffer &aBuffer __unused)
{
  QIDBG_MED("%s:%d] ", __func__, __LINE__);
}
...
OMX_ERRORTYPE OMXJpegDecoder::configureDecodeData()
{
  ...
  //configure the input image size
  ...
  /*decode params*/
  m_decodeParams.setInputSize(m_inputSize);
...
OMX_ERRORTYPE OMXJpegDecoder::decodeImageHeader(OMX_BUFFERHEADERTYPE *a_inBuffer)
{
  ...
  if (!m_abort_flag) {
    ...
    m_outImgLength = (OMX_U32)((float)CEILING16(m_mainFrameInfo->width) *
                     (float)CEILING16(m_mainFrameInfo->height) *
                     lImgSzFactor);

     /*config the inPort info from the header*/
     m_inPort->format.image.nFrameWidth = m_mainFrameInfo->width;
     m_inPort->format.image.nFrameHeight = m_mainFrameInfo->height;
     m_inPort->format.image.nStride = CEILING16(m_mainFrameInfo->width);
     m_inPort->format.image.nSliceHeight = CEILING16(m_mainFrameInfo->height);
     m_inPort->nBufferSize = m_outImgLength;
...
int OMXJpegDecoder::DecodeComplete(QImage *aOutputImage __unused)
{
  QIDBG_MED("%s:%d] ", __func__, __LINE__);
  OMX_ERRORTYPE lret = OMX_ErrorNone;
