...
{
  QIDBG_MED("%s %d:] Component ptr is %p", __func__, __LINE__, aobj);
  QOMXImageCodec *lobj = (QOMXImageCodec *)aobj;
  OMX_COMPONENTTYPE *lcomponent_fns =  lobj->ComponentHandle();
  lcomponent_fns->nSize               = (uint32_t)sizeof(OMX_COMPONENTTYPE);
  ...
  OMX_IN OMX_PTR appData)
{
  ...
  if (lthis){
    QIDBG_LOW("%s:%d] This ptr addr %p",__func__, __LINE__, lthis);
...
OMX_ERRORTYPE qomx_component_deinit(OMX_IN OMX_HANDLETYPE hComp)
{
  ...
  if(lthis){
    ...
    QIDBG_MED("%s %d: Component State: %d Calling Deinit", __func__, __LINE__,
      (int)lstate);
