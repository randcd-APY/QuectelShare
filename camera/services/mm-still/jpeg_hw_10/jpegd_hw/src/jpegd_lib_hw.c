...
static const uint32_t g_step_sz[] = { 8, 1, 2, 3, 4, 5, 6, 7 };

static const uint32_t g_hstep_sz_v2[] =
{ 128, 128, 128, 128, 128, 128, 128, 128 };

/**
  * step size for 8994
  */
static const uint32_t g_hstep_sz_8994[] =
  ...
  struct msm_jpeg_hw_cmd* p_hw_cmd)
{
  ...
  if (p_hw_cmds) {
    p_hw_cmds->m = (uint32_t)(size / sizeof(struct msm_jpeg_hw_cmd));
    if (p_hw_cmd) {
 ...
 * DESCRIPTION:
 *==========================================================================*/
int jpegd_hw_input_len_cfg(int fd, uint32_t input_len)
{
  struct msm_jpeg_hw_cmd hw_cmd;
  int rc;
  uint32_t val = 0;
  ...
  uint32_t version)
{
  ...
  int rc = 0;
  uint32_t val;
  uint32_t image_width = write_config->image_width;
  uint32_t image_height = write_config->image_height;
  uint32_t actual_width = write_config->actual_width;
  float pl_factor_w[3] = {1.0, 1.0, 1.0};
  float pl_factor_h[3] = {1.0, 1.0, 1.0};
  int use_pln2 = 0;
  const uint32_t *l_hstep_sz;
  ...
  if (IS_8994_OR_GREATER(version)) {
    l_hstep_sz = g_hstep_sz_8994;
    l_step_fact = &g_hstep_factor_v2[0];
  } else if (IS_8974_V2(version)) {
  ...
  plane0_hstep = (uint32_t)((float)l_hstep_sz[scale_type] *
      l_step_fact[JPEGD_PLN_0][we_cfg->jpegdFormat]);
  plane1_hstep = (uint32_t)((float)l_hstep_sz[scale_type] *
      l_step_fact[JPEGD_PLN_1][we_cfg->jpegdFormat]);
  plane2_hstep = (uint32_t)((float)l_hstep_sz[scale_type] *
  ...
  MEM_OUTF2(&val, PLN1_WR_BUFFER_SIZE, WIDTH, HEIGHT,
    ((float)image_width * pl_factor_w[1]),
    ((float)image_height * pl_factor_h[1]));

  p_jpegd_writes(PLN1_WR_BUFFER_SIZE, val);

  val = 0;
  MEM_OUTF2(&val, PLN2_WR_BUFFER_SIZE, WIDTH, HEIGHT,
    ((float)image_width * pl_factor_w[2]),
    ((float)image_height * pl_factor_h[2]));
  p_jpegd_writes(PLN2_WR_BUFFER_SIZE, val);

  p_jpegd_writes(PLN0_WR_STRIDE, stride);
  p_jpegd_writes(PLN1_WR_STRIDE, (uint32_t)((float)stride * pl_factor_w[1]));
  p_jpegd_writes(PLN2_WR_STRIDE, (uint32_t)((float)stride * pl_factor_w[2]));
  ...
  MEM_OUTF2(&val, PLN0_WR_BLOCK_CFG, BLOCKS_PER_COL, BLOCKS_PER_ROW,
   ...
   num_blocks_height = (uint32_t) ((float)image_height * pl_factor_h[1]);
   num_blocks_height = DIV_CEILING(num_blocks_height, plane1_vstep);

   num_blocks_width = (uint32_t) ((float)image_width * pl_factor_w[1]);
  ...
  num_blocks_height = (uint32_t) ((float)image_height * pl_factor_h[2]);
  num_blocks_height = DIV_CEILING(num_blocks_height, plane2_vstep);

  num_blocks_width = (uint32_t) ((float)image_width * pl_factor_w[2]);
...
int jpegd_hw_huff_config(int fd, jpegd_cmd_huff_cfg_t *huff_cfg)
{
  ...
  p_hw_cmds =
    jpegd_hw_cmd_malloc_and_copy(sizeof(struct msm_jpeg_hw_cmd) *
    (size_t)(1 + 1 + 12 + 12 + huff_tbl_size + huff_tbl_size + 1),
     NULL);
  if (!p_hw_cmds) {
