...
--------   ---     -------------------------------------------------------
12/10/09   vma     Removed the dependency on os_thread_detach_jpg by joining
...
static int jpegd_engine_sw_start(jpegd_engine_obj_t *p_obj)
{
    ...
    rc = os_thread_create_jpg(&p_engine->decode_thread, jpegd_engine_sw_decode_thread, (void *)p_engine);
    ...
    if (rc)
    {
        ...
        os_mutex_unlock_jpg(&p_engine->mutex);
        JPEG_DBG_HIGH("jpegd_engine_sw_start: os_thread_create_jpg() failed: %d\n", rc);
