    ...
    jpege_engine_event_handler_t  p_handler)
{
    ...
    (void)os_mutex_init_jpg(&(p_hybrid->mutex));
    (void)os_cond_init_jpg(&(p_hybrid->cond));
    // Initialize the condition used for last buffer output
    os_cond_init_jpg(&(p_hybrid->final_output_cond));
    ...
    jpege_engine_dst_t   *p_dest)
{
    ...
    count = 0;
    os_mutex_lock_jpg(&(p_hybrid->mutex));
           ...
           count < p_hybrid->job_count)
    {
        ...
        if (JPEG_FAILED(rc))
        {
            ...
            os_mutex_unlock_jpg(&(p_hybrid->mutex));
            return rc;
        }
    ...
    jpege_engine_obj_t *p_obj)
{
    ...
    os_cond_signal_jpg(&(p_hybrid->final_output_cond));
    while (count < p_hybrid->engine_count)
    {
    ...
    jpege_engine_obj_t *p_obj)
{
    ...
    if (p_obj)
    {
        ...
        if (p_hybrid)
        {
            ...
            (void)os_mutex_destroy_jpg(&(p_hybrid->mutex));
            (void)os_cond_destroy_jpg(&(p_hybrid->cond));
            (void)os_cond_destroy_jpg(&(p_hybrid->final_output_cond));
