/*****************************************************************************
 * Copyright (c) 2008-2011, 2014-2015, 2017 Qualcomm Technologies, Inc.
 * All Rights Reserved.
    ...
    void                  *p_user_data)
{
    ...
    (void)os_recursive_mutex_init_jpg(&(p_encoder->mutex));
    (void)os_cond_init_jpg(&(p_encoder->cond));
    ...
    exif_info_obj_t  *p_exif_info)
{
    ...
    os_mutex_lock_jpg(&(p_encoder->mutex));

    if (p_encoder->state == JPEGE_ABORTING)
    {
        p_encoder->state = JPEGE_IDLE;
        os_mutex_unlock_jpg(&(p_encoder->mutex));
...
void jpege_destroy(jpege_obj_t *p_obj)
{
    if (p_obj)
    {
        ...
        if (p_encoder)
        {
            ...
            (void)os_mutex_destroy_jpg(&(p_encoder->mutex));
            (void)os_cond_destroy_jpg(&(p_encoder->cond));
