/*****************************************************************************
 * Copyright (c) 2009-2011, 2014-2015, 2017 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
05/18/10   vma     Rely on os_thread_detach_jpg for thread detachment rather
                   than joining
    ...
    jpege_engine_event_handler_t  p_handler)
{
    ...
    (void)os_mutex_init_jpg(&(p_engine->mutex));             // Initialize the mutex
    (void)os_cond_init_jpg(&(p_engine->cond));               // Initialize the condition variables
    (void)os_cond_init_jpg(&(p_engine->output_cond));
    // Initialize the condition used for output thread consume
    (void)os_cond_init_jpg(&(p_engine->consume_cond));
    // Initialize the condition used for last buffer output
    (void)os_cond_init_jpg(&(p_engine->final_output_cond));
    ...
    if (os_thread_create_jpg_ex(&(p_engine->output_thread),
                            ...
                            (uint8_t*)jpeg_engine_bs_output_thread_name))
    {
        ...
        (void)os_mutex_destroy_jpg(&(p_engine->mutex));
        (void)os_cond_destroy_jpg(&(p_engine->final_output_cond));
        (void)os_cond_destroy_jpg(&(p_engine->cond));
        (void)os_cond_destroy_jpg(&(p_engine->consume_cond));
        (void)os_cond_destroy_jpg(&(p_engine->output_cond));
#if 0 // No PPF
    ...
    jpege_engine_obj_t *p_obj)
{
    ...
    (void)os_mutex_lock_jpg(&(p_engine->mutex));
    if (p_engine->is_active)
    {
        p_engine->abort_flag = true;
        while (p_engine->abort_flag)
        {
            (void)os_cond_wait_jpg(&(p_engine->cond), &(p_engine->mutex));
        }
    }
    (void)os_mutex_unlock_jpg(&(p_engine->mutex));
    return JPEGERR_SUCCESS;
}
    ...
    jpege_engine_obj_t *p_obj)
{
    if (p_obj)
    {
        ...
        if (p_engine)
        {
            ...
            (void)os_cond_signal_jpg(&(p_engine->output_cond));
            (void)os_cond_signal_jpg(&(p_engine->consume_cond));
            (void)os_cond_signal_jpg(&(p_engine->final_output_cond));
            (void)os_mutex_unlock_jpg(&(p_engine->mutex));

            // Join the output thread
            (void)os_thread_join_jpg(&p_engine->output_thread, NULL);
    ...
    jpege_engine_dst_t   *p_dest)
{
    ...
    rc = os_thread_create_jpg_ex(&(p_engine->thread),
    ...
    {
        (void)os_mutex_unlock_jpg(&p_engine->mutex);
        JPEG_DBG_ERROR("jpege_engine_bs_start: os_thread_create_jpg() failed: %d\n", rc);
        return JPEGERR_EFAILED;
    }
    ...
    os_thread_detach_jpg(&p_engine->thread);
...
static jpeg_event_t jpege_engine_bs_copy(jpege_engine_bs_t *p_engine)
{
    ...
    while (bytes_remaining)
    {
        ...
        if (p_engine->abort_flag)
        {
            ...
            p_engine->abort_flag = false;
            (void)os_cond_signal_jpg(&(p_engine->cond));
            (void)os_mutex_unlock_jpg(&(p_engine->mutex));
    ...
    jpege_engine_bs_thread(OS_THREAD_FUNC_ARG_T arg)
{
    ...
    while (p_engine->final_output_flag)
    {
        ...
        (void)os_cond_wait_jpg(&(p_engine->final_output_cond), &(p_engine->mutex));
    }
    ...
    jpege_engine_bs_output_thread(OS_THREAD_FUNC_ARG_T p_arg)
{
    ...
    for (;;)
    {
        ...
        {
            (void)os_cond_wait_jpg(&(p_engine->output_cond), &(p_engine->mutex));
        }
...
static int jpege_engine_bs_flush_output(jpege_engine_bs_t *p_engine)
{
    ...
    while (p_engine->pending_outputs[p_engine->pending_output_q_tail].is_valid)
    {
        ...
        (void)os_cond_wait_jpg(&(p_engine->consume_cond), &(p_engine->mutex));
