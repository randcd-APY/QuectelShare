/*******************************************************************************
* Copyright (c) 2009, 2014-2015, 2017 Qualcomm Technologies, Inc.
* All Rights Reserved.
...
#include "jpegerr.h"
#include <errno.h>
#define JPEG_DBG_HIGH_ENABLE

// thread functions
int os_thread_create_jpg(os_thread_t *p_thread, os_thread_func_t p_start_func, void *func_arg)
{
    return pthread_create(p_thread, NULL, p_start_func, func_arg);
}
int os_thread_create_jpg_ex(os_thread_t *p_thread, os_thread_func_t p_start_func,
...
int os_recursive_mutex_init_jpg(os_mutex_t *p_mutex)
{
    int rc;
...
int os_cond_timedwait_jpg(os_cond_t *p_cond, os_mutex_t *p_mutex, uint32_t ms)
{
    struct timespec ts;
...
int os_cond_broadcast_jpg(os_cond_t *p_cond)
{
