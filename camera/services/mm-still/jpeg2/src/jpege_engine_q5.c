    ...
    jpege_engine_dst_t   *p_dest)
{
    ...
    {
       // Wait for encode ack
        rc = os_cond_timedwait_jpg(&(p_engine->cond), &(p_engine->mutex), 2000);
        if (rc < 0) break;
    }
    if (rc < 0)
    {
    ...
    jpege_engine_obj_t *p_obj)
{
    ...
    os_cond_broadcast_jpg(&(p_engine->cond)); /*Signal the start routine if its waiting*/
    os_mutex_unlock_jpg(&(p_engine->mutex));
    return JPEGERR_SUCCESS;
}
    ...
    case JPEG_Q5_MSG_JPEGE_ENCODE_ACK:
        {
            ...
            os_cond_broadcast_jpg(&(p_engine->cond));
            os_mutex_unlock_jpg(&(p_engine->mutex));
        }
        break;
    case JPEG_Q5_MSG_JPEGE_IDLE_ACK:
        {
            ...
            if (done_wait)
            {
                ...
                os_thread_t thread;
                if (os_thread_create_jpg(&thread, jpege_engine_q5_notify_done,
                                   (void *)p_engine))
                {
