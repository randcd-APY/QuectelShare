...
int jpegd_engine_hw_event_handler(void* p_user, jpegd_event_t *p_evt)
{
  ...
  os_cond_signal_jpg(&p_engine->frame_done_cond);
  ...
  jpegd_engine_input_req_handler_t p_input_req_handler)
{
  ...
  os_mutex_init_jpg(&(p_engine->mutex));
  os_cond_init_jpg(&(p_engine->cond));
...
static int jpegd_engine_hw_start(jpegd_engine_obj_t *p_obj)
{
  ...
  if (p_engine->is_dthread_need_join) {
    ...
    (void)os_thread_join_jpg(&p_engine->decode_thread, NULL);
  }
  p_engine->is_dthread_need_join = true;

  /* Start Jpeg Decoding as a new thread*/
  rc = os_thread_create_jpg(&p_engine->decode_thread,
  ...
  if (rc) {
    ...
    os_mutex_unlock_jpg(&p_engine->mutex);
    JPEG_DBG_ERROR("%s:%d] os_thread_create_jpg() failed:%d",
  ...
  jpegd_engine_hw_t *p_engine)
{
  ...
  ********************************************************************/
  ...
  char *output_filename = "/data/misc/camera/dec.yuv";
