                            ...
                            void                   *p_user_data)
{
    ...
    os_mutex_init_jpg(&(p_processor->mutex));
    os_cond_init_jpg(&(p_processor->cond));
...
{
    os_mutex_lock_jpg(&(p_processor->mutex));
    ...
    *************************************************************************/
    ...
    {
        os_cond_wait_jpg(&(p_processor->cond), &(p_processor->mutex));
    }
    ...
    **************************************************************************/
    if (!p_processor->output_buffers[p_processor->output_buf_q_head].is_in_q)
    {
        os_mutex_unlock_jpg(&(p_processor->mutex));
...
void jpeg_postprocessor_destroy(jpeg_postprocessor_t *p_processor)
{
    if (p_processor)
    {
        ...
        os_mutex_destroy_jpg(&(p_processor->mutex));
        os_cond_destroy_jpg(&(p_processor->cond));
