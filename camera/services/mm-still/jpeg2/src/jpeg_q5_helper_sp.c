                        ...
                        jpeg_q5_event_handler_t  p_handler)
{
    ...
    os_mutex_init_jpg(&(p_helper->mutex));
    os_cond_init_jpg(&(p_helper->cond));

    // Create thread to listen for kernel event
    os_mutex_lock_jpg(&(p_helper->mutex));
    os_thread_create_jpg(&(p_helper->thread),
                   jpeg_q5_event_listener, (void *)p_helper);
    os_cond_wait_jpg(&(p_helper->cond), &(p_helper->mutex));
    os_mutex_unlock_jpg(&(p_helper->mutex));
...
void jpeg_q5_helper_destroy(jpeg_q5_helper_t *p_helper)
{
    ...
    if (p_helper->state != JPEG_Q5_HELPER_UNINITIALIZED)
    {
        ...
        if (p_helper->state == JPEG_Q5_HELPER_READY)
        {
            ...
            os_thread_join_jpg(&p_helper->thread, NULL);
        }
        else
        {
            os_mutex_unlock_jpg(&(p_helper->mutex));
        }
        os_mutex_destroy_jpg(&(p_helper->mutex));
        os_cond_destroy_jpg(&(p_helper->cond));
        if (p_helper->fd >= 0)
...
static void* jpeg_q5_event_listener(void *arg)
{
    ...
    os_mutex_lock_jpg(&(p_helper->mutex));
    os_cond_signal_jpg(&(p_helper->cond));
    os_mutex_unlock_jpg(&(p_helper->mutex));
