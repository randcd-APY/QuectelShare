...
int jpeg_queue_init(jpeg_queue_t *p_queue)
{
    ...
    *p_queue= (jpeg_queue_t)p_q;
    ...
    (void)os_mutex_init_jpg(&(p_q->mutex));
    (void)os_cond_init_jpg(&(p_q->get_cond));
    (void)os_cond_init_jpg(&(p_q->abort_cond));
    ...
    uint32_t        enqueue_entry_cnt)
{
    ...
    os_cond_signal_jpg(&(p_q->get_cond));
    os_mutex_unlock_jpg(&(p_q->mutex));
    return JPEGERR_SUCCESS;
}
    ...
    uint32_t       time_out_ms)
{
    ...
    while (p_q->queue_cnt <= 0 && QUEUE_STATE_ABORT != p_q->state)
    {
        ...
        rc = os_cond_timedwait_jpg(&(p_q->get_cond), &(p_q->mutex), time_out_ms);
        if (JPEG_FAILED(rc))
        {
            ...
            if (QUEUE_STATE_ABORT == p_q->state)
            {
                ...
                os_cond_signal_jpg(&(p_q->abort_cond));
            }
...
void  jpeg_queue_destroy(jpeg_queue_t  *p_queue)
{
    if (p_queue)
    {
        ...
        if (p_q)
        {
            ...
            // queue clean up
            os_cond_destroy_jpg(&(p_q->get_cond));
            os_cond_destroy_jpg(&(p_q->abort_cond));
            os_mutex_destroy_jpg(&(p_q->mutex));
            JPEG_FREE(p_q);
        }
        *p_queue = NULL;
    }
...
int  jpeg_queue_abort(jpeg_queue_t  *p_queue)
{
    ...
    if (p_q->state == QUEUE_STATE_TIMEWAIT)
    {
        ...
        while (QUEUE_STATE_ABORT == p_q->state)
		{
			...
			os_cond_wait_jpg(&(p_q->abort_cond), &(p_q->mutex));
		}
	}
    p_q->state = QUEUE_STATE_ABORTED;
    os_mutex_unlock_jpg(&(p_q->mutex));
