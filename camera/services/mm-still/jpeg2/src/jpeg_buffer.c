...
int jpeg_buffer_init(jpeg_buffer_t *p_buffer)
{
    ...
    *p_buffer = (jpeg_buffer_t)p_buf;
    ...
    p_buf->is_empty = true;
    os_mutex_init_jpg(&(p_buf->mutex));
    os_cond_init_jpg(&(p_buf->cond));
...
{
    os_mutex_lock_jpg(&(p_buf->mutex));
    while (!p_buf->is_empty)
    {
        os_cond_wait_jpg(&(p_buf->cond), &(p_buf->mutex));
    }
    os_mutex_unlock_jpg(&(p_buf->mutex));
}
...
void jpeg_buffer_destroy(jpeg_buffer_t *p_buffer)
{
    ...
    if (p_buffer)
    {
        ...
        if (p_buf)
        {
            ...
            os_mutex_destroy_jpg(&(p_buf->mutex));
            os_cond_destroy_jpg(&(p_buf->cond));
            JPEG_FREE(p_buf);
        }
        *p_buffer = NULL;
    }
