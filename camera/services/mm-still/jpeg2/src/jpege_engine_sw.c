/*******************************************************************************
* Copyright (c) 2008-2012,2014-2015, 2017 Qualcomm Technologies, Inc.
* All Rights Reserved.
...
05/18/10   vma     Rely on os_thread_detach_jpg for thread detachment rather
                   than joining
05/11/10   sigu    Commenting out os_join_create_jpg, as it is trying to join on itself
    ...
    jpege_engine_hybrid_output_handler_t  p_output_handler)
{
    ...
    os_cond_init_jpg(&(p_engine->cond));                     // Initialize the condition variables
    os_cond_init_jpg(&(p_engine->output_cond));
    // Initialize the condition used for output thread consume
    os_cond_init_jpg(&(p_engine->consume_cond));
    // Initialize the condition used for last buffer output
    os_cond_init_jpg(&(p_engine->final_output_cond));
    ...
    jpege_engine_hybrid_cfg_t   *p_hybrid_cfg)
{
   ...
   // Make sure no threads are running
    ...
    {
        os_mutex_unlock_jpg(&p_engine->mutex);
        JPEG_DBG_ERROR("jpege_engine_sw_start: os_thread_create_jpg() failed: %d\n", rc);
    ...
    jpege_engine_hybrid_obj_t *p_obj)
{
    ...
    if (p_engine->is_active)
    {
        ...
        os_cond_signal_jpg(&(p_engine->output_cond));
        os_cond_signal_jpg(&(p_engine->consume_cond));
        os_cond_signal_jpg(&(p_engine->final_output_cond));
    ...
    jpege_engine_hybrid_obj_t *p_obj)
{
    if (p_obj)
    {
        ...
        if (p_engine)
        {
            ...
            if (p_engine->output_thread != NULL) {
              os_thread_join_jpg(&p_engine->output_thread, NULL);
            } else {
              JPEG_DBG_LOW("%s:%d] output_thread NULL\n", __func__, __LINE__);
            }
            ...
            os_cond_destroy_jpg(&(p_engine->final_output_cond));
            os_cond_destroy_jpg(&(p_engine->cond));
            os_cond_destroy_jpg(&(p_engine->consume_cond));
            os_cond_destroy_jpg(&(p_engine->output_cond));
    ...
    jpege_engine_sw_encode(OS_THREAD_FUNC_ARG_T arg)
{
    ...
    if (p_engine->final_output_flag && p_engine->abort_flag != 1)
    {
        ...
        os_cond_wait_jpg(&(p_engine->final_output_cond), &(p_engine->mutex));
    }
    os_mutex_unlock_jpg(&(p_engine->mutex));
