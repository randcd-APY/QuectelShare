  ...
  unsigned int sessionid)
{
  ...
  if (!module || strcmp(MCT_OBJECT_NAME(module), "afd")){
    AFD_ERR("AFD module name does not match!");
    return FALSE;
  }
  ...
  if (port == NULL){
    AFD_ERR("Failure creating AFD Port!");
    goto start_done;
  }
...
*  Return: Boolean
**/
static boolean afd_module_query_mod(mct_module_t *module, void *buf, unsigned int sessionid)
{
  (void) sessionid;
