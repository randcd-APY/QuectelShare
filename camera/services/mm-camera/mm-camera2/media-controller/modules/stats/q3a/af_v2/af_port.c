  ...
  af_output_data_t *af_out)
{
  ...
  if (af_out->pdaf_config.reset_to_default_config) {
    ...
    PD_early_interrupt_enabled &= af_port->pdaf_early_int_enabled_set_prop;
    AF_HIGH("HAF-TAF: PD_early_interrupt_enabled %d", PD_early_interrupt_enabled);
    af_config_t       af_conf;
    //if (af_conf.mask & MCT_EVENT_STATS_BF_FW &&
    ...
    AF_HIGH("HAF-TAF: PDAF ROI config MCT output: y %f %d ROI height %f x %f %d Width %f ",
  ...
  af_output_data_t *af_out)
{
  ...
  event.u.module_event.current_frame_id = (FOVC_MODE_ISP == af_port->fovc_data.fovc_mode) ?
    af_port->sof_id : (af_port->sof_id - frame_adjustment);
...
static void af_port_populate_reserve_tuning_params(
  ...
  *core_tuning, af_algo_tune_parms_adapter_t *tuning_info){
  ...
  if (resv_enabled) {
    ...
    core_tuning->tof_min_distance_range =
      (int) tuning_info->af_algo.af_haf.tof.tof_extended.tof_min_distance_range;
  ...
  mct_event_module_t *mod_evt)
{
  ...
  if (af_msg != NULL) {
    ...
    af_port_set_fovc_enable(af_port, enabled);
  ...
  if (stats_event) {
    ...
    if ((stats_event->stats_mask & (1 << MSM_ISP_STATS_BF)) ||
      ...
      if (stats_event->stats_mask & (1 << MSM_ISP_STATS_BF)) {
        ...
        boolean PD_early_interrupt_enabled =
          af_port->af_haf_tuning.pdaf.pd_extended.PD_early_interrupt_enabled;
        AF_LOW("Tuning PD_early_interrupt_enabled %d", PD_early_interrupt_enabled);
        PD_early_interrupt_enabled &= af_port->pdaf_early_int_enabled_set_prop;
        AF_HIGH("SetProp PD_early_interrupt_enabled %d", PD_early_interrupt_enabled);
  ...
  void *output)
{
  ...
  do {
    ...
    // If PD stat comes from baf interrupt this flag is TRUE,
    // If it comes from idviert/end of frame this flag is FALSE
    boolean is_early_interrupt =
      pdaf_data->pdaf_early_interrupt_data.is_early_interrupt;
    /* Pack data to sensor for parsing the buffer*/
    if (is_early_interrupt) { //call from baf interrupt/MCT_EVENT_MODULE_STATS_EXT_DATA
      early_stats_buff =
      ...
      AF_LOW("baf interrupt");
    } else { // call from end of frame MCT_EVENT_MODULE_ISP_DIVERT_TO_3A
      /* Pack data to sensor for parsing the buffer*/
      idivert_stats_buff =
    ...
    AF_HIGH("frame_id %d buf_idx %d pd_stats %p pd_stats_read_data_length %d is_early_interrupt: %d",
      input->info.frame_id, buf_idx, pdaf->pd_stats,
      input->u.pdaf_info.num_of_valid_data_wrt_camif, is_early_interrupt);
    ...
    if (pdaf_data->pdaf_early_interrupt_data.is_early_interrupt) {
      af_port->pdaf_processing_result_early_int =
        pdaf->stat_process_result;
    }

    AF_HIGH("frame_id %d buf_idx %d pd_stats %p pdaf_processing_result_early_int %d is_early_interrupt %d number of valid line %d",
      input->info.frame_id, buf_idx, pdaf->pd_stats,
      pdaf->stat_process_result, is_early_interrupt, pdaf->num_of_valid_data_wrt_camif);
  ...
  if (pdaf_data->pdaf_early_interrupt_data.is_early_interrupt) {
    AF_LOW("af_port_buf_div_pd_early_done"); ("af_port_buf_div_pd_early_done");
  ...
  boolean is_early_interrupt)
{
  ...
  pdaf_data->pdaf_early_interrupt_data.is_early_interrupt = is_early_interrupt;
  void* stats_buff = NULL;
  uint32_t buf_idx = 0;
  uint32_t frame_id = 0;
  q3a_bf_stats_t* early_stats_buff = NULL;

  // If PD stat comes from baf interrupt
  if (is_early_interrupt) { // PD stat from baf int.
    mct_event_stats_ext_t* stats_ext_event =
      (mct_event_stats_ext_t *)mod_evt->module_event_data;
    mct_event_stats_isp_t *stats_event = stats_ext_event->stats_data;
    ...
    AF_HIGH("BAF int. is_early_interrupt %d frame_id %d buf_idx %d stats_buff %p pd_data %p",
    ...
    AF_HIGH("idivert frame_id %d buf_idx %d pd_data %p stats_buff %p isp_buff %p is_early_interrupt: %d ",
      frame_id, buf_idx, isp_buff->vaddr, stats_buff, isp_buff, is_early_interrupt);
  }
  ...
  AF_HIGH("is_early_interrupt: %d frame_id %d buf_idx %d stats_buff %p rc %d",
    is_early_interrupt, frame_id, buf_idx, stats_buff, rc);
  ...
  mct_event_module_t *mod_evt)
{
  ...
  case MCT_EVENT_MODULE_STATS_EXT_DATA: {
    if (af_port->af_initialized) {
      ...
      // If pd early interrupt is supported and enabled
      boolean PD_early_interrupt_enabled =
        af_port->af_haf_tuning.pdaf.pd_extended.PD_early_interrupt_enabled;
      AF_LOW("Tuning PD_early_interrupt_enabled %d", PD_early_interrupt_enabled);
      PD_early_interrupt_enabled &= af_port->pdaf_early_int_enabled_set_prop;
      ...
      if (PD_early_interrupt_enabled) {
        ...
        // If PD stat message failed to be queued, call pd_early_done
        if (FALSE == rc) {
          AF_HIGH("Pdaf message failed to be queued");
          mct_event_stats_ext_t* stats_ext_event =
            (mct_event_stats_ext_t *)mod_evt->module_event_data;
          mct_event_stats_isp_t *stats_event = stats_ext_event->stats_data;
          q3a_bf_stats_t* early_stats_buff =
            (q3a_bf_stats_t*)stats_event->stats_data[MSM_ISP_STATS_BF].stats_buf;
          if (early_stats_buff) {
            af_port_buf_div_pd_early_done(af_port, &early_stats_buff->u.fw_stats.pd_data);
          }
        ...
        AF_LOW("PD_early_interrupt_enabled %d", PD_early_interrupt_enabled);
      }
 ...
 case MCT_EVENT_MODULE_IMGLIB_DCRF_OUTPUT: {
  ...
  case MCT_EVENT_MODULE_ISP_DIVERT_TO_3A: {
    ...
    AF_HIGH("Received PDAF stats: frame_id %d buf_idx %d pd_sata %p stats_buff %p pd_sata %p",
    ...
    // If pd stat has been already processed in early interrupt,
    // no need to process it again here
    if (FALSE == PD_early_interrupt_enabled ||
      af_port->pdaf_processing_result_early_int == PDAF_PDLIB_RESULT_PARTIAL) {
      boolean is_early_interrupt = FALSE;
      rc = af_port_handle_pdaf_stats(port, af_port, mod_evt, is_early_interrupt);
    }
    // If PD stat has been already processed, or af_port_handle_pdaf_stats reurns FALSE
    // means it couldn't enqueue PDAF stat in AF thread,
    // No need to idivert buffer any more and can be marked as done
    if (TRUE == PD_early_interrupt_enabled || FALSE == rc) {
      /* rc is FALSE means isp buffer will not be handled in af thread,
       * thus ACK here if buffer ready */
      AF_HIGH("af_port_buf_div_done");
...
boolean af_port_init(mct_port_t *port, unsigned int *session_id)
{
  ...
  private->pdaf_early_int_enabled_set_prop =
    q3a_prop_get("debug.camera.early_int_enable", "1");
...
static void af_port_update_roi_info(mct_port_t *port,
  ...
  af_port_private_t  *af_port = (af_port_private_t *)(port->port_private);
  /* save the roi for HAL3, will using this roi when report back to HAL */
  AF_HIGH("update roi to HAL x =%d, y=%d, dx=%d, dy=%d, weight=%d",
