  ...
  unsigned int sessionid)
{
  ...
  if (!module || strcmp(MCT_OBJECT_NAME(module), "asd")){
    ASD_ERR("ASD module name does not match!");
    return FALSE;
  }

  private = module->module_private;
  if (!private) {
    ASD_ERR("Private port NULL!");
    return FALSE;
  }
  ...
  ASD_LOW("Create ASD port!");
  port = mct_port_create("asd_sink");
  if (port == NULL){
    ASD_ERR("Failure creating ASD Port!");
    goto start_done;
  }

  ASD_LOW("Initi ASD port!");
  if (asd_port_init(port, sessionid) == FALSE) {
    ASD_ERR("ASD port init failure!");
    goto port_init_error;
  }

  ASD_LOW("Add ASD port!");
  if (mct_module_add_port(module, port) == FALSE) {
    ASD_ERR("Failure adding ASD port!");
    goto port_add_error;
  }
