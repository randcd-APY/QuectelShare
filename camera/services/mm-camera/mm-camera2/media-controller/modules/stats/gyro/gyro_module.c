...
/** gyro_module_dynamic_disable:
 *
 *  Returns true if user selects to disable the module using set prop
 **/
inline static boolean gyro_module_dynamic_disable(void)
{
  boolean rc = FALSE;
  STATS_DISABLE_GYRO_MODULE(rc);
  return rc;
}
...
mct_module_t* gyro_module_init(const char *name)
{
  ...
  if (strcmp(name, "gyro"))
    return NULL;
  if(!gyro_module_dynamic_disable()){
    ...
    IS_ERR("User selected to disable Gyro module");
    return NULL;
  }
