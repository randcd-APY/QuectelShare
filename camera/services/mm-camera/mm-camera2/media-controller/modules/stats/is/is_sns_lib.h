...
/* Initialize the IS sensors library */
void *sns_is_init(sns_lib_input_s *input_params);


/* Stop/flush/release any state/resources held by the IS library */
int sns_is_stop(void *sns_handle);
...
int set_bias(
  void *sns_handle,
  const int32_t *bias   /* in: z data not used.  Rely on autocal for z bias correction */
);
...
int get_last_pan(void *sns_handle, int32_t *angle);


/* Update the pan filter coefficient. Use this alpha to correct for pan when calculating the quaternion rotation. */
int set_alpha(void *sns_handle, double alpha);


void pan_filter_reset_after_breach(void *sns_handle, double alpha);
...
void set_sns_apps_offset(void *sns_handle, uint64_t t);


void poke_gyro_sample(void *sns_handle, uint64_t t, int32_t gx, int32_t gy, int32_t gz);


double interp_sample_linear_db(uint64_t t1, double y1, uint64_t t2, double y2,
  uint64_t t);
#endif
