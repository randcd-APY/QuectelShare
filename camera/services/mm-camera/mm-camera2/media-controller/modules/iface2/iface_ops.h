...
#ifndef __IFACE_OPS_H__
#define __IFACE_OPS_H__


typedef struct {
  void *parent;      /* parent pointer */
  uint32_t handle;   /* used to support multiple axi hws */
  int (*notify) (void *parent, uint32_t handle, uint32_t session_id,
...
typedef struct {
  ...
  int (*init) (void *ctrl, void *in_params, iface_notify_ops_t *notify_ops);
