...
#define C2D_WAIT_TIMEOUT 300000000

#define C2D_PTHREAD_COND_WAIT_TIME(cond_ptr, mutex_ptr, \
    timeout_ptr, ns, rc) \
  { \
...
typedef struct {
  ...
  c2d_buf_align_caps_t    buf_align_caps;
} c2d_hardware_caps_t;
...
typedef struct _c2d_hardware_t {
  ...
  void *c2d_utils;
  unsigned int (*get_c2d_stride_alignment) ();
} c2d_hardware_t;
...
typedef enum {
  ...
  C2D_HW_CMD_DIVERT_BUF,
  C2D_HW_CMD_RELEASE_DIVERT_BUF,
  C2D_HW_CMD_STREAM_WAIT,
...
static int32_t c2d_hardware_divert_buffer(c2d_hardware_t *c2dhw,
  c2d_hardware_event_data_t event_data);
