...
static float sensor_register_to_real_gain(unsigned int reg_gain)
{
    ...
    if (reg_gain < (unsigned int)(MIN_GAIN * 128)) {
        reg_gain = (unsigned int)(MIN_GAIN * 128);
    } else if (reg_gain > (unsigned int)(MAX_ANALOG_GAIN * 128)) {
        reg_gain = (unsigned int)(MAX_ANALOG_GAIN * 128);
    }
  ...
  __attribute__((unused)) float s_real_gain)
{
    ...
    (float)exp_info->sensor_digital_gain / ov2281_MAX_DGAIN_DECIMATOR;
  ...
  __attribute__((unused)) int is_hdr_enabled)
{
    ...
    /*
    1x << gain < 2x: 366a[2:0] = 0
    2x << gain < 4x: 366a[2:0] = 1
    4x << gain < 8x: 366a[2:0] = 3
    8x << gain < 8x: 366a[2:0] = 7
    */
    reg_setting->reg_setting[reg_count].reg_addr = 0x366A;
    if (gain >= 128 && gain < 256)
        reg_setting->reg_setting[reg_count].reg_data = 0x0;
    else if (gain >= 256 && gain < 512)
        reg_setting->reg_setting[reg_count].reg_data = 0x1;
    else if (gain >= 512 && gain < 1024)
        reg_setting->reg_setting[reg_count].reg_data = 0x3;
    else
        reg_setting->reg_setting[reg_count].reg_data = 0x7;
    reg_count++;
