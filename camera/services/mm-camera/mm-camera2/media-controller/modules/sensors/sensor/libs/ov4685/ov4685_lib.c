...
static unsigned int sensor_real_to_register_gain(float gain)
{
  ...
  if (gain < 2.0) {
    ...
    reg_gain_bit_10to8 = 7;
    reg_gain_bit_7to0  = (((unsigned char)(MAX_ANALOG_GAIN * 16)) - 8) & 0xFF;
    SHIGH("set max_analog_gain gain %f / set default 0x%02x / 0x%02x",
      gain, reg_gain_bit_10to8, reg_gain_bit_7to0);
  }
  ...
  __attribute__((unused)) float s_real_gain)
{
  ...
  exp_info->sensor_real_dig_gain =
    (float)exp_info->sensor_digital_gain / OV4685_MAX_DGAIN_DECIMATOR;
  ...
  __attribute__((unused)) int is_hdr_enabled)
{
  ...
  if (SENSOR_HDR_RAW == is_hdr_enabled) {
    SLOW("%s:%d hdr_mode, line long/short = (%d,%d), gain = 0x%x, digital_gain = 0x%x\n",
      __func__, __LINE__, line, s_linecount, gain, digital_gain);
    ...
    reg_setting->reg_setting[reg_count].reg_addr =
      sensor_lib_ptr.exp_gain_info.short_coarse_int_time_addr - 1;
    reg_setting->reg_setting[reg_count].reg_data = s_linecount >> 12;
    reg_count++;

    reg_setting->reg_setting[reg_count].reg_addr =
      sensor_lib_ptr.exp_gain_info.short_coarse_int_time_addr;
    reg_setting->reg_setting[reg_count].reg_data = s_linecount >> 4;
    reg_count++;
    ...
    reg_setting->reg_setting[reg_count].reg_addr =
      sensor_lib_ptr.exp_gain_info.short_global_gain_addr;
    reg_setting->reg_setting[reg_count].reg_data = (gain & 0xFF00) >> 8;
    reg_count++;

    reg_setting->reg_setting[reg_count].reg_addr =
      sensor_lib_ptr.exp_gain_info.short_global_gain_addr + 1;
    reg_setting->reg_setting[reg_count].reg_data = (gain & 0xFF);
    reg_count++;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = REG_DIGITAL_SHORT_GAIN_FRAC;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0xFF00) >> 8;
    reg_count++;

    reg_setting->reg_setting[reg_count].reg_addr = REG_DIGITAL_SHORT_GAIN_FRAC + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0xFF);
    reg_count++;
  } else {
    reg_setting->reg_setting[reg_count].reg_addr =
