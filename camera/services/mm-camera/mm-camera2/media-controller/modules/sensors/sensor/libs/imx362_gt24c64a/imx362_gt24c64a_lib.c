...
#include <stdio.h>
#include "debug_lib.h"
#include "imx362_gt24c64a_lib.h"


/**
 * FUNCTION: imx362_gt24c64a_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int imx362_gt24c64a_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < IMX362_GT24C64A_MIN_AGAIN)
    {
        SLOW("IMX362_GT24C64A Requested gain(%f) is lower than the sensor minimum(%d)",
          real_gain, IMX362_GT24C64A_MIN_AGAIN);
        real_gain = IMX362_GT24C64A_MIN_AGAIN;
    }
    else if (real_gain > IMX362_GT24C64A_MAX_AGAIN)
    {
        real_gain = IMX362_GT24C64A_MAX_AGAIN;
    }
...
/**
 * FUNCTION: imx362_gt24c64a_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float imx362_gt24c64a_register_to_real_gain(unsigned int reg_gain)
{
    float gain;

    if(reg_gain > IMX362_GT24C64A_MAX_AGAIN_REG_VAL)
        reg_gain = IMX362_GT24C64A_MAX_AGAIN_REG_VAL;
...
/**
 * FUNCTION: imx362_gt24c64a_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int imx362_gt24c64a_digital_gain_calc(
 float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = IMX362_GT24C64A_MIN_DGAIN_REG_VAL;
    float real_dig_gain = IMX362_GT24C64A_MIN_DGAIN;

    if(real_gain > IMX362_GT24C64A_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = IMX362_GT24C64A_MIN_DGAIN;
    }

    if(real_dig_gain > IMX362_GT24C64A_MAX_DGAIN)
    {
        real_dig_gain = IMX362_GT24C64A_MAX_DGAIN;
    }
  ...
  unsigned int line_count, sensor_exposure_info_t *exp_info, float s_real_gain)
{
    ...
    exp_info->reg_gain = imx362_gt24c64a_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain =
      imx362_gt24c64a_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      imx362_gt24c64a_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain =
      (float)exp_info->sensor_digital_gain / IMX362_GT24C64A_MAX_DGAIN_DECIMATOR;
    ...
    exp_info->s_reg_gain = imx362_gt24c64a_real_to_register_gain(sensor_s_real_gain);

    return 0;
}

/**
 * FUNCTION: imx362_gt24c64a_fill_exposure_array
 *
  ...
  __attribute__((unused)) int is_hdr_enabled)
{
    ...
    if(luma_delta != 0){
      ...
      reg_setting->reg_setting[reg_count].reg_addr = IMX362_GT24C64A_EXP_RATIO_ADDR;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX362_GT24C64A_DIG_GAIN_GLOBAL_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX362_GT24C64A_DIG_GAIN_GLOBAL_ADDR + 1;
  ...
  unsigned short awb_gain_b, struct camera_i2c_seq_reg_setting* reg_setting)
{
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX362_GT24C64A_ABS_GAIN_R_WORD_ADDR;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX362_GT24C64A_ABS_GAIN_B_WORD_ADDR;
