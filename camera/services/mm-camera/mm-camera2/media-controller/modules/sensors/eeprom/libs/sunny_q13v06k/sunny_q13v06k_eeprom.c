...
struct camera_i2c_reg_array g_reg_array[OTP_REG_ARRAY_SIZE];
...
{
  if(!e_ctrl){
    SERR("null pointer exception");
    return 0;
  }
  if(addr > OTP_DRV_END_ADDR){
    SERR("addr accessed is overflowed");
    return 0 ;
  }
...
static int sunny_q13v06k_read_wbdata(sensor_eeprom_data_t *e_ctrl)
{
  ...
  SLOW("vendor_id:%d;lens_id:%d; date of product:%d/%d/%d",
...
void sunny_q13v06k_generate_lensshading()
{
  ...
  if(otp_data.flag & 0x02){
    ...
    for(i = 0; i < sizeof(data_in); i++)
      ...
      if(otp_data.checksumLSC == checksumLSC &&
      otp_data.checksumOTP == checksumOTP)
        lenc_flag = 1;
      ...
      if(lenc_flag){
        ...
        SLOW("OTP decode successfully");
      }else{
        SHIGH("OTP decode failed");
      }
  }
}
 ...
 * Return: void.
 **/
static void sunny_q13v06k_format_afdata(sensor_eeprom_data_t *e_ctrl)
{
  uint8_t  flag = get_data_from_buffer(ADDR_VCM_INFO_FLAG, e_ctrl);
  uint8_t *buffer = 0;
  short af_infinity, af_marco;
  int dac_range;

  if((flag & 0xc0) == 0x40) {
    buffer = e_ctrl->eeprom_params.buffer +
      (ADDR_VCM_INFO_FLAG - ADDR_BASIG_INFO_FLAG) + 1;
  }else if((flag & 0x30) == 0x10) {
    buffer = e_ctrl->eeprom_params.buffer +
      (ADDR_VCM_INFO_FLAG - ADDR_BASIG_INFO_FLAG) + VCM_INFO_GRP_SIZE + 1;
  }else {
    SERR("no valid lsc otp data");
    return ;
  }

  af_infinity = buffer[0] << 2 | buffer[2] >> 6;
  af_marco    = buffer[1] << 2 | (buffer[2] & 0x30) >> 4;

  e_ctrl->eeprom_data.afc.infinity_dac = af_infinity * 0.827;
  e_ctrl->eeprom_data.afc.macro_dac    = af_marco * 1.118;

  SHIGH("OTP:AF Macro DAC 12bit= %d",
  (short)e_ctrl->eeprom_data.afc.macro_dac);
  SHIGH("OTP:AF Infinity DAC 12bit= %d",
  (short)e_ctrl->eeprom_data.afc.infinity_dac);

}

/** sunny_q13v06k_format_dcdata:
 ...
 * Return: Void
 */
void sunny_q13v06k_format_dcdata(sensor_eeprom_data_t *e_ctrl)
{
  ...
  e_ctrl->eeprom_data.dualc.system_calib_params.
  master_sensor_mirror_flip_setting = *(unsigned short *)&buffer[DC_OTHERS + 14];
...
void sunny_q13v06k_format_calibration_data(void *e_ctrl) {
  ...
  sunny_q13v06k_format_afdata(ectrl);
  sunny_q13v06k_format_dcdata(ectrl);
}
 ...
 * Return: void.
 **/
int sunny_q13v06k_get_raw_data(void *e_ctrl, void *data)
{
