...
isp_pipeline_t *isp_pipeline48_open(uint32_t isp_version, uint32_t isp_id)
{
  ...
  if (isp_version == ISP_SDM660) {
      ...
      curr_pipeline->max_nominal_pix_clk = ISP_PIX_CLK_480MHZ;
      curr_pipeline->max_turbo_pix_clk = ISP_PIX_CLK_576MHZ;
    ...
    curr_pipeline->max_nominal_pix_clk = ISP_PIX48_CLK_USER_MAX;
