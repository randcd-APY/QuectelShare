...
typedef struct {
  ...
  uint32_t raw_bhist_stats[MAX_BHIST_STATS];
  int32_t led_enable;
...
#define VALIDATE_TST_VEC(ip, op, range, str)({\
  if (((ip - op) > range) || ((op - ip) > range)) {\
      has_mismatch = 1; \
      if (show_mismatch) \
      CDBG_TV("\tBET: %s: Mismatch, %s, diff: %d, ip: %08x, op : %08x\n",\
      __func__, str, (ip - op), ip, op);\
  } else { \
      if (show_match) \
...
#define VALIDATE_TST_LUT64(ip, op, range, str, index)({\
    if (((ip - op) > range) || ((op - ip) > range)) {\
      has_mismatch = 1; \
      if (show_mismatch) \
        CDBG_TV("\tBET: %s: Mismatch, %s, Index :%d, diff: %llu, ip: %llx, op : %llx\n",\
         __func__, str, index, ip - op, ip, op);\
    } else{ \
      if (show_match) \
        CDBG_TV("\tBET: %s: Match, %s, Index :%d, diff: %llu, ip: %llx, op : %llx\n",\
         __func__, str, index, ip - op, ip, op);\
    }\
})
