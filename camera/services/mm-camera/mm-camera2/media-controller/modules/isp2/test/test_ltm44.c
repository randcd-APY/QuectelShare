...
#include "ltm/ltm44/ltm44.h"
#include "ltm_reg.h"
#include "camera_dbg.h"
#include <unistd.h>

/*===========================================================================
 * FUNCTION    - ltm_validate_wb_cfg -
 *
 * DESCRIPTION:
 *==========================================================================*/
//void ltm_validate_wb_cfg(ISP_ltm_WB_Cfg *in, ISP_ltm_WB_Cfg
 // *out)
//{
  ...
  //VALIDATE_TST_VEC(in->ch0Offset, out->ch0Offset, 0, "ch0Offset");
  //VALIDATE_TST_VEC(in->ch1Offset, out->ch1Offset, 0, "ch1Offset");
  //VALIDATE_TST_VEC(in->ch2Offset, out->ch2Offset, 0, "ch2Offset");
//}/*ltm_validate_wb_cfg*/

/*===========================================================================
 * FUNCTION    - ltm_validate_cfg -
 *
 * DESCRIPTION:
 *==========================================================================*/
  ...
  *out)
{
  ...
  VALIDATE_TST_VEC(in->dc_cfg.initCellNumX, out->dc_cfg.initCellNumX, 0, "dc_initCellNumX");
  VALIDATE_TST_VEC(in->ip_init.initCellNumX, out->ip_init.initCellNumX, 0, "ip_initCellNumX");
...
void ltm_populate_vpad_data(uint32_t *reg, ISP_LTMScaleVPadCfg *pcmd)
{
  void *dst = (void *)pcmd;
  uint32_t size = 1 * sizeof(uint32_t);
  memcpy(dst, (void *)(reg + (ISP_LTM_H_PAD_OFF + 4)/4), size);
...
{
  void *dst = (void *)pcmd;
  uint32_t size = 6 * sizeof(uint32_t);
  memcpy(dst, (void *)(reg + ISP_LTM_H_SCALE_OFF/4), size);

}
#if 0
void ltm_populate_VScale_data(uint32_t *reg, isp_ltm_scale_v_cfg_t *pcmd)
{
  void *dst = (void *)pcmd;
  ...
  vfe_test_table_t *in, vfe_test_table_t *out, uint32_t size, char* channel)
{
  uint32_t i;
  for (i = 0; i < size; i++) {
    //VALIDATE_TST_LUT((in->table[i] & 0xfff), (out->table[i] & 0xfff), 0, channel, i);
  ...
  void* test_output)
{
  ...
  //ltm_validate_wb_cfg(&in.wb_cfg, &out.wb_cfg);
  //ltm_validate_wb_cfg(&in.wb_right_cfg, &out.wb_right_cfg);
  //printf("edge_hv_nz\n");
  ltm_validate_cfg(&in.ltm_cfg, &out.ltm_cfg);
 // printf("edge_diag_nz\n");
//  ltm_validate_cfg_status(&in.ltm_status, &out.ltm_status);
 // printf("edge_hv_coeff\n");
