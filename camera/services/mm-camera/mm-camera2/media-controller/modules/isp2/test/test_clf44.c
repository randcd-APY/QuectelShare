...
#include "clf/clf44/clf44.h"
#include "clf/clf44/clf44_reg.h"
#include "camera_dbg.h"
#include <unistd.h>

/*===========================================================================
 * FUNCTION    - clf_validate_wb_cfg -
 *
 * DESCRIPTION:
 *==========================================================================*/
void clf_validate_wb_cfg(ISP_CLF_WB_Cfg *in, ISP_CLF_WB_Cfg
  *out)
{
...
{
  void *dst = (void *)pcmd;
  uint32_t size = ISP_CLF44_CFG_LEN_1 * sizeof(uint32_t);
  memcpy(dst, (void *)(reg + ISP_CLF44_CFG_OFF_1/4), size);
  dst = (void *)((uint8_t*)pcmd + size);
  memcpy(dst, (void *)(reg + ISP_CLF44_CFG_OFF_2/4), ISP_CLF44_CFG_LEN_2 * sizeof(uint32_t));
}
  ...
  void* test_output)
{
  ...
  clf_validate_wb_cfg(&in.wb_cfg, &out.wb_cfg);
  //clf_validate_wb_cfg(&in.wb_right_cfg, &out.wb_right_cfg);
