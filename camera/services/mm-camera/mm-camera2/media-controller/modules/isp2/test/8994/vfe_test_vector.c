...
#include "../module/isp46/isp_pipeline_reg.h"
...
#define SHOW_GAMMA_MISMATCH 0
#define SHOW_CC_MISMATCH 0
#define SHOW_CV_MISMATCH 0
#define SHOW_GIC_MISMATCH 0
#define SHOW_GTM_MISMATCH 0
#define SHOW_PEDES_MISMATCH 0
#define SHOW_ABF_MISMATCH 0
#define SHOW_CLF_MISMATCH 0
#define SHOW_CS_MISMATCH 0
#define SHOW_ROLLOFF_MISMATCH 0
#define SHOW_DEMOSAIC_MISMATCH 0
#define SHOW_BCC_MISMATCH 1
#define SHOW_BPC_MISMATCH 1
#define SHOW_LA_MISMATCH 0
#define SHOW_MCE_MISMATCH 0
#define SHOW_LINEAR_MISMATCH 0
  ...
  char* line, uint32_t *module_type)
{
  ...
  *module_type = 0;
  if (!mod || !line || !module_type) {
    CDBG_ERROR("failed %s NULL pointer mod %p line %p module_type %p",
      __func__, mod, line, module_type);
  }
  ...
  char** p_val, int count, vfe_test_table_t *table, char *mod_name)
{
  ...
  if (!mod ||!table || !mod_name) {
    CDBG_ERROR("failed: Null pointer mod %p table %p mod_name %p",
      mod, table, mod_name);
  }
  for (i = 0; i < count; i++) {
  ...
  char** p_val, int count)
{
  ...
  CDBG("%s:bet_dbg  E, module type = %d\n", __func__, mod->module_type);
  ...
  while (token != NULL) {
    ...
    if((len <=2) && (token[0] == '/') && (token[1] == '/')) {
      ...
      if (status != VFE_SUCCESS) {
        CDBG_ERROR("%s:%d parsed failed!, status = %d\n",
...
#define FETCH_TABLE_ROLLOFF(tbl, bank, bank_sel_reg, bank_sel_bit, type, sz) \
  ...
  FETCH_TABLE(tv_params->pedestal_t1_gr_r,
    PEDESTAL_F0_RAM_L_GR_R_BANK0, ISP_PEDESTAL_OFF, 8, VFE_READ_DMI_32BIT, sizeof(uint32_t));
  FETCH_TABLE(tv_params->pedestal_t1_gb_b,
    PEDESTAL_F0_RAM_L_GB_B_BANK0, ISP_PEDESTAL_OFF, 8, VFE_READ_DMI_32BIT, sizeof(uint32_t));
  FETCH_TABLE(tv_params->pedestal_t2_gr_r,
    PEDESTAL_F1_RAM_L_GR_R_BANK0, ISP_PEDESTAL_OFF, 8, VFE_READ_DMI_32BIT, sizeof(uint32_t));
  FETCH_TABLE(tv_params->pedestal_t2_gb_b,
    PEDESTAL_F1_RAM_L_GB_B_BANK0, ISP_PEDESTAL_OFF, 8, VFE_READ_DMI_32BIT, sizeof(uint32_t));
  ...
  testcase_report_t* testcase_report, char *mod_name[])
{
  ...
  CDBG_ERROR("%s:bet_dbg E\n", __func__);
  status = vfe_test_vector_get_output(mod);
  if (VFE_SUCCESS != status) {
    CDBG_ERROR("%s bet_dbg failed to get output: rc = %d\n", __func__, status);
    return VFE_ERROR_GENERAL;
  }

  if (!mod || !testcase_report || !mod_name) {
...
#define TEST(module_id, test_func) \
  ...
  show_mismatch = SHOW_GAMMA_MISMATCH;
  TEST(ISP_MOD_GAMMA, vfe_gamma_tv_validate);

  show_mismatch = SHOW_CC_MISMATCH;
  TEST(ISP_MOD_COLOR_CORRECT, vfe_color_correct_tv_validate);

  show_mismatch = SHOW_CV_MISMATCH;
  TEST(ISP_MOD_CHROMA_ENHANCE, vfe_chroma_enhance_tv_validate);

  show_mismatch = SHOW_ABF_MISMATCH;
  TEST(ISP_MOD_ABF, vfe_abf_tv_validate);

  show_mismatch = SHOW_GIC_MISMATCH;
  TEST(ISP_MOD_GIC, vfe_gic_tv_validate);

  show_mismatch = SHOW_GTM_MISMATCH;
  TEST(ISP_MOD_GTM, vfe_gtm_tv_validate);

  show_mismatch = SHOW_PEDES_MISMATCH;
  TEST(ISP_MOD_PEDESTAL, vfe_pedestal_tv_validate);

  show_mismatch = SHOW_CLF_MISMATCH;
  TEST(ISP_MOD_CLF, vfe_clf_tv_validate);

  show_mismatch = SHOW_CS_MISMATCH;
  TEST(ISP_MOD_CHROMA_SUPPRESS, vfe_chroma_suppression_tv_validate);

  show_mismatch = SHOW_ROLLOFF_MISMATCH;
  TEST(ISP_MOD_ROLLOFF, vfe_mesh_rolloff_tv_validate);

  show_mismatch = SHOW_DEMOSAIC_MISMATCH;
  TEST(ISP_MOD_DEMOSAIC, vfe_demosaic_tv_validate);

  show_mismatch = SHOW_BCC_MISMATCH;
  TEST(ISP_MOD_BCC, vfe_bcc_tv_validate);

  show_mismatch = SHOW_BPC_MISMATCH;
  TEST(ISP_MOD_BPC, vfe_bpc_tv_validate);

  show_mismatch = SHOW_LA_MISMATCH;
  TEST(ISP_MOD_LA, vfe_la_tv_validate);

  show_mismatch = SHOW_MCE_MISMATCH;
  TEST(ISP_MOD_MCE, vfe_mce_tv_validate);
  //TEST(ISP_MOD_SCE, vfe_sce_tv_validate);

  show_mismatch = SHOW_LINEAR_MISMATCH;
