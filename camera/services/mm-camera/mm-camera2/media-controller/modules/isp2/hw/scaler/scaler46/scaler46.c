...
boolean scaler46_parm_zoom(mct_module_t *module,
  scaler46_t *scaler, int32_t *zoom_val, uint32_t identity, mct_event_type type)
{
  boolean ret = TRUE;
  boolean is_stream_on = FALSE;
  int temp_fovc = 0;
  ...
  ret = isp_zoom_get_crop_factor(&scaler->zoom, scaler->zoom_value,
    &scaler->crop_factor);
  if (ret == FALSE) {
    ISP_ERR("failed: isp_zoom_get_crop_factor");
  }

  scaler->crop_factor += (uint32_t)((scaler->fovc_zoom_val * (scaler->crop_factor/4096.0)) * Q12);
