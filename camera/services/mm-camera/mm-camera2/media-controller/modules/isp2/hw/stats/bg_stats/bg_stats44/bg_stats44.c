  ...
  void *data)
{
  ...
  sub_module_output->stats_params->bg_stats_meta_enable = bg_stats->meta_enable;
  if (isp_sub_module->trigger_update_pending == FALSE) {
    ISP_DBG("BG trigger update pending false");
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  cam_exposure_data_t      *cam_exp_data = NULL;
  bg_stats44_t             *bg_stats = NULL;
  ...
  cam_exp_data = (cam_exposure_data_t *)param->parm_data;
  if (cam_exp_data)
    bg_stats->meta_enable = cam_exp_data->enable;

  ISP_DBG("Send BG stats in Meta? %d", bg_stats->meta_enable);
