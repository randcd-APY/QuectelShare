...
typedef struct {
  ...
  /* Tuning param for Homography validation : feature_points
  Minimum number feature points required to be detected in homography
  to apply HDR */
  uint8_t hdr_feature_points_count;
  /* Tuning param for Homography validation : inliers
  Minimum number Inliers required to be detected in homography
  to apply HDR */
  uint8_t hdr_inliers_count;
}hdr_chromatix_t;
