...
/** g_comp_name:
 *
 *  Face detection component names
 **/
char* g_comp_name[MAX_FD_COMP] = {
  "qcom.faceproc",
  "qcom.faceproc_hw",
  "qcom.faceproc_dsp"
};
...
int faceproc_test_init(faceproc_test_t *p_test)
{
  ...
  p_test->config.frame_cfg.max_width = p_test->base->width;
  p_test->config.frame_cfg.max_height = p_test->base->height;
  p_test->config.frame_cfg.min_stride = p_test->base->stride;
  p_test->config.frame_cfg.min_scanline = p_test->base->scanline;
  ...
  rc = img_core_get_comp(IMG_COMP_FACE_PROC,
    g_comp_name[p_test->base->sub_role], p_core_ops);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] rc %d", __func__, __LINE__, rc);
    return rc;
  }
