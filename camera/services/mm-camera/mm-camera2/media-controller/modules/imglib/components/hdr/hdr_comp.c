...
/** QHDR_MIN_GAMMA_INFO_COUNT
 *
 *   minimum number of frame with gamma info
 **/
#define QHDR_MIN_GAMMA_INFO_COUNT (1)

/** QHDR_MIN_AEC_INFO_COUNT
 *
 *   minimum number of frame with aec info
 **/
#define QHDR_MIN_AEC_INFO_COUNT (3)

typedef struct
{
  void *ptr;
  int (*hdrCreateContext)(void **pp_ctx);
  int (*hdrDestroyContext)(void* p_ctx);
  int (*hdrAbortProcessing)(void* p_ctx);
  int (*hdrTwoFrameCore)(void *p_ctx, hdr_config_t *pIn,
    hdr_return_t *pReturnCode);
  int (*hdrSingleFrameCore)(void *p_ctx, hdr_config_t *pIn,
    hdr_return_t *pReturnCode);
} hdr_lib_info_t;
...
int hdr_calculate_gammatbl(hdr_comp_t *p_comp)
{
  ...
  if (p_comp->gamma[0].gamma_t == GAMMA_TBL_ALL) {
    memcpy(p_comp->g_hdr_gamma.red_gamma_table,
      p_comp->gamma[0].gamma_tbl,
    ...
    //For R
    rc = hdr_calc_inverse_gamma(&(p_comp->gamma[0]),
    ...
    hdr_calc_new_gamma(&(p_comp->gamma[0]), p_comp->param.mpRedNewGammatable);
    //For G
    ...
    if (p_comp->gamma[0].gamma_t == GAMMA_TBL_R) {
      //For R
      if (!p_comp->param.mpRedInverseGammatable)
        p_comp->param.mpRedInverseGammatable = (uint32_t *)malloc(
          256 * sizeof(uint32_t));
      ...
      hdr_calc_new_gamma(&(p_comp->gamma[0]), p_comp->param.mpRedNewGammatable);
    } else if (p_comp->gamma[0].gamma_t == GAMMA_TBL_G) {
      //For G
      if (!p_comp->param.mpGreenInverseGammatable)
        p_comp->param.mpGreenInverseGammatable = (uint32_t *)malloc(
          256 * sizeof(uint32_t));
      ...
      hdr_calc_new_gamma(&(p_comp->gamma[0]), p_comp->param.mpGreenNewGammatable);
    } else if (p_comp->gamma[0].gamma_t == GAMMA_TBL_B) {
      //For B
      if (!p_comp->param.mpBlueInverseGammatable)
        p_comp->param.mpBlueInverseGammatable = (uint32_t *)malloc(
          256 * sizeof(uint32_t));
      ...
      hdr_calc_new_gamma(&(p_comp->gamma[0]), p_comp->param.mpBlueNewGammatable);
  ...
  uint32_t * new_gamma_tbl)
{
  ...
  for (i = 0; i < 64; i++) {
    gammavalue = (*pGammaTableValues) & 0xff;
    gammadelta = (*pGammaTableValues++) >> 8;
    IDBG_INFO("gamma value %d %d", gammavalue,gammadelta);

    for (j = 0; j < 64; j++) {
      index = i * 64 + j;
      new_gamma_tbl[index] = ((uint32_t)gammavalue << 2) + (((uint32_t)gammadelta * j + 8) >> 4);
  ...
  for (i = 0; i < 64; i++) {
    ...
    for (j = 0; j < 32; j++) {
      index = i * 32 + j;
      new_gamma_tbl[index] = ((uint32_t)gammavalue << 2) + (((uint32_t)gammadelta * j + 4) >> 3);
      new_gamma_tbl[index] = MIN(new_gamma_tbl[index], 1023);
    }
  }
  ...
  for (i = 0; i < 4096; i++) {
    ...
    if (new_gamma_tbl[i] > 1023) {
      IDBG_WARN("new_gamma_tbl[%d] %d is more than 1023",
        i, new_gamma_tbl[i]);
    }
  }
}
...
int hdr_comp_deinit(void *handle)
{
  ...
  g_hdr_lib.hdrDestroyContext(p_comp->p_hdr_ctx);
...
{
  p_config->imageWidth = (uint32_t)p_frame[0]->frame->plane[IY].stride;
  p_config->imageHeight = (uint32_t)p_frame[0]->frame->plane[IY].height;
  ...
  if (p_comp->hdr_chromatix.enable) {
    p_config->tuning_params.contrastControl =
      p_comp->hdr_chromatix.hdr_contrast_control;
    p_config->tuning_params.chromaSat_wgt =
      (float)((p_comp->hdr_chromatix.hdr_chromaSat_wgt))/10.0f;
    p_config->tuning_params.chromaSat_clamp =
      p_comp->hdr_chromatix.hdr_chromaSat_clamp;
    p_config->tuning_params.chromaSat_shift =
      p_comp->hdr_chromatix.hdr_chromaSat_shift;
    p_config->tuning_params.feature_points_threshold =
      p_comp->hdr_chromatix.hdr_feature_points_count;
    p_config->tuning_params.inliers_count_threshold =
      p_comp->hdr_chromatix.hdr_inliers_count;
  }  else {
      /* By Default, Color saturation control is set to mild level of desaturation */
      p_config->tuning_params.chromaSat_wgt   = 0.5f;
      p_config->tuning_params.chromaSat_clamp = 90;
      p_config->tuning_params.chromaSat_shift = 5;
      p_config->tuning_params.feature_points_threshold =
        FEATURE_POINTS_THRESHOLD;
      p_config->tuning_params.inliers_count_threshold =
        INLIER_COUNT_THRESHOLD;
  }
...
int hdr_comp_execute(hdr_comp_t *p_comp)
{
  ...
  p_config->p_img_perf = &img_perf;

  if (p_comp->mode == SINGLE_FRAME) {
    if (!g_hdr_lib.hdrSingleFrameCore) {
      IDBG_ERROR("%s:%d] Error HDR single frame core not supported", __func__,
          __LINE__);
    ...
    if (p_comp->analyse_image) {
      ...
      g_hdr_lib.hdrSingleFrameCore(p_comp->p_hdr_ctx, p_config, &rc);
    ...
    if (p_comp->analyse_image) {
      hdr_fill_config(p_comp, p_config, p_comp->p_analysis_frame);
      g_hdr_lib.hdrTwoFrameCore(p_comp->p_hdr_ctx, p_config, &rc);
    ...
    // HDR algo does not have separate width and stride.
    // It always works on image stride. But rest of the
    // framework expects crop to be inside width.
    // So horizontal end of the crop needs to be clipped
    // to image width
    if (p_comp->out_crop.start_x + p_comp->out_crop.width >
        p_comp->p_main_frame[0]->frame->plane[IY].width) {
      p_comp->out_crop.width = p_comp->p_main_frame[0]->frame->plane[IY].width -
        p_comp->out_crop.start_x;
    }

    if (p_comp->out_crop.width + p_comp->out_crop.start_x >
      p_comp->p_main_frame[0]->info.width) {
        p_comp->out_crop.width =
          (uint32_t)p_comp->p_main_frame[0]->info.width - 2 * p_comp->out_crop.start_x;
...
int hdr_comp_set_param(void *handle, img_param_type param, void *p_data)
{
  ...
  case QHDR_GAMMA_TABLE: {
    ...
    if (MAX_HDR_FRAMES <= p_comp->gamma_info_count) {
      IDBG_ERROR("%s:%d] too many aec_info", __func__, __LINE__);
      return IMG_ERR_INVALID_INPUT;
    }
    p_comp->gamma[p_comp->gamma_info_count].gamma_tbl = p_gamma->table;
    p_comp->gamma[p_comp->gamma_info_count].entry = GAMMA_TABLE_ENTRIES;
    p_comp->gamma[p_comp->gamma_info_count].gamma_t = GAMMA_TBL_ALL;
    p_comp->gamma_info_count++;
  }
    break;
  ...
  case QHDR_HDR_CHROMATIX: {
    ...
    p_comp->gamma_info_count = 0;
    p_comp->aec_info_count = 0;
  }
    break;
  case QHDR_HDR_AEC_INFO: {
    img_aec_info_t *p_aec_info = (img_aec_info_t *)p_data;

    if (NULL == p_aec_info) {
      IDBG_ERROR("%s:%d] invalid hdr aec_info", __func__, __LINE__);
      return IMG_ERR_INVALID_INPUT;
    }
    if (MAX_HDR_FRAMES <= p_comp->aec_info_count) {
      IDBG_ERROR("%s:%d] too many aec_info", __func__, __LINE__);
      return IMG_ERR_INVALID_INPUT;
    }
    p_comp->aec_info[p_comp->aec_info_count] = *p_aec_info;
    p_comp->aec_info_count++;
  }
    break;
  default:
...
int hdr_comp_start(void *handle, void *p_data)
{
  ...
  if ((QHDR_MIN_GAMMA_INFO_COUNT > p_comp->gamma_info_count) ||
      (QHDR_MIN_AEC_INFO_COUNT > p_comp->aec_info_count)) {
    IDBG_ERROR("%s:%d] wrong medata data count gamma %d aec %d",
      __func__, __LINE__, p_comp->gamma_info_count, p_comp->aec_info_count);
    return IMG_ERR_INVALID_INPUT;
  }
...
int hdr_comp_abort(void *handle, void *p_data)
{
  ...
  if (g_hdr_lib.hdrAbortProcessing != NULL) {
    g_hdr_lib.hdrAbortProcessing(p_comp->p_hdr_ctx);
  }
...
int hdr_comp_create(void* handle, img_component_ops_t *p_ops)
{
  ...
  hdr_result = g_hdr_lib.hdrCreateContext(&p_comp->p_hdr_ctx);
  if ((hdr_result != HDR_SUCESS) ||
    (p_comp->p_hdr_ctx == NULL)) {
    IDBG_ERROR("HDR context creation failed");
    free(p_comp);
    return IMG_ERR_GENERAL;
  }
...
/**
 * Function: hdr_comp_unload_lib
 *
 * Description: Callback to be called from dsp dl mgr to unload llvd.so
 *
 * Input parameters:
 *   @handle: library handle
 ...
 * Notes: none
 **/
static int hdr_comp_unload_lib(void *handle, void *p_userdata)
{
  IMG_UNUSED(handle);
  ...
  g_hdr_lib.hdrCreateContext = NULL;
  g_hdr_lib.hdrDestroyContext = NULL;
  g_hdr_lib.hdrAbortProcessing = NULL;
  g_hdr_lib.hdrSingleFrameCore = NULL;
  g_hdr_lib.hdrTwoFrameCore = NULL;
...
/**
 * Function: hdr_comp_reload_lib
 *
 * Description: Callback to be called from dsp dl mgr to reload llvd.so
 *
 * Input parameters:
 *   @handle: library handle
 *   @name: library name
 ...
 * Notes: none
 **/
static int hdr_comp_reload_lib(void *handle, const char *name, void *p_userdata)
{
  IMG_UNUSED(name);
  IMG_UNUSED(handle);
  IMG_UNUSED(p_userdata);
  ...
  *(void **)&(g_hdr_lib.hdrCreateContext) = dlsym(g_hdr_lib.ptr,
    "hdrCreateContext");
  if (!g_hdr_lib.hdrCreateContext) {
    IDBG_ERROR("%s:%d] Error linking hdrCreateContext", __func__, __LINE__);
    goto error;
  }

  *(void **)&(g_hdr_lib.hdrDestroyContext) = dlsym(g_hdr_lib.ptr,
    "hdrDestroyContext");
  if (!g_hdr_lib.hdrDestroyContext) {
    IDBG_ERROR("%s:%d] Error linking hdrDestroyContext", __func__, __LINE__);
    goto error;
  }

  *(void **)&(g_hdr_lib.hdrAbortProcessing) = dlsym(g_hdr_lib.ptr,
    "hdrAbortProcessing");
  if (!g_hdr_lib.hdrAbortProcessing) {
    IDBG_HIGH("%s:%d] Error linking hdrAbortProcessing", __func__, __LINE__);
  }

  *(void **)&(g_hdr_lib.hdrSingleFrameCore) = dlsym(g_hdr_lib.ptr,
    "hdrSingleFrameCore");
  if (!g_hdr_lib.hdrSingleFrameCore) {
    IDBG_HIGH("%s:%d] Error linking hdrSingleFrameCore", __func__, __LINE__);
  }
...
int hdr_comp_load(const char* name, void* handle)
{
  ...
  if(IMG_SUCCESS != hdr_comp_reload_lib(NULL, NULL, NULL)){
    goto error;
  }

  img_dsp_dlopen(HDR_COMP_LIB_NAME, (void*)&g_hdr_lib, hdr_comp_unload_lib,
    hdr_comp_reload_lib);
  IDBG_HIGH("%s:%d] HDR library loaded successfully", __func__, __LINE__);
  return IMG_SUCCESS;

  error:
    IDBG_HIGH("%s:%d] HDR library load error", __func__, __LINE__);
    return IMG_ERR_NOT_FOUND;
}
...
void hdr_comp_unload(void* handle)
{
  ...
  IDBG_HIGH("%s:%d] ptr %p", __func__, __LINE__, g_hdr_lib.ptr);
  hdr_comp_unload_lib(NULL, NULL);
  img_dsp_dlclose(&g_hdr_lib);
}
