 ...
 *
 * Defines the macro for threshold feature points
 * need to be detected to apply HDR
 *
 **/
#define FEATURE_POINTS_THRESHOLD 10

/** FEATURE_POINTS_THRESHOLD:
 *
 * Defines the macro for threshold inliers
 * need to be detected to apply HDR
 *
 **/
#define INLIER_COUNT_THRESHOLD 15
 ...
 *  @feature_points_threshold - Minimum number of feature points required
       to be detected in Homography. Tunable.
 *  @inliers_count_threshold - Minimum number of inliers required
       to be detected in Homography. Tunable.
 *
 *  hdr_tuning_params_t
 **/
typedef struct {
  ...
  uint8_t feature_points_threshold;
  uint8_t inliers_count_threshold;
} hdr_tuning_params_t;
...
typedef struct {
  ...
  hdr_tuning_params_t tuning_params;
  int32_t LUT_256[256];
...
/** hdrCreateContext:
 *    @pp_ctx: Pointer to get hdr context(p_ctx).
 * Create hdr context that needs to be passed later.
 *
 * Returns hdr_return_t
 **/
hdr_return_t hdrCreateContext(void **pp_ctx);

/** hdrDestroyContext:
 *    @p_ctx: Pointer to hdr context(p_ctx).
 * Destroy HDR context.
 *
 * Returns hdr_return_t
 **/
hdr_return_t hdrDestroyContext(void* p_ctx);

/** hdrAbortProcessing:
 *    @p_ctx: Pointer to hdr context(p_ctx).
 * Abort HDR processing.
 *
 * Returns hdr_return_t
 **/
hdr_return_t hdrAbortProcessing(void* p_ctx);
...
void hdrTwoFrameCore (void *p_ctx, hdr_config_t *pIn,
  hdr_return_t *pReturnStatus);
