...
typedef struct {
  void *ptr;
  int (*cac_module_process)(cac_arg_t  *p_lib);
  int (*cac_module_init)();
...
static void cac_comp_lib_debug(cac_arg_t *p_caclib)
{
  ...
  IDBG_MED("%s:%d] CACLIB dynamic Th Enable %d", __func__, __LINE__,
    p_caclib->dynamicThEnable);
}

/**
 * Function: cac_comp_get_config
 *
 * Description: Get the CAC configuration
 *
 * Input parameters:
 ...
 * Notes: none
 **/
int cac_comp_get_config(cac_comp_t *p_comp)
{
  int rc = IMG_SUCCESS;

  //Fill in the CAC data
  p_comp->chroma_order = CAC_CHROMA_ORDER_CRCB;

  //Need to update with the params from chromatix header after chromatix 303 is integrated
  p_comp->chromatix_info.edgeTH = 20;
  p_comp->chromatix_info.saturatedTH = 120;
  p_comp->chromatix_info.chrom0LowTH = 8;
  p_comp->chromatix_info.chrom0HighTH = 448;
  p_comp->chromatix_info.chrom1LowTH = 8;
  p_comp->chromatix_info.chrom1HighTH = 448;
  p_comp->chromatix_info.chrom0LowDiffTH = 192;
  p_comp->chromatix_info.chorm0HighDiffTH = 320;
  p_comp->chromatix_info.chrom1LowDiffTH = 192;
  p_comp->chromatix_info.chorm1HighDiffTH = 320;
...
int cac_comp_process_frame(cac_comp_t *p_comp, img_frame_t *p_frame)
{
  ...
  if (NULL == p_comp->p_y_buffer) {
    ...
    IDBG_INFO("%s:%d] Allocating temp buffers", __func__, __LINE__);
  ...
  cac_params.awbGR = (uint32_t)p_comp->info_3a.awb_gr_gain;
  cac_params.awbGB = (uint32_t)p_comp->info_3a.awb_gb_gain;
  ...
  cac_params.dynamicThEnable = TRUE;
  ...
  rc = g_cac_lib.cac_module_process(&cac_params);
...
int cac_comp_get_param(void *handle, img_param_type param, void *p_data)
{
  ...
  switch (param) {
  default:
    IDBG_ERROR("%s:%d] Not Supported", __func__, __LINE__);
  }
  return IMG_SUCCESS;
}
...
int cac_comp_load(const char* name, void** handle)
{
  ...
  *(void **)&(g_cac_lib.cac_module_process) =
    dlsym(g_cac_lib.ptr, "cac_module_process");
  if (!g_cac_lib.cac_module_process) {
    IDBG_ERROR("%s:%d] Error linking camera CAC module process",
      __func__, __LINE__);
