...
void module_wnr_port_release_client(module_wnr_t *p_mod,
  wnr_client_t *p_client, unsigned int identity)
{
  mct_list_t *p_temp_list = NULL;
  ...
  unsigned int identity)
{
  ...
  if (MCT_PORT_IS_SRC(port)) {
    module_wnr_port_release_client(p_mod, p_client, identity);
 ...
 * Notes: none
 **/
boolean module_wnr_port_check_caps_reserve(mct_port_t *port, void *caps,
  void *vstream_info)
{
  boolean rc = FALSE;
    ...
    case MCT_EVENT_CONTROL_CMD: {
      ...
      switch (p_ctrl_event->type) {
        case MCT_EVENT_CONTROL_STREAMON: {
          ...
          mct_port_t* port = module_wnr_find_port_with_identity(MCT_PORT_SINK,
          ...
          memset(&p_client->stats_get, 0, sizeof(stats_get_data_t));
      ...
      switch (p_mod_event->type) {
        case MCT_EVENT_MODULE_BUF_DIVERT: {
          ...
          if (p_client->cam_denoise_param.denoise_enable) {
            ...
            IDBG_HIGH("%s:%d] identity %x", __func__, __LINE__,
              event->identity);
 ...
 * Notes: none
 **/
static boolean __unused module_wnr_find_identity(void *p_data, void *p_input)
{
...
/**
 * Function: module_wnr_find_port_with_identity
 *
 * Description: This method is used to find the port with specified identity
 *
 * Arguments:
 *   @dir: port direction
 *   @identity: identity
 ...
 *
 * Return values:
 *     port with specified identity or NULL
 *
 * Notes: none
 **/
mct_port_t* module_wnr_find_port_with_identity(
  ...
  unsigned int sessionid)
{
  ...
  if (p_mct_cap->sensor_cap.sensor_format != FORMAT_YCBCR)
    p_cap->feature_mask |= (cam_feature_mask_t)CAM_QCOM_FEATURE_DENOISE2D;
