...
/**
 * @pd_stats_read_data_length: Read data length for PD data
 * @pd_buffer_data: pd buffer address
 * @buf_idx: buffer index
 * @max_buffers: maximum buffers allocated
 **/
typedef struct _bf_pd_stats_data_t {
  uint32_t pd_stats_read_data_length;   /* Number of line WRT CAMIF */
  void*    pd_buffer_data;              /* PD Buffer Address */
  uint32_t buf_idx;                     /* Buf Index */
  uint32_t max_buffers;                 /* Max Buffers */
} mct_bf_pd_stats_data_t;
