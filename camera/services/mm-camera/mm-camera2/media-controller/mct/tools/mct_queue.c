...
/*  mct_queue_modify_node_by_idx
 *
 *  Description:
 *  Returns the data from index provided.
 *  Does not add/remove anything in queue.
 **/
void *mct_queue_modify_node_by_idx(mct_queue_t *q, uint32_t node_idx)
{
  if (!q)
    return NULL;

  return mct_list_return_node_by_idx(q->head, node_idx);
}
