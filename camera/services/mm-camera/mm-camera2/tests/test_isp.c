...
static int config_isp(mct_port_t *iface_sink_port)
{
  boolean rc = FALSE;
...
static int start_stream(mct_port_t *iface_sink_port)
{
  boolean rc = FALSE;
  mct_event_t *event;
...
int test_isp_init_modules()
{
  ...
  rc = config_isp(iface_sink_port);
  CDBG("%s TEST_ISP: config_isp, rc = %d", __func__, rc);
  rc = start_stream(iface_sink_port);
