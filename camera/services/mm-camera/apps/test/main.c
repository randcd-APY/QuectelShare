...
int reg_vid_vpe_buf(void)
{
  ...
  for (cnt = 0; cnt < VIDEO_FRAMES_NUM; cnt++) {
    ...
    video_frames[cnt].ion_alloc.heap_id_mask =
...
int zsl_init()
{
  ...
  for (cnt = 0; cnt < PREVIEW_FRAMES_NUM; cnt++) {
    ...
    frames[cnt].ion_alloc.heap_id_mask =
      (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
...
int start_zsl_mode(void)
{
  ...
  if (params.useExternalBuffers) {
    ...
    for (i=0; i<MAX_SNAPSHOT_BUFFERS; i++) {
        ...
        zsl_thumb_frame[i].ion_alloc.heap_id_mask =
          (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
    ...
    for (i=0; i<MAX_SNAPSHOT_BUFFERS; i++) {
        ...
        zsl_main_frame[i].ion_alloc.heap_id_mask =
          (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
...
int take_picture (char menu_item) {
  ...
  thumbnail_ion_alloc.heap_id_mask = (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
  ...
  main_ion_alloc.heap_id_mask = (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
...
int take_raw_picture (void) {
  ...
  raw_ion_alloc.heap_id_mask = (0x1 << CAMERA_ION_HEAP_ID | 0x1 << ION_IOMMU_HEAP_ID);
