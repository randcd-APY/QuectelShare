...
/* data structure with striping algorithm parameters */
   struct cpp_striping_algorithm_info {
     ...
     /* 927
      * Padding is required for upscaler because it does not 928
      * pad internally like other blocks, also needed for rotation 929
      * rotation expects all the blocks in the stripe to be the same size 930
      * Padding is done such that all the extra padded pixels 931
      * are on the right and bottom 932
      */
     uint32_t pad_bottom;
     uint32_t pad_top;
     uint32_t pad_right;
     ...
     uint32_t destination_address[2];
     /* 986
      * source_address[1] is used for CbCR planar 987
      * to CbCr interleaved conversion 988
      */
     uint32_t temporal_source_address[2];
     /* 991
      * destination_address[1] is used for CbCr interleved 992
      * to CbCr planar conversion 993
      */
