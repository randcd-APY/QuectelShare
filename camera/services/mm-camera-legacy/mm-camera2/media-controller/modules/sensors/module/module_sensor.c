  ...
  module_sensor_ctrl_t *module_ctrl)
{
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (mct_util_match_subdev_type(entity, MSM_CAMERA_SUBDEV_SENSOR,
  ...
  module_sensor_ctrl_t *module_ctrl)
{
  ...
  while (1) {
    int32_t num_entities = 1, subdev_type = 0;
    snprintf(dev_name, sizeof(dev_name), "/dev/media%d", num_media_devices);
    dev_fd = open(dev_name, O_RDWR | O_NONBLOCK);
    if (dev_fd >= MAX_FD_PER_PROCESS) {
      dump_list_of_daemon_fd();
    ...
    while (1) {
      ...
      SERR("entity name %s type %x group id %d",
        entity.name, entity.type, entity.group_id);
#ifndef USE_4_9_DEFS__KYLE
