...
int main(int argc, char *argv[])
{
  ...
  CDBG_ERROR("CAMERA_DAEMON: open %s\n", dev_name);
  ...
  if (probe_done_fd < 0) {
    CDBG_ERROR("%s: failed open error = %s\n", __func__,strerror(errno));
    goto subscribe_failed;
  }

  cfg.cfgtype = CFG_SINIT_PROBE_DONE;
  if (ioctl(probe_done_fd, VIDIOC_MSM_SENSOR_INIT_CFG, &cfg) < 0) {
    CDBG_ERROR("%s: failed error = %s\n", __func__,strerror(errno));
    ret = FALSE;
  }
  close(probe_done_fd);


  CDBG_ERROR("CAMERA_DAEMON:waiting for camera to open\n");
  do {
