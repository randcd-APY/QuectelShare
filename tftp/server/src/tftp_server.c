...
----------   ---   ---------------------------------------------------------
2018-06-20   aj    Address KW errors.
...
tftp_server_get_client_instance (void)
{
  ...
  client_info->proto.tx_pkt = tftp_malloc (sizeof(struct tftp_pkt_type));
  if (client_info->proto.tx_pkt == NULL)
  {
  ...
  client_info->proto.rx_pkt = tftp_malloc (sizeof(struct tftp_pkt_type));
  if (client_info->proto.rx_pkt == NULL)
  {
    tftp_free (client_info->proto.tx_pkt);
    client_info->proto.tx_pkt = NULL;
  ...
  TFTP_ASSERT (client_info->proto.tx_pkt != NULL);
  TFTP_ASSERT (client_info->proto.rx_pkt != NULL);

  memset (client_info->proto.tx_pkt, 0, sizeof(struct tftp_pkt_type));
  memset (client_info->proto.rx_pkt, 0, sizeof(struct tftp_pkt_type));
    ...
    struct tftp_server_client_info_type *client_info)
{
  ...
  TFTP_ASSERT (client_info->proto.rx_pkt != NULL);
  tftp_free (client_info->proto.rx_pkt);
  client_info->proto.rx_pkt = NULL;
...
tftp_server (int argc, const char *argv[])
{
  ...
  if (init_res != 0)
  {
    TFTP_LOG_DBG ("Server folder init failed! error = %d.\n", init_res);
    ...
    else
    {
      if (open_res != -ENOENT)
      {
        TFTP_LOG_INFO ("open failed2: [%d] [%s] [%s]",
                       open_res, strerror(open_res), path);
        return open_res;
      }
