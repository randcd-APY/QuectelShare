...
int print_vpp_svc_buffer_attribute_t(vpp_svc_frame_group_descriptor_t *frame_group_descp)
{
    ...
    LOGD("frame_group_descp->compressiontype =%d\n", frame_group_descp->compressiontype);
...
int print_vpp_svc_context(vpp_svc_context_t *vpp_svc_context)
{
    LOGD("vpp_svc_context->user_dataLen =%d\n", vpp_svc_context->user_dataLen);
    LOGD("vpp_svc_context->user_data =%p\n", (void*)vpp_svc_context->user_data);
    return 0;
}
