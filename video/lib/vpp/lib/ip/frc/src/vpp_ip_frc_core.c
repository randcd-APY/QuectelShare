 ...
 * Local definitions
 ************************************************************************/
// #define FRC_SUB_OPS_SERIAL

#ifndef FRC_SUB_OPS_SERIAL
                                 ...
                                 t_EVppIpFrcDumpBuf eBuf)
{
    ...
    if (pstCb->stBase.bSecureSession && (eBuf != VPP_IP_FRC_BUF_MC_MBI_IN))
        return;
...
static void vVppIpFrc_HandleMeObd(t_StVppIpFrcCb *pstCb, t_StVppBuf *pstBuf)
{
    ...
    else if (pstBuf->eBufPxType == eVppBufPxDataType_Compressed)
    {
        ...
        {
            if (pstBuf->stExtra.u32FilledLen)
            {
...
static uint32_t u32VppIpFrc_AllocMbiStructures(t_StVppIpFrcCb *pstCb)
{
    ...
    u32Cnt = pstCb->stMbiBufReq.u32MinCnt;
...
static uint32_t u32VppIpFrc_FreeMbiIonBuffers(t_StVppIpFrcCb *pstCb)
{
    ...
    for (i = 0; i < pstCb->stMbiBufReq.u32MinCnt; i++)
    {
...
static uint32_t u32VppIpFrc_AllocMbiIonBuffers(t_StVppIpFrcCb *pstCb)
{
    ...
    for (u32Px = 0; u32Px < pstCb->stMbiBufReq.u32MinCnt; u32Px++)
    {
        u32 = u32VppIon_Alloc(pstCb->stBase.pstCtx,
                              pstCb->stMbiBufReq.u32PxSz,
    ...
    for (u32Ex = 0; u32Ex < pstCb->stMbiBufReq.u32MinCnt; u32Ex++)
    {
        u32 = u32VppIon_Alloc(pstCb->stBase.pstCtx,
                              pstCb->stMbiBufReq.u32ExSz,
                              VPP_FALSE,
...
static uint32_t u32VppIpFrc_UnprepareMbiBufferPool(t_StVppIpFrcCb *pstCb)
{
    ...
    if (u32Cnt != pstCb->stMbiBufReq.u32MinCnt)
    {
        LOGE("unexpected amount of buffers from mbi pool, expected=%u, actual=%u",
             pstCb->stMbiBufReq.u32MinCnt, u32Cnt);
...
static uint32_t u32VppIpFrc_AllocateMbiBuffers(t_StVppIpFrcCb *pstCb)
{
    ...
    u32Min = VPP_MAX(pstCb->stMeOutputReq.u32MinCnt,
                     pstCb->stMcInputReq.u32MinCnt);

    pstCb->stMbiBufReq.u32MinCnt = u32Min;

    pstCb->stMbiBufReq.u32PxSz = VPP_MAX(pstCb->stMeOutputReq.u32PxSz,
                                         pstCb->stMcInputReq.u32PxSz);
    pstCb->stMbiBufReq.u32ExSz = VPP_MAX(pstCb->stMeOutputReq.u32ExSz,
                                         pstCb->stMcInputReq.u32ExSz);

    LOGI("allocating mbi, cnt=%u, u32PxSz=%u, u32ExSz=%u",
         pstCb->stMbiBufReq.u32MinCnt,
         pstCb->stMbiBufReq.u32PxSz,
         pstCb->stMbiBufReq.u32ExSz);
...
void *vpVppIpFrc_Init(t_StVppCtx *pstCtx, uint32_t u32Flags, t_StVppCallback cbs)
{
    ...
    stFrcCb.pv = (void *)&pstCb->astCbCtx[VPP_IP_FRC_ME];
    VPP_FRC_INIT(ME, pstCb, u32Flags, stFrcCb, pstCb->apvFrcCtx[VPP_IP_FRC_ME]);

    stFrcCb.pv = (void *)&pstCb->astCbCtx[VPP_IP_FRC_MC];
    VPP_FRC_INIT(MC, pstCb, u32Flags, stFrcCb, pstCb->apvFrcCtx[VPP_IP_FRC_MC]);
...
#ifdef FRC_SUB_OPS_SERIAL
    VPP_FRC_OPEN(ME, pstCb, u32RetMe);
    VPP_FRC_OPEN(MC, pstCb, u32RetMc);
#else
...
#ifdef FRC_SUB_OPS_SERIAL
    VPP_FRC_CLOSE(ME, pstCb, au32Ret[VPP_IP_FRC_ME]);
    VPP_FRC_CLOSE(MC, pstCb, au32Ret[VPP_IP_FRC_MC]);
#else
...
{
    VPP_UNUSED(ctx);
    VPP_UNUSED(pstInputBufReq);
    VPP_UNUSED(pstOutputBufReq);
    return VPP_OK;
}
                              ...
                              t_StVppBuf *pBuf)
{
    ...
    if (ePort == VPP_PORT_INPUT)
    {
        ...
        if (!u32VppBuf_IsEos(pBuf) && !pBuf->stPixel.u32FilledLen)
            goto ERR_INSUFFICIENT_SZ;
