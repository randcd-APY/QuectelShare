...
TEST(IpGpu_Testcase)
{
    ...
    cb.pv = &stTestCtx;
    pCtx = vpVppIpGpu_Init(NULL, 0, cb);
    DVP_ASSERT_PTR_NNULL(pCtx);

    port_param.height = 1920;
    port_param.width = 1080;
    port_param.fmt = VPP_COLOR_FORMAT_MAX;
...
TEST(IpGpuNoInit_FlushOnTerm)
{
    ...
    FlushOnTermSetup(&stCtxGpu.gpu_ctx, &tctx, VPP_FALSE);
