...
extern t_StVppUsecase uc_mock_uctest_hvx, uc_mock_uctest_fb_hvx, uc_mock_uctest_gpu,
    ...
    uc_mock_uctest_gpu_single_sess, uc_mock_hvx_auto, uc_mock_uctest_hvx_maxfps;
/************************************************************************
 * Local Functions
 ***********************************************************************/
...
void set_frc(t_StVppHqvCtrl *pstCtrl, uint32_t u32En)
{
    ...
    {
        pstCtrl->mode = HQV_MODE_MANUAL;
        pstCtrl->frc.mode = HQV_FRC_MODE_HIGH;
    }
    else
    {
...
TEST(Usecase_GetRequirements)
{
    ...
    // Usecase 1 1920x1080: Credits {5, 10, 15, NOT_SUPPORTED}, 1 metadata
    // Usecase 2 1920x1080: Credits {15,15,15,30}, No metadata, Buf Req (0,1,0,1)
    // Usecase 3 1920x1080: Credits {5,10,15,30}, Buf Req SD, HD, FHD (2,1,0,2), UHD (0,1,0,1)

    // UC1 should return metadata count = 1
    print_uc(pstUc1);
    u32Ret = u32VppUsecase_GetRequirements(pstUc1, vpp_req);
    DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
    DVP_ASSERT_EQUAL(vpp_req->metadata.cnt, 1);

    // UC2 should return metadata count = 0
    print_uc(pstUc2);
