...
TEST(Vpp_Timestamp)
{
    ...
    struct vpp_port_param param;
    t_StVppCb *pstCb;
    uint32_t u32FrameRate, u32DefaultRate, u32TimestampIncrement, u32Timestamp, i;
    ...
    // Max frame rate should go to bypass
    ...
    // Set back to realtime, should use timestamp and go to bypass
    vid_prop.property_type = VID_PROP_NON_REALTIME;
    ...
    // Set operating rate to usecase default.  Should exit bypass
    ...
    // Disable operating rate, should use timestamp and go to bypass
    vid_prop.property_type = VID_PROP_OPERATING_RATE;
