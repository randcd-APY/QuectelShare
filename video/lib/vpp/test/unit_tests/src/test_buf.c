...
TEST(Buf_TestGetPut)
{
    ...
    // Invalid Put cases
    u32Ret = u32VppBuf_InternalPut(NULL, NULL, NULL);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppBuf_InternalPut(NULL, NULL, &pvpp_buf);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppBuf_InternalPut(NULL, pstVppBuf, NULL);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppBuf_InternalPut(NULL, pstVppBuf, &pvpp_buf);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppBuf_InternalPut(&stCtx, NULL, NULL);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppBuf_InternalPut(&stCtx, NULL, &pvpp_buf);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    u32Ret = u32VppBuf_InternalPut(&stCtx, pstVppBuf, NULL);
    DVP_ASSERT_NEQUAL(u32Ret, VPP_OK);
    ...
    if (pstVppBuf)
    {
        ...
        u32Ret = u32VppBuf_InternalPut(&stCtx, pstVppBuf, &pvpp_buf);
        DVP_ASSERT_EQUAL(u32Ret, VPP_OK);
...
void Buf_TestGetPutIon_Internal(t_StVppUsecase *pstUc, uint32_t bIsBufMapped)
{
    ...
    for (i = 0; i < u32MaxCnt; i++)
    {
        ...
        u32Ret = u32VppBuf_InternalPut(&tctx.stVppCtx, pstIntBuf, &pVppBuf);
