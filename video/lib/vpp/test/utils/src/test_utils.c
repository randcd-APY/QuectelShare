...
/************************************************************************
 * End to end Functional Test Functions
 ***********************************************************************/
                          ...
                          uint32_t height, enum vpp_color_format fmt)
{
    ...
    pCtx->port_param.width = width;
    pCtx->port_param.height = height;
    pCtx->port_param.stride = u32VppUtils_CalculateStride(width, fmt);
    pCtx->port_param.scanlines = u32VppUtils_CalculateScanlines(height, fmt);
    pCtx->port_param.fmt = fmt;
