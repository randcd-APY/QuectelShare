...
static uint32_t u32VppIpMock_PortBufPeek(t_StVppIpPort *pstPort,
                                        t_StVppBuf **ppstBuf)
{
    int32_t idx;
...
static uint32_t u32VppIpMock_PortBufGet(t_StVppIpPort *pstPort,
                                       t_StVppBuf **ppstBuf)
{
    int32_t idx;
...
static uint32_t u32ProcBufReqMet(t_StVppIpMockCb *pstCb)
{
    ...
    if (u32InQSz)
    {
        ...
        {
            LOGI("awake because of empty eos buffer");
            return VPP_TRUE;
        }
...
static uint32_t u32VppIpMock_ProcessBuffer(t_StVppIpMockCb *pstCb)
{
    ...
    if (u32OutMin == 0 ||
        ...
        // empty eos buffer, needs to be bypassed
        (VPP_FLAG_IS_SET(pstBufPeek->pBuf->flags, VPP_BUFFER_FLAG_EOS) &&
