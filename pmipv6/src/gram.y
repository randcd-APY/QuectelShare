...
%token		SERVICESELECTIONIDENTIFIER
%token		MNIDSTRING
...
%token <addrv4>	LMAV4ADDRESS
%token <addrv4>	PRIMARYV4DNS
%token <addrv4>	SECONDARYV4DNS
%token		PRIMARYV6DNS
%token		SECONDARYV6DNS
%token <addrv4>	MAGV4ADDRESSINGRESS
%token <addrv4>	MAGV4ADDRESSEGRESS
%token <addrv4>	 PMIPV4SECROUTERDMNPPREFIX
%token 	 PMIPV4SECROUTERDMNPPREFIXLEN
%token 	 PMIPMAXLOGFILESIZE
%token		INV_TOKEN
...
proxymipmagopt	: LMAADDRESS ADDR ';'
		...
		| SECONDARYV4DNS ADDRV4 ';'
		{
			conf.SecondaryV4Dns = $2;
		}
		| PRIMARYV6DNS ADDR ';'
		{
			conf.PrimaryV6Dns = $2;
		...
		| SECONDARYV6DNS ADDR ';'
		{
			conf.SecondaryV6Dns = $2;
		}
		| MAGV4ADDRESSINGRESS ADDRV4 ';'
		{
			conf.Magv4AddressIngress = $2;
		}
		| MAGV4ADDRESSEGRESS ADDRV4 ';'
		{
			conf.Magv4AddressEgress = $2;
		}
		| PMIPV4SECROUTERDMNPPREFIX ADDRV4 ';'
		{
			conf.PMIPV4SecRouterDMNPPrefix = $2;
		}
		| PMIPV4SECROUTERDMNPPREFIXLEN NUMBER ';'
		{
			conf.PMIPV4SecRouterdmnpPrefixLen = $2;
		}
		| PMIPMAXLOGFILESIZE NUMBER ';'
		{
			conf.PmipMaxLogFileSize = $2;
		}
		| RFC5213TIMESTAMPBASEDAPPROACHINUSE BOOL ';'
		{
			conf.RFC5213TimestampBasedApproachInUse = $2;
		...
		| SERVICESELECTIONIDENTIFIER QSTRING ';'
		{
			conf.ServiceSelectionIdentifier = $2;
		}
		| MNIDSTRING QSTRING ';'
		{
			conf.MnIdString = $2;
