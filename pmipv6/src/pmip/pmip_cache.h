...
typedef struct pmip_entry_t {
	...
	struct in_addr			mnv4_addr;			/*!< \brief Full MN v4 Address */
	uint8_t					Is_arp_sent;		/*!< \brief is arp is sent for  MN v4 Address */
	struct in6_addr			mn_serv_mag_addr;	/*!< \brief Serving MAG Address */
	struct in_addr			mn_serv_mag_v4_addr;	/*!< \brief Serving MAG Address */
	struct in6_addr			mn_serv_lma_addr;	/*!< \brief Serving LMA Address */
	struct in_addr			mn_serv_lma_v4_addr;/*!< \brief Serving LMA v4 Address */
	...
	int						tunnel_v4;				/*!< \brief V4 Tunnel interface index */
	...
	struct in_addr				v4_home_prefix;				/*!< \brief v4 home network prefix.*/
	uint8_t						v4_home_prefixlen;
	struct in_addr			dhcp_requested_ip;			/*!< \brief DHCP requested IP */
...
protected_pmip_cache(pthread_rwlock_t pmip_v4_lock;)
...
protected_pmip_cache(pmip_entry_t * pmipv4_cache_get(const struct in6_addr *our_addr, const struct in6_addr *peer_addr);)
...
public_pmip_cache(int pmip_cache_iterate_ext(int (*func) (void *, void *, void *), void *arg, void * usr_data);)
