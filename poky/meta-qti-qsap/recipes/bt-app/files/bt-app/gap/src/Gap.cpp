...
fprintf(stdout, "\n ########### BondStateChangedCb ######### \n");
    BtEvent *event = new BtEvent;
	//state = BT_BOND_STATE_BONDED; // added by Naveen
...
void Gap::HandleEnable(void) {
    ...
    if (adapter_properties_obj_->GetState() == BT_ADAPTER_STATE_OFF) {
       if(bluetooth_interface_->enable(false) == BT_STATUS_SUCCESS) {
           btapp_alarm_set(enable_timer, ENABLE_TIMEOUT_DELAY, enable_timer_expired, NULL);
...
void Gap::HandleDisable(void) {
    ...
    if ((adapter_properties_obj_->GetState() == BT_ADAPTER_STATE_ON) &&
       (bluetooth_interface_->disable() == BT_STATUS_SUCCESS)) {
        btapp_alarm_set(disable_timer, DISABLE_TIMEOUT_DELAY, disable_timer_expired, NULL);
...
void Gap::ProcessEvent(BtEvent* event) {
	...
	if (event)
		fprintf(stdout, " Processing event %d", event->event_id);
	else {
		fprintf(stdout, " Incoming event is NULL!!!");
		return;
	}
#endif
	ALOGD(LOGTAG " Processing event %d", event->event_id);
    ...
    switch (event->event_id) {
        case GAP_EVENT_ADAPTER_STATE:
            ...
            if ( event->state_event.status == BT_STATE_ON ) {
                ...
                if (enable_timer)
                    btapp_alarm_cancel(enable_timer);
                ...
                if (disable_timer)
                    btapp_alarm_cancel(disable_timer);
        ...
        case GAP_API_ENABLE:
            ...
            btapp_alarm_set(profile_startup_timer, PROFILE_STARTUP_TIMEOUT_DELAY,
            ...
            for(profile_id = PROFILE_ID_A2DP_SINK; profile_id < PROFILE_ID_MAX;
				...
				if (profile_id == PROFILE_ID_A2DP_SINK
					|| profile_id == PROFILE_ID_BT_AM
					|| profile_id == PROFILE_ID_AVRCP)
				{
					if(profile_config[profile_id].is_enabled) {
						bt_event = new BtEvent;
					...
					ALOGD(LOGTAG " skipping start to Profile %d",
							profile_id);
				}
#endif
			if(profile_config[profile_id].is_enabled) {
        ...
        case PROFILE_EVENT_START_DONE:
            ...
            for(profile_id = PROFILE_ID_A2DP_SINK; profile_id < PROFILE_ID_MAX;
				...
				if (profile_id == PROFILE_ID_A2DP_SINK
					...
					|| profile_id == PROFILE_ID_AVRCP)
				{
					//don't check the stripped off profiles
					if((profile_config[profile_id].is_enabled)  &&
						(!profile_config[profile_id].start_status)) {
						return;
					}
                ...
                if((profile_config[profile_id].is_enabled)  &&
                    (!profile_config[profile_id].start_status)) {
					//ALOGD(LOGTAG " issue in starting profile");
					ALOGD(LOGTAG " issue in starting Profile %d ",profile_id);
                    return;
                }
            ...
            btapp_alarm_cancel(profile_startup_timer);
            HandleEnable();
        ...
        case PROFILE_EVENT_STOP_DONE:
            ...
            btapp_alarm_cancel(profile_stop_timer);
            HandleDisable();
            break;
        ...
        case A2DP_SINK_CLEANUP_DONE:
            ...
            btapp_alarm_set(profile_stop_timer, PROFILE_STOP_TIMEOUT_DELAY,
...
#if 0 //HCU
			addr_str = (char*) malloc(sizeof(char) * 40);
			bdaddr_to_string(&event->bond_device.bd_addr, addr_str, 40);
			fprintf(stdout, " Creating bond with device : %s ", addr_str);
			delete(addr_str);
#endif
...
fprintf(stdout, "\n ########### Gap checking remote device ######### \n");
    ...
    this->adapter_properties_obj_ = new AdapterProperties(bluetooth_interface_,
                                                        remote_devices_obj_);
fprintf(stdout, "\n ########### Gap checking a2dp sink profile ######### \n");
    for(profile_id = PROFILE_ID_A2DP_SINK; profile_id < PROFILE_ID_MAX;
                                                        profile_id++) {
...
fprintf(stdout, "\n ########### Gap checking vendor interface ######### \n");
    ...
    if( !(profile_startup_timer = btapp_alarm_new())) {
        ALOGE(LOGTAG, " unable to create profile_startup_timer timer.");
		fprintf(stdout, "\n ########### unable to create profile_startup_timer timer. ######### \n");
        return;
    }

    if( !(profile_stop_timer = btapp_alarm_new())) {
        ALOGE(LOGTAG, " unable to create profile_stop_timer timer.");
		fprintf(stdout, "\n ########### unable to create profile_stop_timer timer. ######### \n");
        return;
    }

    if( !(enable_timer = btapp_alarm_new())) {
        ALOGE(LOGTAG, " unable to create enable_timer timer.");
		fprintf(stdout, "\n ########### unable to create enable_timer timer. ######### \n");
        return;
    }

    if( !(disable_timer = btapp_alarm_new())) {
        ALOGE(LOGTAG, " unable to create disable_timer timer.");
		fprintf(stdout, "\n ########### unable to create disable_timer timer. ######### \n");
        return;
    }
	fprintf(stdout, "\n ########### Gap Exited######### \n");
}
...
Gap :: ~Gap() {
    ...
    btapp_alarm_free(profile_startup_timer);
    profile_startup_timer = NULL;

    btapp_alarm_free(profile_stop_timer);
    profile_stop_timer = NULL;

    btapp_alarm_free(enable_timer);
    enable_timer = NULL;

    btapp_alarm_free(disable_timer);
    disable_timer = NULL;
