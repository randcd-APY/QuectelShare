...
static void btapp_alarm_set_internal(alarm_t *alarm, period_ms_t deadline, alarm_callback_t cb, void *data, bool is_periodic);
static void schedule_next_instance(alarm_t *alarm, bool force_reschedule);
static void reschedule_root_alarm(void);
static void timer_callback(void *data);
static void callback_dispatch(void *context);

alarm_t *btapp_alarm_new(void) {
...
void btapp_alarm_free(alarm_t *alarm) {
  if (!alarm)
    return;

  btapp_alarm_cancel(alarm);
  pthread_mutex_destroy(&alarm->callback_lock);
  osi_free(alarm);
}

period_ms_t btapp_alarm_get_remaining_ms(const alarm_t *alarm) {
  assert(alarm != NULL);
...
void btapp_alarm_set(alarm_t *alarm, period_ms_t deadline, alarm_callback_t cb, void *data) {
  btapp_alarm_set_internal(alarm, deadline, cb, data, false);
}

void btapp_alarm_set_periodic(alarm_t *alarm, period_ms_t period, alarm_callback_t cb, void *data) {
  btapp_alarm_set_internal(alarm, period, cb, data, true);
}

// Runs in exclusion with btapp_alarm_cancel and timer_callback.
static void btapp_alarm_set_internal(alarm_t *alarm, period_ms_t period, alarm_callback_t cb, void *data, bool is_periodic) {
#if 0 //HCU
LOG_ERROR(LOG_TAG, "%s setting alarm : is_periodic = %d period= %d",
		  __func__, is_periodic, period);
  if(alarms == NULL) {
	  if (!lazy_initialize())
		  return;
  }
...
void btapp_alarm_cancel(alarm_t *alarm) {
  assert(alarms != NULL);
  assert(alarm != NULL);

  pthread_mutex_lock(&monitor);

  bool needs_reschedule = (!btapp_list_is_empty(alarms) && btapp_list_front(alarms) == alarm);

  btapp_list_remove(alarms, alarm);
  alarm->deadline = 0;
  alarm->callback = NULL;
  alarm->data = NULL;
...
static void schedule_next_instance(alarm_t *alarm, bool force_reschedule) {
  ...
  if (btapp_list_is_empty(alarms) || ((alarm_t *)btapp_list_front(alarms))->deadline >= alarm->deadline)
    btapp_list_prepend(alarms, alarm);
  else
    for (list_node_t *node = btapp_list_begin(alarms); node != btapp_list_end(alarms); node = btapp_list_next(node)) {
      list_node_t *next = btapp_list_next(node);
      if (next == btapp_list_end(alarms) || ((alarm_t *)btapp_list_node(next))->deadline >= alarm->deadline) {
        btapp_list_insert_after(alarms, node, alarm);
        break;
      }
    }

  // If the new alarm has the earliest deadline, we need to re-evaluate our schedule.
  if (force_reschedule || needs_reschedule || (!btapp_list_is_empty(alarms) && btapp_list_front(alarms) == alarm))
    reschedule_root_alarm();
}
...
static void reschedule_root_alarm(void) {
  ...
  if (btapp_list_is_empty(alarms))
    goto done;

  alarm_t *next = btapp_list_front(alarms);
...
static void callback_dispatch(UNUSED_ATTR void *context) {
  while (true) {
    ...
    // nothing left to do.
    if (btapp_list_is_empty(alarms) || (alarm = btapp_list_front(alarms))->deadline > now()) {
      reschedule_root_alarm();
      pthread_mutex_unlock(&monitor);
      continue;
    }
  ...
  LOG_DEBUG_("%s Callback thread exited", __func__);
}
