...
static void btapp_future_free(future_t *future);

future_t *btapp_future_new(void) {
  ...
  return ret;
error:;
  btapp_future_free(ret);
  return NULL;
}

future_t *btapp_future_new_immediate(void *value) {
  future_t *ret = osi_calloc(sizeof(future_t));
  if (!ret) {
    LOG_ERROR("%s unable to allocate memory for return value.", __func__);
    goto error;
...
void btapp_future_ready(future_t *future, void *value) {
  assert(future != NULL);
...
void *btapp_future_await(future_t *future) {
  assert(future != NULL);
