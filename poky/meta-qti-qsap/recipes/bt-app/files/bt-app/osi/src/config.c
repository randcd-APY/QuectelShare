...
config_t *config_new_empty(void) {
  ...
  config->sections = btapp_list_new(section_free);
  if (!config->sections) {
...
void config_remove(config_t *config) {
  ...
  btapp_list_free(config->sections);
  osi_free(config);
}
...
void config_set_string(config_t *config, const char *section, const char *key, const char *value) {
  ...
  if (sec)
    btapp_list_append(config->sections, sec);
  else
  {
  ...
  for (const list_node_t *node = btapp_list_begin(sec->entries); node != btapp_list_end(sec->entries); node = btapp_list_next(node)) {
    entry_t *entry = btapp_list_node(node);
  ...
  btapp_list_append(sec->entries, entry);
}
...
bool config_remove_section(config_t *config, const char *section) {
  ...
  return btapp_list_remove(config->sections, sec);
}
...
bool config_remove_key(config_t *config, const char *section, const char *key) {
  ...
  return btapp_list_remove(sec->entries, entry);
}

const config_section_node_t *config_section_begin(const config_t *config) {
  assert(config != NULL);
  return (const config_section_node_t *)btapp_list_begin(config->sections);
}

const config_section_node_t *config_section_end(const config_t *config) {
  assert(config != NULL);
  return (const config_section_node_t *)btapp_list_end(config->sections);
}

const config_section_node_t *config_section_next(const config_section_node_t *node) {
  assert(node != NULL);
  return (const config_section_node_t *)btapp_list_next((const list_node_t *)node);
}

const char *config_section_name(const config_section_node_t *node) {
  assert(node != NULL);
  const list_node_t *lnode = (const list_node_t *)node;
  const section_t *section = (const section_t *)btapp_list_node(lnode);
  return section->name;
}
...
bool config_save(const config_t *config, const char *filename) {
  ...
  for (const list_node_t *node = btapp_list_begin(config->sections); node != btapp_list_end(config->sections); node = btapp_list_next(node)) {
    const section_t *section = (const section_t *)btapp_list_node(node);
    fprintf(fp, "[%s]\n", section->name);

    for (const list_node_t *enode = btapp_list_begin(section->entries); enode != btapp_list_end(section->entries); enode = btapp_list_next(enode)) {
      const entry_t *entry = (const entry_t *)btapp_list_node(enode);
      fprintf(fp, "%s = %s\n", entry->key, entry->value);
    }

    // Only add a separating newline if there are more sections.
    if (btapp_list_next(node) != btapp_list_end(config->sections))
      fputc('\n', fp);
  }
...
static void config_parse(FILE *fp, config_t *config) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (*line_ptr == '[') {
      ...
      if (line_ptr[len - 1] != ']') {
        LOG_DEBUG_("%s unterminated section name on line %d.", __func__, line_num);
        continue;
      }
      ...
      if (!split) {
        LOG_DEBUG_("%s no key/value separator found on line %d.", __func__, line_num);
        continue;
      }
...
static section_t *section_new(const char *name) {
  ...
  section->entries = btapp_list_new(entry_free);
  return section;
}
...
static void section_free(void *ptr) {
  ...
  btapp_list_free(section->entries);
  osi_free(section);
}

static section_t *section_find(const config_t *config, const char *section) {
  for (const list_node_t *node = btapp_list_begin(config->sections); node != btapp_list_end(config->sections); node = btapp_list_next(node)) {
    section_t *sec = btapp_list_node(node);
