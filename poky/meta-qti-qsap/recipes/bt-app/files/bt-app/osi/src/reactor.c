...
reactor_t *reactor_new(void) {
  ...
  ret->invalidation_list = btapp_list_new(NULL);
...
void reactor_free(reactor_t *reactor) {
  ...
  btapp_list_free(reactor->invalidation_list);
...
void reactor_unregister(reactor_object_t *obj) {
  ...
  btapp_list_append(reactor->invalidation_list, obj);
...
static reactor_status_t run_reactor(reactor_t *reactor, int iterations) {
  ...
  for (int i = 0; iterations == 0 || i < iterations; ++i) {
    pthread_mutex_lock(&reactor->list_lock);
    btapp_list_clear(reactor->invalidation_list);
    pthread_mutex_unlock(&reactor->list_lock);
    ...
    for (int j = 0; j < ret; ++j) {
      ...
      pthread_mutex_lock(&reactor->list_lock);
      if (btapp_list_contains(reactor->invalidation_list, object)) {
        pthread_mutex_unlock(&reactor->list_lock);
        continue;
      }
