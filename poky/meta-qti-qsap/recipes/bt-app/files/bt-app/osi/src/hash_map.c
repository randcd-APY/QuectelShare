...
bool hash_map_set(hash_map_t *hash_map, const void *key, void *data) {
  ...
  if (hash_map->bucket[hash_key].list == NULL) {
    hash_map->bucket[hash_key].list = btapp_list_new_internal(bucket_free_, hash_map->allocator);
    if (hash_map->bucket[hash_key].list == NULL)
        return false;
  }
  list_t *hash_bucket_list = hash_map->bucket[hash_key].list;
  ...
  if (hash_map_entry) {
    ...
    bool rc = btapp_list_remove(hash_bucket_list, hash_map_entry);
    assert(rc == true);
  } else {
    hash_map->hash_size++;
  }
  ...
  return btapp_list_append(hash_bucket_list, hash_map_entry);
}
...
bool hash_map_erase(hash_map_t *hash_map, const void *key) {
  ...
  return btapp_list_remove(hash_bucket_list, hash_map_entry);
}
...
void hash_map_clear(hash_map_t *hash_map) {
  ...
  for (hash_index_t i = 0; i < hash_map->num_bucket; i++){
    if (hash_map->bucket[i].list == NULL)
      continue;
    btapp_list_free(hash_map->bucket[i].list);
...
void hash_map_foreach(hash_map_t *hash_map, hash_map_iter_cb callback, void *context) {
  ...
  for (hash_index_t i = 0; i < hash_map->num_bucket; ++i){
    if (hash_map->bucket[i].list == NULL)
      continue;
    for (const list_node_t *iter = btapp_list_begin(hash_map->bucket[i].list);
        iter != btapp_list_end(hash_map->bucket[i].list);
        iter = btapp_list_next(iter)) {
       hash_map_entry_t *hash_map_entry = (hash_map_entry_t *)btapp_list_node(iter);
...
static hash_map_entry_t * find_bucket_entry_(list_t *hash_bucket_list,
  ...
  for (const list_node_t *iter = btapp_list_begin(hash_bucket_list);
      iter != btapp_list_end(hash_bucket_list);
      iter = btapp_list_next(iter)) {
    hash_map_entry_t *hash_map_entry = (hash_map_entry_t *)btapp_list_node(iter);
