...
thread_t *thread_new_sized(const char *name, size_t work_queue_capacity) {
  ...
  ret->work_queue = btapp_fixed_queue_new(work_queue_capacity);
  if (!ret->work_queue)
    goto error;
...
error:;
  if (ret) {
    btapp_fixed_queue_free(ret->work_queue, osi_free);
...
void thread_free(thread_t *thread) {
  ...
  btapp_fixed_queue_free(thread->work_queue, osi_free);
...
bool thread_post(thread_t *thread, thread_fn func, void *context) {
  ...
  btapp_fixed_queue_enqueue(thread->work_queue, item);
  return true;
}
...
static void *run_thread(void *start_arg) {
  ...
  int fd = btapp_fixed_queue_get_dequeue_fd(thread->work_queue);
  ...
  size_t count = 0;
  work_item_t *item = btapp_fixed_queue_try_dequeue(thread->work_queue);
  while (item && count <= btapp_fixed_queue_capacity(thread->work_queue)) {
    item->func(item->context);
    osi_free(item);
    item = btapp_fixed_queue_try_dequeue(thread->work_queue);
    ++count;
  }

  if (count > btapp_fixed_queue_capacity(thread->work_queue))
    LOG_DEBUG_("%s growing event queue on shutdown.", __func__);
...
static void work_queue_read_cb(void *context) {
  ...
  work_item_t *item = btapp_fixed_queue_dequeue(queue);
  if ( item != NULL ) {
      item->func(item->context);
      osi_free(item);
  }
