...
static int parse_cfg_xml(const char *cfg_path,
    ...
    int ret = 0;
    int exist_module_ui = 0;
    int exist_module_pcba = 0;
    char *value = NULL;
    xmlDocPtr doc = NULL;
    xmlNodePtr node = NULL;
    ...
    node = find_node(doc, &key);
    if(NULL == node) {
        ALOGE("Not found common config for pcba: [%s]", XML_NODE_NAME_MODULES_PCBA);
    } else {
        exist_module_pcba = 1;
        ret = parse_module_info(node->xmlChildrenNode, ordered_pcba_module, module_pcba_map);
        if(SUCCESS != ret) {
            ALOGE("Parse common pcba config failed\n");
            xmlFreeDoc(doc);
            return FAILED;
        }
    }
    ...
    // parse product special config
    SET_NODE_NAME(key, XML_NODE_NAME_PRODUCT);
    SET_ATTR_INFO(key.node_arrt1, XML_NODE_ATTR_TYPE, board_info.subtype_id);
    node = find_node(doc, &key);
    if(NULL == node) {
        ALOGI("Not found product config, product type=%s", board_info.subtype_id);
        if(!exist_module_ui && !exist_module_pcba) {
            ALOGE("Not found module config");
            xmlFreeDoc(doc);
            return FAILED;
        }
    } else {
        ...
        if(NULL == sub_node) {
            ...
            exist_module_ui = 1;
            ret = parse_module_info(sub_node->xmlChildrenNode, ordered_ui_module, module_ui_map);
            if(SUCCESS != ret) {
        ...
        if(NULL == sub_node) {
            ...
            exist_module_pcba = 1;
            ret = parse_module_info(sub_node->xmlChildrenNode, ordered_pcba_module, module_pcba_map);
            if(SUCCESS != ret) {
...
int create_mmi_cfg(void) {
    ...
    // create file: "/data/FTM_AP/mmi.cfg"
    ...
    // create file: "/data/FTM_AP/mmi-pcba.cfg"
...
bool is_create_mmi_cfg(void) {
    ...
    // Create "/data/FTM_AP/.setting" file to save the checksum
    ...
    // if "/data/FTM_AP/mmi.cfg" or "/data/FTM_AP/mmi-pcba.cfg" not exist, create the files
    if(!check_file_exist(MMI_CONFIG) || !check_file_exist(MMI_PCBA_CONFIG)) {
        save_checksum(check_sum_file, cal_checksum);
        return true;
    }

    // Get the checksum from "/data/FTM_AP/.setting"
