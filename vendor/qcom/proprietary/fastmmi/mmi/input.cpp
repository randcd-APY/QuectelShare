...
static bool is_screen_scroll(list < point_t > *points) {
    ...
    if(tmp.x_max - tmp.x_min <= PRESS_SHAKE_X && tmp.y_max - tmp.y_min <= PRESS_SHAKE_Y) {
        ALOGI("it is not screen scroll event, just click");
        return false;
    }
...
static bool is_move_x(list < point_t > *points) {
    ...
    ALOGI("move x home screen : (%d, %d,%d)\n", diff_x, diff_y, points->size());
...
static void screen_scroll_acc() {
    ...
    ALOGI("Scroll ACC for time diff: %llu, speed=%f,nsteps=%d,scroll_acc=%f ", time_diff, speed, nsteps, scroll_acc);

    for(i = 0; i < nsteps; i++) {
        step_len = v0 * FLASH_FREQ_MS + (scroll_acc * FLASH_FREQ_MS * FLASH_FREQ_MS) / 2;
        v0 = v0 + scroll_acc * FLASH_FREQ_MS;
        ALOGI("Scroll ACC for time diff: v0=%f,step_len=%d ", v0, step_len);
...
static void screen_scroll(list < point_t > *points) {
    ...
    if(points == NULL || points->size() < 1) {
        ALOGI("Less then one point, no need to scroll screen ");
        return;
    }
    ...
    ALOGI("Scroll screen for time diff: %llu, nsteps=%d,step_len=%d,back_y=%d,fisrt_y=%d", time_diff, nsteps, step_len,
          back.y, first.y);
    for(i = 0; i < nsteps; i++) {
           ...
           && curlay->m_listview->get_items() != NULL && is_point_in_rect(tail_point, curlay->m_listview->get_rect())) {
            ...
            for(iter = curlay->m_listview->get_items()->begin(); iter != curlay->m_listview->get_items()->end(); iter++) {
                ...
                if(is_point_in_rect(tail_point, &obj->rect)) {
                    ...
                    r->module = obj->mod;
                    enqueue_runnable_locked(&g_runnable_queue, r);
                    ALOGI("g_sem_runable:%p;%d", &g_sem_runable, g_sem_runable);
                    sem_post(&g_sem_runable);
                }
            }
        }
...
static int sw_callback(int type, int code, int value) {
    ...
    if(type == EV_SW) {
        ...
        ALOGI("%s, keycode[%d],down=%d \n", __FUNCTION__, code, down);
        if(down)
            set_bit(code, swbit);
        else
...
void init_input() {
    ...
    if(ret < 0)
        ALOGI("%s, init input fail , (%s)\n", __FUNCTION__, strerror(errno));

    ALOGI("-== sem runable:%d", g_sem_runable);
    usleep(1000 * 1000);
...
void *input_waiting_thread(void *) {
    ...
    ALOGI("input_waiting_thread started\n");
    while(1) {
        if(!ev_wait(-1))
            ev_dispatch();
    }
    ...
    while(1) {
        ...
        if((r != NULL) && (r->cb != NULL) && (r->module != NULL)) {
            ...
            if((time_diff < 100 * 1000) && (time_diff > 0)) {
                ALOGI("Input interval time=%llu, input invalid.\n", time_diff);
                continue;
            } else
                r->cb(r->module);

            ALOGI("handle input event for module:[%s]\n", r->module);
        }
    }
    return NULL;
}
