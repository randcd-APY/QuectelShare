...
/*Delay between UART Clock Vote Off and Wakelock release*/
#define UART_VOTEOFF_WAKELOCK_DELAY      "150"
...
static void ibs_voteOff_timer_stop(void);
    ...
    if (new_vote != old_vote) {
        ...
        else
        {
            ...
            ALOGI("%s: vote UART CLK OFF using UART driver's ioctl()",
                __FUNCTION__);
            //enable the vote_off timer to test the power number
...
static void ibs_serial_clk_voteoff_timeout()
{
    ...
    if(!(ibs_state_machine.rx_vote || ibs_state_machine.tx_vote))
    {
...
static void ibs_voteOff_timer_start()
{
    ...
    struct itimerspec ts;
    struct sigevent se;
    char get_delay[PROPERTY_VALUE_MAX];
    static int uart_voteoff_delay = 0;
    ...
    if(ibs_voteoff_struct.timer_created == false)
    {
        ...
        if (status == 0)
        {
            ...
            property_get_bt("persist.set.rx_delay", get_delay, UART_VOTEOFF_WAKELOCK_DELAY);
            uart_voteoff_delay = atoi(get_delay);
        }
    }

    if(ibs_voteoff_struct.timer_created == true)
    {
        if (uart_voteoff_delay != 0) {
...
void ibs_recv_ibs_cmd(uint8_t *data)
{
    ...
    case IBS_WAKE_ACK: ////0xFC
        ...
        default:
            ...
            if (pthread_mutex_trylock(&ibs_state_machine.hci_ibs_lock) == 0) {
