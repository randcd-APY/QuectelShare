    ...
    switch ( rsp[CMD_RSP_OFFSET] )
    {
        ...
        switch(rsp[RSP_TYPE_OFFSET])
        {
            ...
            case EDL_BOARD_ID_RESPONSE:
                ...
                if (rsp[6] == 0x00) {
                    board_id[0] = 'b';
                    board_id[1] = convert2ascii ((rsp[7] & MSB_NIBBLE_MASK) >> 4);
                    board_id[2] = convert2ascii (rsp[7] & LSB_NIBBLE_MASK);
                    board_id[3] = '\0';
                } else {
                    board_id[0] = convert2ascii (rsp[6] && LSB_NIBBLE_MASK);
                    board_id[1] = convert2ascii ((rsp[7] & MSB_NIBBLE_MASK) >> 4);
                    board_id[2] = convert2ascii (rsp[7] & LSB_NIBBLE_MASK);
                    board_id[3] = '\0';
                }
...
int get_tlv_file(char *file_path)
{
    FILE * pFile;
    long fileSize;
    int readSize, total_segment, remain_size, nvm_length, nvm_tot_len, nvm_index, i;
...
int download_tlv_file(int fd)
{
    ...
    memcpy(nvm_file_path_bid, nvm_file_path, strlen(nvm_file_path) - 3);
...
int get_boardid_req(int fd)
{
    ...
    /* boardid request is responded with a VS event */
    wait_vsc_evt = TRUE;
    /* Frame the HCI CMD to be sent to the Controller */
...
void cherokee_pre_shutdown_vs_cmd(int fd)
{
    int ret = 0;
    unsigned char cmd[4] = {0x01, 0x08, 0xFC, 0x00};
    unsigned char rsp[HCI_MAX_EVENT_SIZE];

    ALOGI(" %s ", __FUNCTION__);

    ret = wcnss_wake_assert();
    if(ret == -1)
    {
       ALOGE("%s:Failed to send wake ibs byte.",__func__);
       return;
    }
    ...
    if (ret != 4) {
        ...
        sem_init(&pre_shutdown_complete,0, 0);
        if (clock_gettime(CLOCK_REALTIME, &ts) == -1)
            ALOGE("get clock_gettime error");

        ts1 = ts;
        /* 100 ms timer for shutdown VS command */
        ts.tv_nsec += 100*1000000L;
        if (ts.tv_nsec >= 1000 * 1000000L)
        {
            ts.tv_sec += 1;
            ts.tv_nsec = ts.tv_nsec - (1000 * 1000000L);
        }
        sem_status = sem_timedwait(&pre_shutdown_complete, &ts);
        if(sem_status == -1)
        {
            ALOGE("%s: Command Complete Event Timeout > 100 ms", __FUNCTION__);
        }
        else
        {
            ...
            if (ts1.tv_nsec >= 1000 * 1000000L)
                ts1.tv_nsec = ts1.tv_nsec - (1000 * 1000000L);
            ALOGI("%s: Continue shutdown process(delayed by %ld us )", __FUNCTION__, (ts2.tv_nsec - ts1.tv_nsec)/1000L);
        }
        sem_destroy(&pre_shutdown_complete);
    }
}
