...
#define RING_BUF_LOG_PATH        "/data/misc/bluetooth"
#define CRASH_DUMP_PATH_BUF_SIZE 60
#define RING_BUF_PATH_BUF_SIZE 255
...
static Reason_map_st reset_reason [] = {
    ...
    { BT_CRASH_REASON_ABT            ,    "ABT"},
    { BT_CRASH_REASON_RAMMASK        ,    "RAM Mask"},
    { BT_CRASH_REASON_PREBARK        ,    "Pre Bark"},
    { BT_CRASH_REASON_BUSERROR       ,    "Bus Error"},
    { BT_CRASH_REASON_IOP_FATAL      ,    "IOP Fatal"},
...
void ts_get_conv (void *timestamp, int type, char *log_buf, int len)
{
    ...
    if (is_unified_logging_enabled()) {
        ...
        case LOG_BT_HCI_CMD_C:
            ALOGI("<<< ts <%02d:%02d:%02d:%06d> type <%s> opcode: <%0x> subopcode: <%0x> len: %d",
                hour, minute, second, (unsigned int)tv.tv_usec, log_type, log_buf[0], log_buf[1], len);
            break;
        case LOG_BT_HCI_TX_ACL_C:
        case LOG_BT_HCI_RX_ACL_C:
            ALOGI("<<< ts <%02d:%02d:%02d:%06d> type <%s> conn handle: <%0x %0x> len: %d",
                hour, minute, second, (unsigned int)tv.tv_usec, log_type, log_buf[0], log_buf[1], len);
            break;
        case LOG_BT_HCI_EV_C:
            ALOGI("<<< ts <%02d:%02d:%02d:%06d> type <%s> opcode: <%0x> len: %d",
                hour, minute, second, (unsigned int )tv.tv_usec, log_type, log_buf[0], len);
            break;
        default:
            ALOGI("<<< ts <%02d:%02d:%02d:%06d> type <%s> len: %d", hour, minute, second,
                (unsigned int)tv.tv_usec, log_type, len);
        }
    }
...
void  dump_uart_ipc_logs(int inx) {
    int ret;
    //UART debug fs should be be of 48K buffer
    ...
    ALOGI("%s", __func__);
    ALOGE("%s inx = %d", __func__, inx);
    if (is_debug_out_ring_buf_enabled()) {
        commit_out_ring_buffer (inx);
    }

    if (is_debug_in_ring_buf_enabled()) {
        commit_in_ring_buffer (inx);
    }

#ifdef DUMP_IPC_LOG

    /* Find index of the oldest UART IPC log entry */
    inx_oldest = (inx + 1) % MAX_CRASH_DUMP_COUNT ;

    snprintf(dpath, UART_IPC_PATH_BUF_SIZE,
            "/data/misc/bluetooth/%.02d-uart_ipc_txlogs.txt", inx_oldest);
    remove (dpath);
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE,
            "/data/misc/bluetooth/%.02d-uart_ipc_txlogs.txt", inx);
    remove (dpath);
    ...
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE,
             "/data/misc/bluetooth/%.02d-uart_ipc_rxlogs.txt", inx_oldest);
    remove(dpath);
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE,
             "/data/misc/bluetooth/%.02d-uart_ipc_rxlogs.txt", inx);
    remove (dpath);
    strlcpy(spath, ipc_log_dir, sizeof(spath));
    ...
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE,
             "/data/misc/bluetooth/%.02d-uart_ipc_statelogs.txt", inx_oldest);
    remove(dpath);
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE,
             "/data/misc/bluetooth/%.02d-uart_ipc_statelogs.txt", inx);
    remove (dpath);
    strlcpy(spath, ipc_log_dir, sizeof(spath));
    ...
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE,
             "/data/misc/bluetooth/%.02d-uart_ipc_pwrlogs.txt", inx_oldest);
    remove(dpath);
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE,
             "/data/misc/bluetooth/%.02d-uart_ipc_pwrlogs.txt", inx);
    remove (dpath);
    strlcpy(spath, ipc_log_dir, sizeof(spath));
    ...
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE, "/data/misc/bluetooth/%0.2d-uart_txlogs.txt", inx_oldest);
    remove(dpath);
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE, "/data/misc/bluetooth/%0.2d-uart_txlogs.txt", inx);
    remove (dpath);
    dump_UART_logs(UART_TX_LOG_COLLECTION_PATH, dpath, 4000, 4000);

    snprintf(dpath, UART_IPC_PATH_BUF_SIZE, "/data/misc/bluetooth/%0.2d-uart_rxlogs.txt", inx);
    remove(dpath);
    snprintf(dpath, UART_IPC_PATH_BUF_SIZE, "/data/misc/bluetooth/%0.2d-uart_rxlogs.txt", inx);
    remove (dpath);
    ...
    if (eventBuf[3] == LOG_BT_CONTROLLER_LOG)
    {
      switch(eventBuf[4])
      {
         ...
         case LOG_BT_MESSAGE_TYPE_MEM_DUMP:
         {
            ...
            static bool dump_started = false, dump_missing = false;
            ...
            if (seq_num == 0 && is_debug_force_special_bytes()) {
               ...
               was_active = soc_crash_wait_timer_stop();
               if (was_active) {
            ...
            if (!dump_started)
            {
                ...
                {   /* Ring Buffer already collected, index should correspond to it */
                    if(!inx_scan)
                        inx_scan = MAX_CRASH_DUMP_COUNT - 1;
                    else
                        inx_scan = inx_scan - 1;
                }
                ALOGD("%s: index of the dumps = %d", __func__, inx_scan);
                ...
                /* Remove the oldest crashdump entry */
                inx_oldest = (inx_scan + 1) % MAX_CRASH_DUMP_COUNT;
                delete_crashdump_file(inx_oldest);

                snprintf(path, CRASH_DUMP_PATH_BUF_SIZE, "%s/%.02d-bt_fw_crashdump_partial.bin",
                            ROME_CRASH_RAMDUMP_PATH,inx_scan);
                remove (path);

                snprintf(path, CRASH_DUMP_PATH_BUF_SIZE, "%s/%.02d-bt_fw_crashdump.bin",
                            ROME_CRASH_RAMDUMP_PATH,inx_scan);
                remove (path);
            ...
            if(dumpFd >= 0) {
                for ( ; (seq_num > seq_num_cnt) && (seq_num !=LAST_SEQUENCE_NUM) ; seq_num_cnt++)
                {
                    ...
                    dump_missing = true;
                    if (seq_num_cnt == 0)
                       /* first pack has total ram dump size (4 bytes) which is not copied */
                       packet_len -=4;

                    if (p != NULL) {
                       /* writing 0xff bytes in place of the missing packet */
                       memset(temp_buf, 0xff, packet_len);
...
int get_dump_index()
{
    ...
    static char path[RING_BUF_PATH_BUF_SIZE];
...
void delete_crashdump_file(int inx)
{
    char path[CRASH_DUMP_PATH_BUF_SIZE];

    snprintf(path, CRASH_DUMP_PATH_BUF_SIZE, "%s/%.02d-bt_fw_crashdump_partial.bin",
                ROME_CRASH_RAMDUMP_PATH,inx);
    ALOGI("%s : deleting the oldest file : %s",__func__, path);
    remove (path);
