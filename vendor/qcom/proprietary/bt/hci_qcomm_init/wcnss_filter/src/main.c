...
#endif
extern void cherokee_pre_shutdown_vs_cmd(int fd);
extern sem_t pre_shutdown_complete;
extern bool uart_dump_collected;
...
void commit_out_ring_buffer (int inx) {
    ...
    snprintf((char *)dpath, 255, "/data/misc/bluetooth/%.02d-out_ringbuf_logs.txt",
                            inx_oldest);
    remove(dpath);
...
void commit_in_ring_buffer (int inx) {
    unsigned char dpath[255];
    int dfd, ret;
    int inx_oldest;
    ALOGV(">%s", __func__);

    /* Find index of the oldest in ring buffer entry */
    inx_oldest = (inx + 1) % MAX_CRASH_DUMP_COUNT;

    snprintf((char *)dpath, 255, "/data/misc/bluetooth/%.02d-in_ringbuf_logs.txt",
                            inx_oldest);
    remove(dpath);
...
//             wcnss_device_can_sleep();
...
#endif
           if((hdr[0] == 0x10) && !uart_dump_collected) {
               ALOGI("%s: hardware error event received without crashdump, collecting UART IPC logs", __func__);
               int inx = get_dump_index();
               dump_uart_ipc_logs(inx);
               /* Remove the oldest crashdump entry */
               inx = (inx + 1) % MAX_CRASH_DUMP_COUNT;
               delete_crashdump_file(inx);
           }
    ...
    /* Check if it is pre-shutdown command comeplete event*/
     if ( (buf[0] == LOG_BT_EVT_PACKET_TYPE) && (buf[1] == LOG_BT_EVT_CMD_CMPLT)
         && (buf[4] == LOG_BT_VS_CMD_PRE_SHUTDOWN) && (buf[5] == LOG_BT_VS_CMD_OPCODE))
     {
         ALOGI("cherokee_pre_shutdown_vs_cmd Event Received");
         sem_post(&pre_shutdown_complete);
         free(buf);
         return 0;
     }
...
int deinit_hci_transport() {
    ...
    switch (soc_type) {
        ...
        case BT_SOC_CHEROKEE: {
            if (getpid() == gettid()) {
                ALOGI("%s: called from Rx&main thread, do not send pre-shutdown",
                    __func__);
            }
            else {
                cherokee_pre_shutdown_vs_cmd(fd_transport);
            }
