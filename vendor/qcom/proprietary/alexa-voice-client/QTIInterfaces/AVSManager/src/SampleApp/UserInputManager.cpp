...
static const char RESTARTSVA = 'w';
...
void UserInputManager::run() {
    ...
    while (true) {
        ...
        else if (x == RESTARTSVA) {
            m_interactionManager->restartSVA();
        }
        /* AMEYA -- comment
        else if (x == HELP) {
            ConsolePrinter::simplePrint(WELCOME_MESSAGE);
        }
        */
        else if (x == TAPTOTALK) {
            m_interactionManager->tap();
	...
	alexaClientSDK::avsManager::ConsolePrinter::simplePrint("UserInputManager::run() is DONE!");
}
