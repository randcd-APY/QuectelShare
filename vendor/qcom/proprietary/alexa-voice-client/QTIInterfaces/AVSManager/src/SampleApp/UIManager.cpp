...
void UIManager::onDialogUXStateChanged(DialogUXState state) {
    m_executor.submit([this, state]() {
        ...
        if (avsCommon::sdkInterfaces::DialogUXStateObserverInterface::DialogUXState::IDLE == state) {
            ...
            m_stHal->stopRecording();
            //stop recording form Audio Hal
            m_audioHal->stopRecording();
        ...
        if (avsCommon::sdkInterfaces::DialogUXStateObserverInterface::DialogUXState::LISTENING == state) {
            ...
            //Get DOA information form ST HAL - Function not part of QSTHWInterface
            int doaIndex = m_stHal->getDOADirection();
            std::cout<< "LISTENING - DOA Index = "<< doaIndex <<std::endl;

            //Turn ON ALL LEDs - BLUE
            m_ledManager->setLEDRingOn(BLUE_LED);
        ...
        if (avsCommon::sdkInterfaces::DialogUXStateObserverInterface::DialogUXState::FINISHED == state) {
            std::cout<< "InteractionManager::onDialogUXStateChanged to FINISHED "<<std::endl;
        ...
        // AMEYA -- comment for compiling
        if (avsCommon::sdkInterfaces::DialogUXStateObserverInterface::DialogUXState::EXPECTING_SPEECH == state) {
            std::cout<< "InteractionManager::onDialogUXStateChanged to EXPECTING_SPEECH "<<std::endl;
            //Stop ST Hal recognition
            m_stHal->stopRecognition();

            //start recording from Audio Hal using ChannelIndex from STHAL
            m_audioHal->startRecording(m_stHal->getChannelIndex());
    ...
    if (m_connectionStatus == avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::Status::DISCONNECTED) {
        ...
        switch (m_dialogState) {
            ...
            case DialogUXState::EXPECTING_SPEECH:
                ConsolePrinter::prettyPrint("Expected Speech...");
                return;
...
void UIManager::addQTIInterfaces(
        ...
        std::shared_ptr<alexaClientSDK::qahwManager::QAHWManager> audioHal,
        std::shared_ptr<alexaClientSDK::avsManager::LEDManager> ledManager) {
...
void UIManager::tap() {
    std::cout << " UIManager::tap() - Opening Audio HAL for recording " << std::endl;
    //Stop ST Hal recognition
    m_stHal->stopRecognition();
