...
void InteractionManager::playbackPlay() {
    m_executor.submit([this]() { auto tempM_client = m_client.lock(); tempM_client->getPlaybackRouter()->playButtonPressed(); });
}

void InteractionManager::playbackPause() {
    m_executor.submit([this]() { auto tempM_client = m_client.lock(); tempM_client->getPlaybackRouter()->pauseButtonPressed(); });
}

void InteractionManager::playbackNext() {
    m_executor.submit([this]() { auto tempM_client = m_client.lock(); tempM_client->getPlaybackRouter()->nextButtonPressed(); });
}

void InteractionManager::playbackPrevious() {
    m_executor.submit([this]() { auto tempM_client = m_client.lock(); tempM_client->getPlaybackRouter()->previousButtonPressed(); });
}
...
void InteractionManager::setFirmwareVersion(avsCommon::sdkInterfaces::softwareInfo::FirmwareVersion firmwareVersion) {
    m_executor.submit([this, firmwareVersion]() { auto tempM_client = m_client.lock(); tempM_client->setFirmwareVersion(firmwareVersion); });
}
