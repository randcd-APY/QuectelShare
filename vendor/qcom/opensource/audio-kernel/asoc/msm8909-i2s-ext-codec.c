...
#define DRV_NAME "msm8909-i2s-wcd"
...
struct msm8909_wsa881x_dev_info {
...
static int msm8909_enable_extcodec_ext_clk(struct snd_soc_codec *codec,
...
static int msm8909_wsa881x_init(struct snd_soc_component *component)
{
	...
	struct msm8909_asoc_mach_data *pdata;
...
static int msm8909_mclk_event(struct snd_soc_dapm_widget *w,
			      struct snd_kcontrol *kcontrol, int event);

static const struct snd_soc_dapm_widget msm8909_dapm_widgets[] = {

	SND_SOC_DAPM_SUPPLY_S("MCLK", -1, SND_SOC_NOPM, 0, 0,
	msm8909_mclk_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
...
static int msm8909_get_port_id(int id)
{
	switch (id) {
...
static int ext_mi2s_clk_ctl(struct snd_pcm_substream *substream, bool enable)
{
	...
	if (enable) {
		msm8909_enable_extcodec_ext_clk(codec, 1, true);
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			mi2s_rx_clk.enable = enable;
			mi2s_rx_clk.clk_id = msm8909_get_clk_id(port_id);
			mi2s_rx_clk.clk_freq_in_hz =
				get_mi2s_rx_clk_val();
			ret = afe_set_lpass_clock_v2(port_id, &mi2s_rx_clk);
		} else if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
			mi2s_tx_clk.enable = enable;
			mi2s_tx_clk.clk_id = msm8909_get_clk_id(port_id);
		...
		msm8909_enable_extcodec_ext_clk(codec, 0, true);
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			mi2s_rx_clk.enable = enable;
			mi2s_rx_clk.clk_id = msm8909_get_clk_id(port_id);
...
static int msm_prim_auxpcm_startup(struct snd_pcm_substream *substream)
{
	...
	struct msm8909_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
...
static struct snd_soc_dai_link msm8909_9326_dai[] = {
	/* Backend DAI Links */
	{
...
static struct snd_soc_dai_link msm8909_9326_dai_links[
				ARRAY_SIZE(msm8909_dai) +
				ARRAY_SIZE(msm_tdm_fe_dai) +
				ARRAY_SIZE(msm8909_9326_dai) +
				ARRAY_SIZE(msm_afe_rxtx_lb_be_dai_link) +
				ARRAY_SIZE(msm_tdm_be_dai)];

static struct snd_soc_card snd_soc_card_9326_msm8909 = {
	.name = "msm8909-tashalite-snd-card-tdm",
};

struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	struct snd_soc_card *card = &snd_soc_card_9326_msm8909;
	...
	len3 = len2 + ARRAY_SIZE(msm8909_9326_dai);

	memcpy(msm8909_9326_dai_links, msm8909_dai,
			sizeof(msm8909_dai));
	memcpy(msm8909_9326_dai_links + len1, msm_tdm_fe_dai,
			sizeof(msm_tdm_fe_dai));
	memcpy(msm8909_9326_dai_links + len2, msm8909_9326_dai,
			sizeof(msm8909_9326_dai));

	if (of_property_read_bool(dev->of_node, "qcom,afe-rxtx-lb")) {
		dev_dbg(dev, "%s(): AFE RX to TX loopback supported\n",
				__func__);
		memcpy(msm8909_9326_dai_links + len3,
	...
	if (of_property_read_bool(dev->of_node, "qcom,tdm-audio-intf")) {
		dev_dbg(dev, "%s(): TDM support present\n",
				__func__);
		memcpy(msm8909_9326_dai_links + len3, msm_tdm_be_dai,
			sizeof(msm_tdm_be_dai));
		len3 += ARRAY_SIZE(msm_tdm_be_dai);
	}

	card->dai_link = msm8909_9326_dai_links;
	card->num_links = len3;
	card->dev = dev;
...
static int msm8909_init_wsa_dev(struct platform_device *pdev,
		struct snd_soc_card *card)
{
	...
	struct msm8909_wsa881x_dev_info *wsa881x_dev_info;
	...
	wsa881x_dev_info = devm_kcalloc(&pdev->dev, wsa_max_devs,
			sizeof(struct msm8909_wsa881x_dev_info),
			GFP_KERNEL);
	if (!wsa881x_dev_info)
		return -ENOMEM;
	...
	msm8909_aux_dev = devm_kcalloc(&pdev->dev, card->num_aux_devs,
	...
	msm8909_codec_conf = devm_kcalloc(&pdev->dev, card->num_aux_devs,
			sizeof(struct snd_soc_codec_conf),
			GFP_KERNEL);
	if (!msm8909_codec_conf)
		return -ENOMEM;
	...
	for (i = 0; i < card->num_aux_devs; i++) {
		...
		msm8909_aux_dev[i].name = dev_name_str;
		msm8909_aux_dev[i].codec_name = NULL;
		msm8909_aux_dev[i].codec_of_node =
			wsa881x_dev_info[i].of_node;
		msm8909_aux_dev[i].init = msm8909_wsa881x_init;
		msm8909_codec_conf[i].dev_name = NULL;
		msm8909_codec_conf[i].name_prefix = wsa_auxdev_name_prefix[0];
		msm8909_codec_conf[i].of_node =
			wsa881x_dev_info[i].of_node;
	}
...
static const struct of_device_id msm8909_asoc_machine_of_match[]  = {
	{ .compatible = "qcom,msm8909-audio-i2s-codec",
	  .data = "tasha_codec"},
	{},
};
...
static int msm8909_asoc_machine_probe(struct platform_device *pdev)
{
	...
	struct msm8909_asoc_mach_data *pdata = NULL;
	...
	pdata = devm_kzalloc(&pdev->dev,
			sizeof(struct msm8909_asoc_mach_data), GFP_KERNEL);
	if (!pdata) {
		ret = -ENOMEM;
		goto err1;
	}
	...
	match = of_match_node(msm8909_asoc_machine_of_match,
			      pdev->dev.of_node);
	if (!match)
		goto err;
	...
	ret = msm8909_init_wsa_dev(pdev, card);
	if (ret)
		goto err;
...
module_init(msm8909_machine_init);
