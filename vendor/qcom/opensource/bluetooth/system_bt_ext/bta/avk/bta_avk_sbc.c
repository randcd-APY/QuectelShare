...
typedef int (tBTA_AVK_SBC_ACT)(void *p_src, void *p_dst,
...
typedef struct
{
    ...
    tBTA_AVK_SBC_ACT     *p_act;     /* the action function to do the conversion */
...
tBTA_AVK_SBC_UPS_CB bta_avk_sbc_ups_cb;
...
**
*******************************************************************************/
void bta_avk_sbc_init_up_sample (UINT32 src_sps, UINT32 dst_sps, UINT16 bits, UINT16 n_channels)
{
    bta_avk_sbc_ups_cb.cur_pos   = -1;
    bta_avk_sbc_ups_cb.src_sps   = src_sps;
    bta_avk_sbc_ups_cb.dst_sps   = dst_sps;
    bta_avk_sbc_ups_cb.bits      = bits;
    bta_avk_sbc_ups_cb.n_channels= n_channels;
    ...
    if(n_channels == 1)
    {
        ...
        if(bits == 8)
        {
            bta_avk_sbc_ups_cb.p_act = bta_avk_sbc_up_sample_8m;
            bta_avk_sbc_ups_cb.div   = 1;
        }
        else
        {
            bta_avk_sbc_ups_cb.p_act = bta_avk_sbc_up_sample_16m;
            bta_avk_sbc_ups_cb.div   = 2;
        }
    }
    else
    {
        ...
        if(bits == 8)
        {
            bta_avk_sbc_ups_cb.p_act = bta_avk_sbc_up_sample_8s;
            bta_avk_sbc_ups_cb.div   = 2;
        }
        else
        {
            bta_avk_sbc_ups_cb.p_act = bta_avk_sbc_up_sample_16s;
            bta_avk_sbc_ups_cb.div   = 4;
        }
    }
}
...
**                  When called with bta_avk_sbc_up_sample(src, uint8_array_dst..)
...
**
*******************************************************************************/
int bta_avk_sbc_up_sample (void *p_src, void *p_dst,
                         UINT32 src_samples, UINT32 dst_samples,
                         UINT32 *p_ret)
{
    UINT32 src;
    ...
    if(bta_avk_sbc_ups_cb.p_act)
    {
        src = src_samples/bta_avk_sbc_ups_cb.div;
        dst = dst_samples/bta_avk_sbc_ups_cb.div;
        return (*bta_avk_sbc_ups_cb.p_act)(p_src, p_dst, src, dst, p_ret);
    }
    else
    {
        *p_ret = 0;
...
**
*******************************************************************************/
int bta_avk_sbc_up_sample_16s (void *p_src, void *p_dst,
                         UINT32 src_samples, UINT32 dst_samples,
                         UINT32 *p_ret)
{
    INT16   *p_src_tmp = (INT16 *)p_src;
    INT16   *p_dst_tmp = (INT16 *)p_dst;
    INT16   *p_worker1 = &bta_avk_sbc_ups_cb.worker1;
    INT16   *p_worker2 = &bta_avk_sbc_ups_cb.worker2;
    UINT32  src_sps = bta_avk_sbc_ups_cb.src_sps;
    UINT32  dst_sps = bta_avk_sbc_ups_cb.dst_sps;

    while (bta_avk_sbc_ups_cb.cur_pos > 0 && dst_samples)
    {
        *p_dst_tmp++    = *p_worker1;
        *p_dst_tmp++    = *p_worker2;

        bta_avk_sbc_ups_cb.cur_pos -= src_sps;
        dst_samples--;
    }

    bta_avk_sbc_ups_cb.cur_pos = dst_sps;
    ...
    if (bta_avk_sbc_ups_cb.cur_pos == (INT32)dst_sps)
        bta_avk_sbc_ups_cb.cur_pos = 0;
...
**
*******************************************************************************/
int bta_avk_sbc_up_sample_16m (void *p_src, void *p_dst,
                              UINT32 src_samples, UINT32 dst_samples,
                              UINT32 *p_ret)
{
    INT16   *p_src_tmp = (INT16 *)p_src;
    INT16   *p_dst_tmp = (INT16 *)p_dst;
    INT16   *p_worker = &bta_avk_sbc_ups_cb.worker1;
    UINT32  src_sps = bta_avk_sbc_ups_cb.src_sps;
    UINT32  dst_sps = bta_avk_sbc_ups_cb.dst_sps;
...
**
*******************************************************************************/
int bta_avk_sbc_up_sample_8s (void *p_src, void *p_dst,
                             UINT32 src_samples, UINT32 dst_samples,
                             UINT32 *p_ret)
{
...
**
*******************************************************************************/
int bta_avk_sbc_up_sample_8m (void *p_src, void *p_dst,
                             UINT32 src_samples, UINT32 dst_samples,
                             UINT32 *p_ret)
{
    UINT8   *p_src_tmp = (UINT8 *)p_src;
...
**
*******************************************************************************/
UINT8 bta_avk_sbc_cfg_for_cap(UINT8 *p_peer, tA2D_SBC_CIE *p_cap, tA2D_SBC_CIE *p_pref)
{
    ...
    {
        APPL_TRACE_ERROR("bta_avk_sbc_cfg_for_cap: ch_mode(0x%02X) not supported", p_pref->ch_mode);
        return A2D_FAIL;
    }
    ...
    {
        APPL_TRACE_ERROR("bta_avk_sbc_cfg_for_cap: samp_freq(0x%02X) not supported", p_pref->samp_freq);
        return A2D_FAIL;
    }
    ...
    {
        APPL_TRACE_ERROR("bta_avk_sbc_cfg_for_cap: block_len(0x%02X) not supported", p_pref->block_len);
        return A2D_FAIL;
    }
    ...
    {
        APPL_TRACE_ERROR("bta_avk_sbc_cfg_for_cap: num_subbands(0x%02X) not supported", p_pref->num_subbands);
        return A2D_FAIL;
    }
    ...
    {
        APPL_TRACE_ERROR("bta_avk_sbc_cfg_for_cap: alloc_mthd(0x%02X) not supported", p_pref->alloc_mthd);
        return A2D_FAIL;
    }
...
/*******************************************************************************
**
** Function         bta_avk_sbc_cfg_matches_cap
**
...
**
*******************************************************************************/
UINT8 bta_avk_sbc_cfg_matches_cap(UINT8 *p_cfg, tA2D_SBC_CIE *p_cap)
{
    UINT8           status = 0;
    ...
    {
        APPL_TRACE_ERROR(" bta_avk_sbc_cfg_matches_cap Parsing Failed %d", status);
        return status;
    }
...
**
*******************************************************************************/
UINT8 bta_avk_sbc_cfg_in_cap(UINT8 *p_cfg, tA2D_SBC_CIE *p_cap)
{
    UINT8           status = 0;
    tA2D_SBC_CIE    cfg_cie;
...
**
*******************************************************************************/
void bta_avk_sbc_bld_hdr(BT_HDR *p_buf, UINT16 fr_per_pkt)
{
    UINT8   *p;

    p_buf->offset -= BTA_AVK_SBC_HDR_SIZE;
    p = (UINT8 *) (p_buf + 1) + p_buf->offset;
    p_buf->len += BTA_AVK_SBC_HDR_SIZE;
