...
)
{
  ...
  memset(proc_name,0,proc_name_size);
  /* Obtain the last entry if directory names are present */
  if (NULL == (ptr = strrchr(tmp_proc_name, '/')))
  {
    memcpy(proc_name, tmp_proc_name, (size_t)proc_name_size-1);
  }
  else
  {
    memcpy(proc_name, ptr+1, (size_t)proc_name_size-1);
  }
...
)
{
  ...
  QMI_DEBUG_MSG_2("linux_qmi_qmux_if_client_get_proc_group_name: gid=%d, grp_name=%s\n",
    ...
    memset(grp_name,0,grp_name_size);
    memcpy(grp_name, grp->gr_name, (size_t)grp_name_size-1);
