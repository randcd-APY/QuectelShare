...
#define BIND_MUX_DATA_PORT_RETRY_MAX      (10)
#define BIND_MUX_DATA_PORT_RETRY_SLEEP_US (1000000) /* Sleep in microseconds (1000 ms) */
#endif
...
)
{
  ...
  if (0 < mux_id)
  {
    ...
    /* Send bind_req */
#ifdef FEATURE_EXTERNAL_AP
    /* Retry mechanism for binding mux data port on external ap.
       since we do not know exactly when dpm port open finished on
       modem side for binding to work */
    for (retry_cnt = 0; retry_cnt < BIND_MUX_DATA_PORT_RETRY_MAX; retry_cnt++)
    {
      ...
      {
        usleep(BIND_MUX_DATA_PORT_RETRY_SLEEP_US);
        continue;
      }
      break;
    }
