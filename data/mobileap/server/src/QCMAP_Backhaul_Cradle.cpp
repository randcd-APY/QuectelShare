...
)
{
  ...
  *qmi_err_num = QMI_ERR_NO_EFFECT_V01;
  ...
  if((QCMAP_MSGR_CRADLE_LAN_BRIDGE_V01 != mode) || (QCMAP_MSGR_CRADLE_LAN_ROUTER_V01 != mode))
  {
    if(QcMapVLAN && (QcMapVLAN->GetVLANConfig(&vlan_config, &num_of_vlans, qmi_err_num)))
    {
      ...
      LOG_MSG_INFO1("Get VLAN Config succeeded.",0,0,0);
    } else {
  ...
  */
  ...
  switch (mode)
  {
    ...
    default:
      /* Unknown mode */
      LOG_MSG_ERROR("Cradle mode unknown %d",mode,0,0);
      return false;
    break;
  }
                                                   ...
                                                   ip_version_enum_type ip_vsn)
{
  ...
  if ( !QcMapBackhaulCradleMgr || !QcMapBackhaulMgr || !lan_obj )
  {
    LOG_MSG_ERROR("QcMapBackhaulCradleMgr: %p, QcMapBackhaulMgr: %p, lan_obj: %p",
                             QcMapBackhaulCradleMgr, QcMapBackhaulMgr, lan_obj);
    return;
  }
...
void QCMAP_Backhaul_Cradle::SwitchToCradleBackhaul()
{
  ...
  if (QcMapBackhaulCradleMgr->cradle_v4_available)
  {
    if (!QcMapBackhaulCradleMgr->IsCradleWANBridgeActivated() && lan_obj)
      lan_obj->AllowBackhaulAccessOnIface(IP_V4, ECM_IFACE);
    if(QcMapNatAlg != NULL)
      QcMapNatAlg->EnableNATonApps();
    else
    {
  ...
  {
    if (!QcMapBackhaulCradleMgr->IsCradleWANBridgeActivated() && lan_obj)
      lan_obj->AllowBackhaulAccessOnIface(IP_V6, ECM_IFACE);
