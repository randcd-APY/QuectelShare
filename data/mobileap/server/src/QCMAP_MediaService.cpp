...
#define MINIDLNA_CONFIG_PATH        "/etc/data/minidlna.conf"
#define MINIUPNPD_CONFIG_PATH       "/etc/data/miniupnpd/miniupnpd.conf"
...
boolean QCMAP_MediaService::EnableUPNP(qmi_error_type_v01 *qmi_err_num)
{
  ...
  if(!QcMapLANMgr)
  {
    LOG_MSG_INFO1("Default LAN object is NULL", 0, 0, 0);
  ...
  if(QcMapLANMgr->IsBridgeInited())
  {
    /* update the presentation page url to the current IP address */
    snprintf(command, MAX_COMMAND_STR_LEN,
               "sed \"/presentation_url=/d\" %s",MINIUPNPD_CONFIG_PATH);
    qcmap_edit_file(command, strlen(command), MINIUPNPD_CONFIG_PATH, strlen(MINIUPNPD_CONFIG_PATH));
    ...
    snprintf(command,sizeof(command), "%s%s:8201\" %s", "sed \"56 apresentation_url=/", inet_ntoa(addr), MINIUPNPD_CONFIG_PATH);
    qcmap_edit_file(command, strlen(command), MINIUPNPD_CONFIG_PATH, strlen(MINIUPNPD_CONFIG_PATH));
...
)
{
  ...
  snprintf(command,MAX_COMMAND_STR_LEN,"sed \"/media_dir=\\/.*/d\" %s",MINIDLNA_CONFIG_PATH);
  ret = qcmap_edit_file(command, strlen(command), MINIDLNA_CONFIG_PATH, strlen(MINIDLNA_CONFIG_PATH));
  ...
  while (token != NULL)
  {
    ...
    snprintf(command,MAX_COMMAND_STR_LEN, "%s%s\" %s","sed \"13 amedia_dir=",token,MINIDLNA_CONFIG_PATH);
    ret = qcmap_edit_file(command, strlen(command), MINIDLNA_CONFIG_PATH, strlen(MINIDLNA_CONFIG_PATH));
...
)
{
  ...
  file = fopen(MINIDLNA_CONFIG_PATH, "rt");
  if (file == NULL)
  {
    LOG_MSG_ERROR("GetDLNAMediaDir unable to mind %s!", MINIDLNA_CONFIG_PATH, 0, 0);
    *qmi_err_num = QMI_ERR_NO_EFFECT_V01;
    return false;
  }
...
)
{
  ...
  snprintf(command, MAX_COMMAND_STR_LEN,
               "sed \"/notify_interval=/d\" %s",MINIUPNPD_CONFIG_PATH);
  qcmap_edit_file(command, strlen(command), MINIUPNPD_CONFIG_PATH, strlen(MINIUPNPD_CONFIG_PATH));
  snprintf(command,MAX_COMMAND_STR_LEN,"%s%d\" %s","sed \"62 anotify_interval=",notify_int,MINIUPNPD_CONFIG_PATH);
  qcmap_edit_file(command, strlen(command), MINIUPNPD_CONFIG_PATH, strlen(MINIUPNPD_CONFIG_PATH));
  return true;
}
...
)
{
  FILE *file = NULL;
  char temp[MAX_SCAN_SIZE] = "";
  file = fopen(MINIUPNPD_CONFIG_PATH, "rt");
  if (file == NULL)
  {
    LOG_MSG_ERROR("GetUPNPNotifyInterval unable to find %s!", MINIUPNPD_CONFIG_PATH, 0, 0);
...
)
{
  ...
  snprintf(command, MAX_COMMAND_STR_LEN,"sed \"/notify_interval=/d\" %s",MINIDLNA_CONFIG_PATH);
  qcmap_edit_file(command, strlen(command), MINIDLNA_CONFIG_PATH, strlen(MINIDLNA_CONFIG_PATH));

  snprintf(command, MAX_COMMAND_STR_LEN, "%s%d\" %s", "sed \"50 anotify_interval=", notify_int, MINIDLNA_CONFIG_PATH);
  qcmap_edit_file(command, strlen(command), MINIDLNA_CONFIG_PATH, strlen(MINIDLNA_CONFIG_PATH));
  return true;
}
...
)
{
  ...
  if (file == NULL)
  {
    LOG_MSG_ERROR("GetUPNPNotifyInterval unable to find %s!", MINIDLNA_CONFIG_PATH, 0, 0);
    *qmi_err_num = QMI_ERR_INSUFFICIENT_RESOURCES_V01;
    return false;
  }
...
)
{
  ...
  if (file == NULL)
  {
    LOG_MSG_ERROR("Get DLNAPortNumber unable to find %s!", MINIDLNA_CONFIG_PATH, 0, 0);
    *qmi_err_num = QMI_ERR_INSUFFICIENT_RESOURCES_V01;
    return false;
  }
