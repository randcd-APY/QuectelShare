...
#define IP_NEIGH_FILE                       "/tmp/neigh.txt"
#define TMP_FDB_TABLE                       "/tmp/tmpfdbtable.txt"
#define FDB_TABLE                           "/tmp/fdbtable.txt"
...
)
{
  ...
  static boolean kpi_add_rndis_to_bridge = true;
  static boolean kpi_add_ecm_to_bridge = true;
  static boolean kpi_add_eth_to_bridge = true;
  ...
  while(NLMSG_OK(nlh, buflen))
  {
    ...
    switch(nlh->nlmsg_type)
    {
      ...
      case RTM_NEWNEIGH:
        ...
        ds_log_high(" NEWNEIGH Received on %s IFACE with IP family as %d and"
        ...
        /*--------------------------------------------------------------------------------
          Check if the new neigh event is received on the VLAN iface
          If it is
          1. Extract the phy_iface and the vlan_id info. Copy the phy iface name to
          devname
          2. Store the vlan_id
          -------------------------------------------------------------------------------*/

        if(qcmap_nl_get_phyifaceName_vlan_id_from_iface(dev_name,
                                                        phy_iface_name,
                                                        &vlan_id))
        {
          LOG_MSG_INFO1("Rcvd NEWNEIGH Event on VLAN iface:%s Phy_iface:%s vlan_id:%d",
                        dev_name,phy_iface_name,vlan_id);

          if((lan_mgr == NULL) || (lan_mgr->GetLANBridge(vlan_id) == NULL))
          {
            break;
          }

          strlcpy(dev_name,phy_iface_name,QCMAP_MAX_IFACE_NAME_SIZE_V01);
       ...
       if (((qcmap_nl_is_recv_on_dev(dev_name, QCMAP_NL_RNDIS_INTERFACE) ==
        ...
        snprintf(logmsg, LOG_MSG_LEN, "rm %s",FDB_TABLE);
        ds_system_call(logmsg, strlen(logmsg));

        snprintf(logmsg, LOG_MSG_LEN, "rm %s",TMP_FDB_TABLE);
        ds_system_call(logmsg, strlen(logmsg));
...
----------------------------------------------------------------------*/
      case RTM_DELNEIGH:
        ...
        if(qcmap_nl_get_phyifaceName_vlan_id_from_iface(dev_name,
                                                        ...
                                                        &vlan_id))
        {
          LOG_MSG_INFO1("Rcvd DELNEIGH Event on VLAN iface:%s Phy_iface:%s vlan_id:%d",
                        dev_name,phy_iface_name,vlan_id);
          strlcpy(dev_name,phy_iface_name,QCMAP_MAX_IFACE_NAME_SIZE_V01);
        }
        ...
        else if (qcmap_nl_is_recv_on_dev(dev_name, QCMAP_NL_ETH_INTERFACE) == QCMAP_NL_SUCCESS)
        {
      ...
      case RTM_NEWROUTE:
       ...
       case RTM_NEWLINK:
       {
        ...
        /* Only listen to rndis, eth and ecm interface */
        if (qcmap_nl_is_recv_on_dev(dev_name, ETH_IFACE) ==
                                                           QCMAP_UTIL_SUCCESS)
        {
          if (kpi_add_eth_to_bridge)
          {
            snprintf(logmsg, LOG_MSG_LEN, "ifconfig %s up",dev_name);
            ds_system_call(logmsg, strlen(logmsg));
            snprintf(logmsg, LOG_MSG_LEN, "brctl addif bridge0 %s",dev_name);
            ds_system_call(logmsg, strlen(logmsg));
            kpi_add_eth_to_bridge = false;
          }
        }
        else if (qcmap_nl_is_recv_on_dev(dev_name, QCMAP_NL_RNDIS_INTERFACE) ==
                                                           QCMAP_UTIL_SUCCESS)
        {
          if (kpi_add_rndis_to_bridge)
          {
            snprintf(logmsg, LOG_MSG_LEN, "ifconfig %s up",dev_name);
            ds_system_call(logmsg, strlen(logmsg));
            snprintf(logmsg, LOG_MSG_LEN, "brctl addif bridge0 %s",dev_name);
            ds_system_call(logmsg, strlen(logmsg));
            kpi_add_rndis_to_bridge = false;
          }
        }
        else if (qcmap_nl_is_recv_on_dev(dev_name, QCMAP_NL_ECM_INTERFACE) ==
                                                             QCMAP_UTIL_SUCCESS)
        {
          if (kpi_add_ecm_to_bridge)
          {
            snprintf(logmsg, LOG_MSG_LEN, "ifconfig %s up",dev_name);
            ds_system_call(logmsg, strlen(logmsg));
            snprintf(logmsg, LOG_MSG_LEN, "brctl addif bridge0 %s",dev_name);
            ds_system_call(logmsg, strlen(logmsg));
            kpi_add_ecm_to_bridge = false;
          }
        }
        else
        {
          LOG_MSG_INFO1("RTM_NEWLINK recvd on interface other than"
                        " RNDIS/ETH/ECM",0,0,0);
          break;
        }
...
DESCRIPTION
  ...
  This function
  - Uses mac address and vlan_id to find out the public ip address of the connect device.
...
void Getclientaddr(qcmap_nl_addr_t* nl_addr, int16_t vlan_id)
{
  ...
  snprintf(command, MAX_COMMAND_STR_LEN,"ip neighbour show |grep %s |grep %s > %s",
      mac_addr_char, GET_ASSOCIATED_BRIDGE_IFACE_NAME(vlan_id), IP_NEIGH_FILE);
