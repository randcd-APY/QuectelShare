...
#define DHCPCD_PID_PATH   "/var/run/dhcpcd"
#define DHCPCD_LEASE_PATH "var/run/db/dhcpcd"

#define IS_WLAN_MODE_STA(wifi_mode) \
                             (wifi_mode == QCMAP_MSGR_WLAN_MODE_AP_STA_V01 ||   \
                              wifi_mode == QCMAP_MSGR_WLAN_MODE_STA_ONLY_V01 ||  \
                              wifi_mode == QCMAP_MSGR_WLAN_MODE_AP_AP_STA_V01 ||  \
                              wifi_mode == QCMAP_MSGR_WLAN_MODE_STA_P2P_V01)
...
)
{
  ...
  {
    LOG_MSG_ERROR(" Failed to create WLAN Proxy Cleanup timer",0,0,0);
  ...
  if (xml_path)
  {
    ...
    if (ReadAPSTAConfigFromXML())
    {
      ...
      {
        LOG_MSG_ERROR(" Failed to create WLAN STA ASSOC timer",0,0,0);
      ...
      {
        LOG_MSG_ERROR(" Failed to create WLAN STA DHCP timer",0,0,0);
        perror("Failed to create WLAN STA DHCP timer");
      }
      this->apsta_cfg.ap_sta_bridge_sta_dhcp_running=false;
      if(QcMapMgr)
        snprintf(this->apsta_cfg.sta_interface, QCMAP_MSGR_INTF_LEN, "wlan%d", QcMapMgr->sta_iface_index);
      return;
    }
  /* Read Config from XML failed. Use defaults. */
    LOG_MSG_ERROR("QCMAP Backhaul APSTA Read XML failed.\n",0,0,0);
  }
...
QCMAP_Backhaul_WLAN::~QCMAP_Backhaul_WLAN()
{
  ...
  /* Destroy Proxy Cleanup timer */
  if (timer_delete(this->apsta_cfg.timerid_proxy_cleanup) == QCMAP_CM_ERROR)
  {
    LOG_MSG_ERROR("delete Proxy Cleanup timer FAILED", 0, 0, 0);
  }
  if (pTimerProxy)
    free(pTimerProxy);
  pTimerProxy = NULL;

  /* Destroy STA ASSOC timer */
  if (timer_delete(this->apsta_cfg.timerid_sta_assoc) == QCMAP_CM_ERROR)
  {
    LOG_MSG_ERROR("delete STA ASSOC timer FAILED", 0, 0, 0);
  }
  if (pTimerSTA)
    free(pTimerSTA);
  pTimerSTA = NULL;
  ...
  /* Destroy STA ASSOC timer */
  if (timer_delete(this->apsta_cfg.timerid_sta_dhcp) == QCMAP_CM_ERROR)
  {
    LOG_MSG_ERROR("delete STA DHCP timer FAILED", 0, 0, 0);
  }
  if (pTimerSTA_DHCP)
    free(pTimerSTA_DHCP);
  pTimerSTA_DHCP = NULL;
  ...
  if(QcMapWLANMgr)
  {
    ...
    else
    {
      if (QcMapMgr && !QcMapMgr->dual_wifi_mode)
      {
        ...
        /* Run wpa_cli */
        snprintf(command, MAX_COMMAND_STR_LEN,
                 "wpa_cli -i %s -p %s -B -a %s",
                 QcMapBackhaulWLANMgr->apsta_cfg.sta_interface,
                 WPA_CTRL_PATH, STA_INTERFACE_CTRL_PATH);
        ds_system_call(command, strlen(command));
...
)
{
  ...
  if(QcMapWLANMgr)
  {
    if(QcMapWLANMgr->wifi_mode == QCMAP_MSGR_WLAN_MODE_STA_P2P_V01)
    {
      LOG_MSG_ERROR("\n Activate supplicant configuration is not supported for STA-P2P mode.\n", 0, 0, 0);
      *qmi_err_num = QMI_ERR_NOT_SUPPORTED_V01;
      return false;
    }
    if ( (QcMapWLANMgr->wifi_mode != QCMAP_MSGR_WLAN_MODE_AP_STA_V01 &&
    ...
    if (QcMapMgr && !QcMapMgr->dual_wifi_mode)
    {
      ...
      /* Run wpa_cli */
      snprintf( command, MAX_COMMAND_STR_LEN, "wpa_cli -i %s -p %s -B -a %s",QcMapBackhaulWLANMgr->apsta_cfg.sta_interface,
              WPA_CTRL_PATH, STA_INTERFACE_CTRL_PATH);
      ds_system_call(command, strlen(command));
    }
    else
    ...
    {
      if(lan_obj && (lan_obj->ActivateLAN(&qmi_err, true) == false)) //forcefully reactivate LAN
      {
...
boolean QCMAP_Backhaul_WLAN::EnableStaMode()
{
  ...
  if ( !IS_WLAN_MODE_STA(QcMapWLANMgr->wifi_mode) ||
       (QcMapMgr && QcMapMgr->sta_iface_index == QCMAP_LAN_INVALID_IFACE_INDEX) )
  {
  ...
  if (QcMapMgr && !QcMapMgr->dual_wifi_mode)
  {
    ...
    /* Run wpa_cli */
    snprintf( command, MAX_COMMAND_STR_LEN, "wpa_cli -i %s -p %s -B -a %s",this->apsta_cfg.sta_interface,
              WPA_CTRL_PATH, STA_INTERFACE_CTRL_PATH);
    ds_system_call(command, strlen(command));
...
boolean QCMAP_Backhaul_WLAN::DisableStaMode()
{
  ...
  if (QcMapWLANMgr && (!IS_WLAN_MODE_STA(QcMapWLANMgr->wifi_mode)) ||
       (QcMapMgr && QcMapMgr->sta_iface_index == QCMAP_LAN_INVALID_IFACE_INDEX) )
  {
    LOG_MSG_ERROR("\n WLAN is in incorrect Mode.\n",0,0,0);
    return false;
  ...
  if (QcMapWLANMgr && QcMapMgr && !QcMapMgr->dual_wifi_mode)
  {
    this->DisconnectSTA();
    QcMapWLANMgr->active_if_count--;
  }
...
void QCMAP_Backhaul_WLAN::ProcessAddrAssign(void *cb_user_data, char *devname, ip_version_enum_type ip_vsn)
{
  ...
  if (ip_vsn == IP_V4)
  {
    ...
    if ( true == ret )
    {
          ...
          QCMAP_Backhaul_Cradle::IsCradleWANBridgeActivated() == false)
      {
        if (backhaul_switch_required)
        {
          ...
          lan_obj->AllowBackhaulAccessOnIface(IP_V4, QcMapBackhaulWLANMgr->apsta_cfg.sta_interface);
          QCMAP_Backhaul::current_backhaul = BACKHAUL_TYPE_AP_STA_ROUTER;
    ...
    {
      LOG_MSG_ERROR("Error: GetStaIP Failed. Bailing out.\n",0,0,0);
      return;
    }
  }
  else if (ip_vsn == IP_V6)
  {
        ...
        (QCMAP_Backhaul_Cradle::IsCradleWANBridgeActivated() == false))
    {
      ...
      lan_obj->AllowBackhaulAccessOnIface(IP_V6, QcMapBackhaulWLANMgr->apsta_cfg.sta_interface);
      QCMAP_Backhaul::current_backhaul = BACKHAUL_TYPE_AP_STA_ROUTER;
      QcMapBackhaulWLANMgr->sta_packet_stat_bh_switch_ind_send = false;
...
static void QCMAP_Backhaul_WLAN::ProcessIPv6AddrAssign(void *cb_user_data, qcmap_nl_sock_msg_t *nlmsg)
{
  ...
  {
    LOG_MSG_ERROR("getifaddrs failed", 0,0,0);
    return;
  }
...
void QCMAP_Backhaul_WLAN::ProcessStaDHCPIPFail(void *cb_user_data)
{
     ...
     (this->apsta_cfg.static_ip_config.gw_ip == 0))
  {
    ...
    if(lan_obj && (lan_obj->ActivateLAN(&qmi_err_num, true) == false)) // Forcefully reactivate LAN
    {
...
void QCMAP_Backhaul_WLAN::ProcessStaAssocForWLANBridge(void *cb_user_data)
{
  ...
  if ( this->apsta_cfg.conn_type ==QCMAP_MSGR_STA_CONNECTION_DYNAMIC_V01 )
  {
    ...
    /* Clean the PIDs */
    snprintf( command, MAX_COMMAND_STR_LEN, "rm -rf %s-%s.pid",DHCPCD_PID_PATH,this->apsta_cfg.sta_interface);
    ds_system_call(command, strlen(command));
    snprintf( command, MAX_COMMAND_STR_LEN, "rm -rf %s-%s.pid",DHCPCD_PID_PATH,BRIDGE_IFACE);
    ds_system_call(command, strlen(command));
    /* Clean the leases */
    snprintf( command, MAX_COMMAND_STR_LEN, "rm -rf %s-%s.lease",DHCPCD_LEASE_PATH,this->apsta_cfg.sta_interface);
    ds_system_call(command, strlen(command));
    snprintf( command, MAX_COMMAND_STR_LEN, "rm -rf %s-%s.lease",DHCPCD_LEASE_PATH,BRIDGE_IFACE);
    ds_system_call(command, strlen(command));
...
void QCMAP_Backhaul_WLAN::SwitchToAPSTABackhaul()
{
  ...
  if(QcMapMgr == NULL || QcMapBackhaulMgr == NULL || QcMapBackhaulWLANMgr == NULL)
  {
    LOG_MSG_ERROR("OOPS!!! QcMapMgr=%p QcMapBackhaulMgr=%p, QcMapBackhaulWLANMgr=%p",
                    QcMapMgr, QcMapBackhaulMgr, QcMapBackhaulWLANMgr);
    return;
  }
  if(QcMapFirewall == NULL || QcMapNatAlg == NULL || lan_obj == NULL)
  {
    LOG_MSG_ERROR("OOPS!!! QcMapFirewall=%p QcMapNatAlg=%p, lan_obj=%p", QcMapFirewall, QcMapNatAlg, lan_obj);
    return;
  }
