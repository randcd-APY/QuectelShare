...
#define QCMAP_DEFAULT_DUN_DONGLE_MODE 0

#ifdef FEATURE_DUN_SOFTAP_LEGACY_MODE_TTY_DEVICE
#define USB_DUN_TTY_PORT "/dev/ttyGS1"
#else
#define USB_DUN_TTY_PORT "/dev/ttyGS0"
#endif
...
class QCMAP_Tethering
{
  ...
  public:
    ...
    /*set and get Dun Dongle Mode from XML*/
    static boolean SetGetDunDongleModeFromXML(qcmap_action_type action, char *data, int data_len);

    /*set Dun Dongle Mode flag*/
    static boolean SetDunDongleMode(boolean dun_dongle_mode_state,
                                        qmi_error_type_v01 *qmi_err_num);

    /*get Dun Dongle Mode flag*/
    static boolean GetDunDongleMode(boolean *dun_dongle_mode_status,
                                        qmi_error_type_v01 *qmi_err_num);

    /*Bring down PPPD if backhaul down*/
    static boolean BringDownPPPD();
};
#endif
