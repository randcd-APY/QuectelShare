...
-------------------------------------------------------------------------*/
  ...
  {
     LOG_MSG_ERROR("QTI:Netlink Query to Kernel failed errno:%d",errno,0,0);
     snprintf((char *)logmsg,LOG_MSG_LEN,"echo QTI:Netlink Query to Kernel Failed %d > /dev/kmsg",errno);
     ds_system_call((char *)logmsg,strlen(logmsg));
     return QCMAP_UTIL_FAILURE;
  }

  snprintf((char *)logmsg,LOG_MSG_LEN,"echo QTI:Netlink Query to Kernel Success > /dev/kmsg");
  ds_system_call((char *)logmsg,strlen(logmsg));
...
-----------------------------------------------------------------------------*/
  link_info->metainfo = NLMSG_DATA(nlh);
...
/*!
@brief
  Gets physical iface name, vlan ID from iface name.
...
/*------------------------------------------------------------------------*/
  ...
  if((iface_name == NULL) || (phy_iface_name == NULL) || (vlan_id == NULL))
   {
      LOG_MSG_ERROR("Invalid input params iface_name:%p phy_iface_name:%p vlan_id:%p",
                    iface_name,phy_iface_name,vlan_id);
      return false;
   }
  ...
  do
  {
    ...
    *vlan_id = 0;
    ...
    LOG_MSG_INFO1("base iface_name:%s",iface_name,0,0);
    ptr = strchr(iface_name,'.');

    if (ptr == NULL)
    {
      if(strncmp(iface_name,"bridge",BRIDGE_LEN) == 0)
      {
        ptr = iface_name + BRIDGE_LEN;
        strlcpy(vlan_id_text,ptr,QCMAP_MAX_IFACE_NAME_SIZE_V01);
        *vlan_id = atoi(vlan_id_text);
        strlcpy(phy_iface_name,BRG_IFACE,strlen(BRG_IFACE)+1);
      }
  ...
  LOG_MSG_INFO1("vlan_id returned is %d:ret_val:%d",*vlan_id,ret_val,0);
  return ret_val;
}
