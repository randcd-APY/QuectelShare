...
#ifdef FEATURE_EXTERNAL_AP
  ds_get_epid(dev_name, &ep_type, &epid);
  netmgr_log_low("%s(): Got EPID %d for device %s on external ap\n", __func__, epid, dev_name);
  netmgr_main_cfg.epid = epid;
  netmgr_main_cfg.ep_type = (uint32_t) ep_type;
#else
  if (netmgr_kif_ifioctl_get_epid(dev_name, &epid) != NETMGR_SUCCESS)
  {
    netmgr_log_err("%s(): Error getting the EPID information for dev %s\n", __func__, dev_name);
...
#if !defined (FEATURE_EXTERNAL_AP) && defined (FEATURE_DATA_LINUX_LE)
  if(chown(server_socket.sun_path, -1, AID_RADIO) != 0)
  {
    netmgr_log_err("%s(): unable to own the unix server socket %s - %s",
            __func__,
