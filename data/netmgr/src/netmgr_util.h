...
extern boolean log_to_syslog;
/* Disable Logging on External AP until we can reduce logs going to syslog */
#define EX_AP_LOGGING 0
...
#ifdef FEATURE_EXTERNAL_AP
#define  netmgr_log_err(fmt, args...)                                                      \
  do {                                                                                     \
    if (log_to_syslog && EX_AP_LOGGING)                                                    \
    {                                                                                      \
      syslog(LOG_NOTICE, "%s %40s %40s():%3u (ERR) " fmt,                                  \
             LOG_TAG, __FILE__, __func__, __LINE__, ##args);                               \
    }                                                                                      \
  } while(0);
#else
...
#ifdef FEATURE_EXTERNAL_AP
#define  netmgr_log_high(fmt, args...)                                                     \
  do {                                                                                     \
    if (log_to_syslog && EX_AP_LOGGING)                                                    \
    {                                                                                      \
      syslog(LOG_NOTICE, "%s %40s %40s():%3u (HIGH) " fmt,                                 \
             LOG_TAG, __FILE__, __func__, __LINE__, ##args);                               \
    }                                                                                      \
  } while(0);
#else
...
#ifdef FEATURE_EXTERNAL_AP
#define  netmgr_log_med(fmt, args...)                                                      \
  do {                                                                                     \
    if (log_to_syslog && EX_AP_LOGGING)                                                    \
    {                                                                                      \
      syslog(LOG_NOTICE, "%s %40s %40s():%3u (MED) " fmt,                                  \
             LOG_TAG, __FILE__, __func__, __LINE__, ##args);                               \
    }                                                                                      \
  } while(0);
#else
#define  netmgr_log_med(...)                                                               \
  do {                                                                                     \
    ...
    {                                                                                      \
      char timestamp[MAX_TIMESTAMP_LEN];                                                   \
      TIMESTAMP(timestamp);                                                                \
      fprintf(logging_fd,"%25s %30s %40s():%3u (MED):",                                    \
              timestamp, __FILE__, __func__, __LINE__);                                    \
      fprintf(logging_fd,__VA_ARGS__);                                                     \
      fprintf(logging_fd,"\n");                                                            \
      fflush(logging_fd);                                                                  \
...
#ifdef FEATURE_EXTERNAL_AP
#define  netmgr_log_low(fmt, args...)                                                      \
  do {                                                                                     \
    if (log_to_syslog && EX_AP_LOGGING)                                                    \
    {                                                                                      \
      syslog(LOG_NOTICE, "%s %40s %40s():%3u (LOW) " fmt,                                  \
             LOG_TAG, __FILE__, __func__, __LINE__, ##args);                               \
    }                                                                                      \
  } while(0);
#else
...
#ifdef FEATURE_EXTERNAL_AP
#define  netmgr_log_sys_err(fmt, args...)                                                  \
  do {                                                                                     \
    if (log_to_syslog && EX_AP_LOGGING)                                                    \
    {                                                                                      \
      syslog(LOG_NOTICE, "%s %40s %40s():%3u (SYS ERR) " fmt,                              \
             LOG_TAG, __FILE__, __func__, __LINE__, ##args);                               \
    }                                                                                      \
  } while(0);
#else
