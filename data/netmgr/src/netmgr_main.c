...
#if ((!defined(NETMGR_OFFTARGET)) && (!defined(FEATURE_DATA_TARGET_MCTM) || !defined(FEATURE_DATA_TARGET_FUSION_8096_9X55)))
...
#if (!defined(FEATURE_DATA_CV2X) && !defined(FEATURE_EXTERNAL_AP))
  {0,}                          /* cmd_exec options */
#else
  {0,},                         /* cmd_exec options */
  0                             /* Max congigurable MTU */
#endif /* (!defined(FEATURE_DATA_CV2X) && !defined(FEATURE_EXTERNAL_AP)) */
...
LOCAL void netmgr_main_process_target()
{
  ...
  if(NETMGR_SUCCESS != netmgr_config_load_ex_ap(target_str))
  {
    netmgr_log_err("%s: unable to load config for external AP target:%s",
                   __func__, target_str);
    /* Abort can misbehave sometimes; print the error twice */
    NETMGR_ABORT("%s: unable to load config for external AP target:%s",
                 __func__, target_str);
  }
