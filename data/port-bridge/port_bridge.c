...
#define PCIE_FILE_PATH        ("/dev/mhi_pipe_32")
...
#define HOST_PORT_CONFIG  "/etc/data/port_bridge/port_bridge.config"
...
static boolean getHostPort(void)
{
        FILE *cmd = NULL;
        char mode =0;
        char buff[MAX_COMMAND_STR_LEN];
        snprintf(buff,MAX_COMMAND_STR_LEN, "cat %s", HOST_PORT_CONFIG);
        ...
        /*Mode 3 will be used for communication through PCIE,otherwise communication will be through USB*/
        if( mode == '3')
        {
                return TRUE;
        }
...
static int pb_resolve_target(struct pb_targetinfo *targetinfo)
{
  ...
  switch (target)
  {
    ...
    case DS_TARGET_LE_STINGRAY:
      ...
      rc = 0;
      break;
    case DS_TARGET_LE_CHIRON:
      if(getHostPort())
      {
       LOGI("IN PCIE PATH");
       targetinfo->mdm_chardev = MDM_FILE_PATH_GLINK;
       targetinfo->usb_chardev = PCIE_FILE_PATH;
       targetinfo->mdm_handler = modem_to_usb_bridge_glink;
       rc=0;
       break;
      }
