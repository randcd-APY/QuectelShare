...
const char* dsi_device_control_port_names [] =
{
  QMI_PORT_RMNET_DATA_0,
  QMI_PORT_RMNET_DATA_1,
  QMI_PORT_RMNET_DATA_2,
...
/*===========================================================================
  FUNCTION:  dsi_config_load_ex_ap
===========================================================================*/
int dsi_config_load_ex_ap
(
  const char *target
)
{
  int i;
  DSI_LOG_INFO("DSI: Configuring for target %s\n", target);


  dsi_config.rmnet_data_enable = TRUE;
  dsi_config.qos_enable = FALSE;
  strlcpy(dsi_config.phys_net_dev, "rmnet_mhi0", sizeof(dsi_config.phys_net_dev));
  dsi_config.single_qmux_ch_enabled = TRUE;
  strlcpy(dsi_config.single_qmux_ch_name, "rmnet_mhi0", sizeof(dsi_config.single_qmux_ch_name));

  dsi_config.num_dsi_handles = 8;
  ...
  for(i=0; i < dsi_config.num_dsi_handles; ++i)
  {
     ...
     strlcpy(dsi_device_names[i], dsi_device_control_port_names[i], DSI_DEV_STR_MAX_LEN);
  }

  for(i=0; i < dsi_config.num_dsi_handles; ++i)
  {
     dsi_qmi_port_names[i] = get_port_name(dsi_device_control_port_names[i]);
  }
  dsi_config_print();
