...
----------------------------------------------------------------------------*/
        ...
        (qcmap_enable_resp_msg_v01.mobile_ap_handle_valid != TRUE))
    {
        ...
        usleep(QTI_500MSEC_SLEEP);
        retry_count++;
/*---------------------------------------------------------------------------
  If disable mobileap is in progress, retry enable mobileap after 0.5 sec
----------------------------------------------------------------------------*/

      if (qcmap_enable_resp_msg_v01.resp.error == QMI_ERR_INVALID_OPERATION_V01)
      {
        LOG_MSG_INFO1("Mobile ap disable in progress, Enable mobileap retry in progress", 0, 0, 0);
        continue;
      }
...
---------------------------------------------------------------------------*/
  ...
  qcmap_qti_tethered_link_up_req_msg.usb_link = iface;
...
/*--------------------------------------------------------------------------*/
  ...
  snprintf(command, QTI_MAX_COMMAND_STR_LEN, "echo QTI:TETHERED link down > /dev/kmsg");
  ds_system_call(command, strlen(command));
...
---------------------------------------------------------------------------*/
  ...
  qcmap_qti_tethered_link_down_req_msg.usb_link = iface;
...
---------------------------------------------------------------------------*/
      ...
      LOG_MSG_INFO1("Got TETHERED link up event on interface %d",iface, 0, 0);
...
---------------------------------------------------------------------------*/
      ret_val = qti_tethered_link_up(iface);
      if (ret_val==QTI_SUCCESS)
      {
        LOG_MSG_INFO1(" Setup USB tethering: successful. Interface = %d",iface, 0, 0);
      }
      else
      {
        LOG_MSG_ERROR(" Setup USB tethering: unsuccessful. Interface = %d",iface, 0, 0);
        return QTI_FAILURE;
      }
/*--------------------------------------------------------------------------
...
---------------------------------------------------------------------------*/
      ret_val = qti_tethered_link_down(iface);
      if (ret_val==QTI_SUCCESS)
      {
        LOG_MSG_INFO1(" Bring down USB tethering: successful. Interface = %d",iface, 0, 0);
      }
      else
      {
        LOG_MSG_ERROR(" Bring down USB tethering: unsuccessful. Interface = %d",iface, 0, 0);
        return QTI_FAILURE;
      }
...
---------------------------------------------------------------------------*/
    ...
    case QTI_QCMAP_INIT_EVENT:
    {
      ...
      if(ret_val != QTI_SUCCESS)
      {
        LOG_MSG_ERROR("Failed to initialize QTI as a QCMAP client",0, 0, 0);
        return QTI_FAILURE;
      }
      qcmap_init = true;
/*-------------------------------------------------------------------------
...
--------------------------------------------------------------------------*/
      if( qcmap_nl_query_if(&sk_info, RTMGRP_LINK) == QTI_SUCCESS)
      {
        LOG_MSG_INFO1("RTM_GETLINK: Netlink Query succeeds",0,0,0);
      }
      else
      {
        LOG_MSG_ERROR("RTM_GETLINK: Netlink Query failed",0,0,0);
      }
      break;
    }
