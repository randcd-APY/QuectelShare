...
-----------------------------------------------------------------------------*/
  ...
  LOG_MSG_INFO1("Sending to modem SMD contol channel %s",
                 qti_dpm_port_open_req_msg.control_port_list[0].port_name, 0, 0);

  /*Dont send Data Port info it is MBIM mode, as QBI sends it*/
  if(!is_mbim_mode)
  {
