...
)
{
  ...
  else
  {
    ...
    PRINT_QMI_MSG((unsigned char *) qmi_qmux_if_buf.data + QMI_QMUX_IF_MSG_HDR_SIZE - QMI_QMUX_HDR_SIZE,
                 (int)actual_data_size + QMI_QMUX_HDR_SIZE);
...
/*-----------------------------------------------------------------------*/
  ...
  HEXDUMP_LOG_MSG ("Hex dumps received from %s peripheral :",
                  (rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_enabled)?"USB" : "MHI");
  HEXDUMP_QXDM_MSG("Hex dumps received from %s peripheral",
                  (rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_enabled)?"USB" : "MHI", 0, 0);
      ...
      (DS_TARGET_LE_MDM9x06 == rmnet_state_config->target))
  {
    ...
    HEXDUMP_LOG_MSG("Hex dumps received from %s peripheral after EP translation:", 
                   (rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_enabled)?"USB" : "MHI");

    HEXDUMP_QXDM_MSG("Hex dumps received from %s peripheral after EP translation",
                    (rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_enabled)?"USB" : "MHI", 0, 0);
    qti_hex_dumps_logging(data, len);
  }
  LOG_MSG_INFO1("Will send QMI message of size %d to modem", len, 0 , 0);
...
void qti_rmnet_modem_not_in_service()
{
  ...
  if( (DS_TARGET_MSM8994 == rmnet_state_config->target) ||
      ...
      (DS_TARGET_LE_CHIRON == rmnet_state_config->target))
  {
    if (rmnet_state_config->modem_iface_fd)
    {
      ret_val = qti_rmnet_modem_reset();
...
void qti_rmnet_modem_in_service()
{
      ...
      (DS_TARGET_LE_CHIRON == rmnet_state_config->target))
  {
    ...
    if (line_state == 0)
    {
      LOG_MSG_INFO1("USB/MHI cable not connected. So dont initialize modem components",
                    0, 0, 0);
      goto bail;
    }
    else if (line_state == 1)
