...
----------------------------------------------------------------------------*/
            ...
            if (ret_val != QTI_SUCCESS)
            {
              LOG_MSG_ERROR("Failed to set modem out of service state on peripheral driver file", 0, 0, 0);
            }

            ret_val = qti_rmnet_ph_set_modem_state(MODEM_IN_SERVICE);
            if (ret_val != QTI_SUCCESS)
            {
              LOG_MSG_ERROR("Failed to set modem in service state on peripheral driver file", 0, 0, 0);
            }
          }
        ...
        {
          LOG_MSG_INFO1("will be handled later, DTR status:%d & is_ssr_in_progress:%d ",
...
----------------------------------------------------------------------------*/
  ...
  {
    line_state = qti_rmnet_ph_get_mhi_state();
...
/*----------------------------------------------------------------------------------
  Reset MHI channels. This is needed because the next plugin may be from USB or MHI.
-----------------------------------------------------------------------------------*/
          qti_ph_reset(PH_DRIVER_TYPE_MHI);
        }
      }
...
)
{
 ...
 /*-------------------------------------------------------------------------*/
  ...
  LOG_MSG_INFO1("Read %d data from peripheral device file:%s",
                ret, (rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_enabled)?"USB" : "MHI" , 0);
  if (ret < 0)
  {
...
)
{
  ...
  if (rmnet_state_config->target != DS_TARGET_LYKAN)
  {
    ...
    else
    {
      ...
      {
        rmnet_state_config->ph_iface[PH_DRIVER_TYPE_MHI].ph_enabled = TRUE;
        LOG_MSG_INFO1("Enabling MHI peripheral as MHI state is connected",0,0,0);
      }
      qti_rmnet_process_ph_reset();
    }
  }
...
FUNCTION QTI_RMNET_PH_SET_MHI_STATE()
...
int qti_rmnet_ph_set_mhi_state(int mhi_state)
{
  if (rmnet_state_config == NULL)
  {
    LOG_MSG_ERROR("RmNet state config not set", 0, 0, 0);
  ...
  rmnet_state_config->mhi_connected = mhi_state;
...
FUNCTION QTI_RMNET_PH_GET_MHI_STATE()
...
int qti_rmnet_ph_get_mhi_state()
{
  return rmnet_state_config->mhi_connected;
}
...
)
{
  ...
  if ( rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_enabled )
  {
    LOG_MSG_ERROR("USB tethering already enabled. MHI peripheral concurrency not supported", 0, 0, 0);
    return QTI_FAILURE;
  }
/*-------------------------------------------------------------------------*/
...
---------------------------------------------------------------------------*/
  ...
  if(strcmp(buf, MHI_DEV_CONNECTED) == 0)
  {
    ...
    qti_notify_adpl(MHI_CONNECTED);

    if(qti_rmnet_ph_set_mhi_state(MHI_CONNECTED) != QTI_SUCCESS)
    {
  ...
  else if(strcmp(buf, MHI_DEV_DISCONNECTED) == 0)
  {
    ds_system_call("echo QTI:Processing MHI LINK_DOWN > /dev/kmsg",
                   strlen("echo QTI:Processing MHI LINK_DOWN > /dev/kmsg"));
    qti_notify_adpl(MHI_DISCONNECTED);

    if(qti_rmnet_ph_set_mhi_state(MHI_DISCONNECTED) != QTI_SUCCESS)
    {
      LOG_MSG_ERROR("Setting MHI state %d failed",MHI_DISCONNECTED,0,0);
    }
    LOG_MSG_INFO1("Got link down event on MHI interface",0, 0, 0);
    ret_val = qti_rmnet_process_ph_reset();
    if (ret_val != QTI_SUCCESS)
    {
      LOG_MSG_ERROR("Failed to process peripheral reset", 0, 0, 0);
...
FUNCTION MHI_CTRL_NODE_MONITOR()

DESCRIPTION

  This function
  - Monitors the /dev/ folder for mhi ctrl file node creation
  - If the notification is received that the node is created, then MHI ctrl
    node is processed to read its contents
...
)
{
  ...
  LOG_MSG_INFO1("Received notifications in dir %s", MHI_CTRL_DIR_NAME, 0, 0);

  struct inotify_event* event = NULL;
  length = read(inotify_fd, buffer, INOTIFY_BUF_LEN);
  LOG_MSG_INFO1("Read following event bytes: %d", length, 0, 0);
  ...
  if (event->len > 0)
  {
    if(event->mask & IN_CREATE)
    {
       ...
       {
          LOG_MSG_INFO1("The directory %s was 0x%x\n", event->name, event->mask, 0);
          return QTI_SUCCESS;
       }
       else if(!strncmp(event->name, MHI_CTRL_FILE_NAME, event->len))
       {
         LOG_MSG_INFO1("File \"%s\" created notification with mask 0x%x\n", event->name, event->mask, 0);
         ret_val = qti_file_open(rmnet_state_config->mhi_dev_ctrl_file_name,
                                 &(rmnet_state_config->mhi_dev_ctrl_file_fd),
                                 MHI_CTRL_FILE_NODE_OPEN_MAX_RETRY);
         ...
         {
           LOG_MSG_INFO1("Opened MHI file's fd is %d",rmnet_state_config->mhi_dev_ctrl_file_fd, 0, 0);
           ret_val = qti_rmnet_mhi_ctrl_recv_msg(rmnet_state_config->mhi_dev_ctrl_file_fd);
           if( ret_val == QCMAP_UTIL_SUCCESS)
           {
              LOG_MSG_INFO1("Successfully read the mhi ctrl file node on inotify event", 0, 0, 0);
           }
           else
           {
              LOG_MSG_ERROR("Failed to read the mhi ctrl file node on inotify event", 0, 0, 0);
           }
           ...
           if( qcmap_nl_addfd_map(rmnet_state_config->fd_set,
                                  ...
                                  MAX_NUM_OF_FD) != QCMAP_UTIL_SUCCESS)
           {
             LOG_MSG_ERROR("Failed to map MHI ctrl fd with the read function",0,0,0);
             close(rmnet_state_config->mhi_dev_ctrl_file_fd);
             rmnet_state_config->mhi_dev_ctrl_file_fd = 0;
           }
           else
           {
             LOG_MSG_INFO1("Successfully mapped the recv msg function to ctrl node", 0, 0, 0);
             /*Clear inotify_fd once mhi_ctrl opened successfully*/
             if (qti_clear_fd(rmnet_state_config->fd_set, inotify_fd) != QTI_SUCCESS)
             {
                LOG_MSG_ERROR("Failed to clear fd from fd set", 0, 0, 0);
             }
             ret_val = inotify_rm_watch(inotify_fd, wd);
             if(ret_val < 0)
                LOG_MSG_ERROR("Failed to remove the inotify watch", 0, 0, 0);
             close(inotify_fd);
             inotify_fd = 0;
           }
         }
       ...
       {
         LOG_MSG_INFO1("Event for another file created and not the CTRL file", 0, 0, 0);
       }
       LOG_MSG_INFO1("Received monitored IN_CREATE event %s.\n", event->name, 0, 0);
     }
   }
   free(event);
...
/*-------------------------------------------------------------------------*/
  ...
  if(ret_val == QTI_FAILURE)
  {
    LOG_MSG_ERROR("Failed to open MHI ctrl device file. Error %d",
                   errno, 0, 0);
    /*Open inotify for the first time only or after closing of mhi ctrl*/
    if (inotify_fd == 0)
    {
      inotify_fd = inotify_init();
      if (inotify_fd < 0)
      {
         LOG_MSG_ERROR("Error in inotify_init", 0, 0, 0);
      }
      else
      {
         wd = inotify_add_watch(inotify_fd, MHI_CTRL_DIR_NAME, mask);
         if(wd > 0)
         {
            LOG_MSG_INFO1("Successfully got the watch descriptor", 0, 0, 0);
            if( qcmap_nl_addfd_map(fd_set,
                                   inotify_fd,
                                   mhi_ctrl_node_monitor,
                                   MAX_NUM_OF_FD) != QCMAP_UTIL_SUCCESS)
            {
               LOG_MSG_ERROR("Failed to map inotify fd with the read function",0,0,0);
           ...
           LOG_MSG_ERROR("Failed to add a watch to the dev folder",0,0,0);
      }
    }
  }
  else
...
/*-----------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_INFO1("Successfully sent message to %s host size %d on fd %d",
                  (rmnet_state_config->ph_iface[PH_DRIVER_TYPE_USB].ph_enabled)?"USB" : "MHI", ret, ph_iface_fd);
  }
}
