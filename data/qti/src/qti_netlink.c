...
/*----------------------------------------------------------------------------*/
  ...
  LOG_MSG_INFO1("Assigned device names %s,%s,%s",
...
==========================================================================*/
void retry_link_bringup(int fd, int iface)
{
...
int qti_nl_recv_msg(int fd)
{
  ...
  else
  {
    ...
    while(NLMSG_OK(nlh, msglen))
    {
      ...
      {
        LOG_MSG_INFO1("Done receiving all msgs",0,0,0);
        break;
      }

      if(QCMAP_UTIL_SUCCESS != qcmap_nl_decode_link_nlmsg((char*)nlh,
                                                            msglen,
                                                            nlmsg,
                                                            nlh->nlmsg_type))
      {
      ...
      if(interface_num == 0)
      {
        ...
        {
          LOG_MSG_INFO1("Ignoring Netlink event on RNDIS since ECM is enabled", 0, 0, 0);
          /* Move to the NEXT Message */
          nlh = NLMSG_NEXT(nlh, msglen);
          continue;
        }
      ...
      else if(interface_num == 1)
      {
        ...
        {
          LOG_MSG_INFO1("Ignoring Netlink event on ECM since RNDIS is enabled", 0, 0, 0);
          /* Move to the NEXT Message */
          nlh = NLMSG_NEXT(nlh, msglen);
          continue;
        }
      ...
      if(nlmsg->link_event)
      {
        LOG_MSG_INFO1("flag: nl_link_info.metainfo.ifi_change=%d, ifi_flags=0x%x",
                        nlmsg->nl_link_info.metainfo->ifi_change,
                        nlmsg->nl_link_info.metainfo->ifi_flags,
                        0);
        /*--------------------------------------------------------------------------
            ...
            (nlmsg->type == RTM_NEWLINK))
        {
          ...
          else
          {
            ...
            {
              LOG_MSG_INFO1("Ignoring the LINK UP event as QTI didn't become a client to QCMAP yet",0,0,0);
            }
          }
        }
        /*--------------------------------------------------------------------------
