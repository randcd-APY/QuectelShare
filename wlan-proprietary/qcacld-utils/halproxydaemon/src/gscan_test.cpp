...
namespace GSCAN_TEST
{
    ...
    const char *GScanTestSuite::SSID_WHITE_LIST = "ssid_wl";
    const char *GScanTestSuite::ROAM_PARAMS = "params";
    const char *GScanTestSuite::LAZY_ROAM = "lazyroam";
    const char *GScanTestSuite::BSSID_PREFS = "bssid_prefs";
                                        ...
                                        int band, oui scan_oui)
    {
        ...
        switch(cmdIndex) {
            ...
            case 10:
                ...
                break;
            case 11:
                gscanSendSetSsidHotlistRequest(argc, argv);
                break;
            case 12:
                gscanSendResetSsidHotlistRequest(argc, argv);
                break;
            case 13:
            ...
            case 15:
                ...
                break;
            case 16:
                gscanSendSSIDWhiteList(argc, argv);
                break;
            case 17:
                gscanSendGscanRoamParams(argc, argv);
                break;
            case 18:
                gscanSendEnableLazyRoam(argc, argv);
                break;
            case 19:
                gscanSendSetBssidPrefs(argc, argv);
                break;
            case 20:
    ...
    wifi_error GScanTestSuite::processCmd(int argc, char **argv)
    {
        ...
        if (strcasecmp(argv[1], "roam") == 0) {
            if (strcasecmp(argv[ARGV_CMDID_INDEX], "ssid_wl") == 0)
                cmd = GSCAN_CMD_SSID_WHITE_LIST;
            else if (strcasecmp(argv[ARGV_CMDID_INDEX], "params") == 0)
                cmd = GSCAN_CMD_ROAM_PARAMS;
            else if (strcasecmp(argv[ARGV_CMDID_INDEX], "lazyroam") == 0)
                cmd = GSCAN_CMD_LAZY_ROAM;
            else if (strcasecmp(argv[ARGV_CMDID_INDEX], "bssid_prefs") == 0)
                cmd = GSCAN_CMD_BSSID_PREFS;
            else if (strcasecmp(argv[ARGV_CMDID_INDEX], "bssid_bl") == 0)
        ...
        switch(cmd) {
            ...
            case GSCAN_CMD_SET_MAC_OUI:
                if ((ARGV_CMDID_INDEX + 1 < argc) &&
                    ...
                    gscanSendSetMacOui(argc, argv, scan_oui);
                break;
            case GSCAN_CMD_SET_SSID_HOTLIST:
                gscanSendSetSsidHotlistRequest(argc, argv);
                break;
            case GSCAN_CMD_RESET_SSID_HOTLIST:
                gscanSendResetSsidHotlistRequest(argc, argv);
                break;
            ...
            case GSCAN_CMD_RESET_PASSPOINT_LIST:
                gscanSendResetPasspointListRequest();
                break;
            case GSCAN_CMD_SSID_WHITE_LIST:
                gscanSendSSIDWhiteList(argc, argv);
                break;
            case GSCAN_CMD_ROAM_PARAMS:
                gscanSendGscanRoamParams(argc, argv);
                break;
            case GSCAN_CMD_LAZY_ROAM:
                gscanSendEnableLazyRoam(argc, argv);
                break;
            case GSCAN_CMD_BSSID_PREFS:
                gscanSendSetBssidPrefs(argc, argv);
                break;
        ...
        if (f_read) {
            ...
            for ( i =0; i < params.num_buckets; i++ )
            {
                if ( (fscanf(f_read, "%d", &params.buckets[i].bucket) &&
                    ...
                    fscanf(f_read, "%d", &params.buckets[i].exponent) &&
    ...
    void GScanTestSuite::gscanSendSetSsidHotlistRequest(int argc, char **argv)
    {
        fprintf(stderr, "gscanSendSetSsidHotlistRequest: Sending GSCAN "
            "SetSsidHotlist Request. \n");
        int i, ret = 0;
        /* A string listing valid short options letters.  */
        const char* const short_options = "h:";
        /* An array describing valid long options.  */
        ...
        /* Override with command line arguements */
        int long_index = 0, opt = 0;
        char *ssids[] = {"Hydra-Test1", "Hydra-Test2", "Hydra-Test3"};
        while ((opt = getopt_long(argc, argv, short_options,
                long_options, &long_index )) != -1)
        {
            switch (opt)
            {
                case 'h':
                default:
                    gscanPrintCmdUsage(argv, GSCAN_CMD, GSCAN_SET_SSID_HOTLIST,
                        long_options,
                        sizeof(long_options)/sizeof(struct option));
                    return;
            }
        ...
        fprintf(stderr, "Parse params from "
            "/etc/wifi/gscan_set_ssid_hotlist_params.txt \n");
        wifi_hotlist_ssid_handler handler;
        wifi_ssid_hotlist_params params;

        FILE *f_read = fopen("/etc/wifi/gscan_set_ssid_hotlist_params.txt",
            "r");
        fprintf(stderr, "\n");
        if (f_read) {
            if (fscanf(f_read, "%d", &params.lost_ssid_sample_size)) {
                fprintf(stderr, "gscanSendSetSsidHotlistRequest: Parsed lost"
                    " ssid_sample size:%d\n", params.lost_ssid_sample_size);
            }
            if (fscanf(f_read, "%d", &params.num_ssid)) {
                fprintf(stderr, "gscanSendSetSsidHotlistRequest: Parsed "
                    "number of SSIDs:%d\n", params.num_ssid);
            }
            for ( i = 0; i < params.num_ssid; i++ )
            {
                if ((fscanf(f_read, "%s", params.ssid[i].ssid) &&
                    fscanf(f_read, "%d", &params.ssid[i].band) &&
                    fscanf(f_read, "%d", &params.ssid[i].low) &&
                    fscanf(f_read, "%d", &params.ssid[i].high)) == 0 ) {
                    fprintf(stderr, "%s: Failed parsing GSCAN Set SSID hotlist"
                        " params file. Exit\n", __func__);
                    return;
                }

                fprintf(stderr, "SSID[%d].ssid:%s\n", i, params.ssid[i].ssid);
                fprintf(stderr, "SSID[%d].band:%d\n SSID[%d].low:%d\n "
                    "SSID[%d].high:%d\n\n",
                    i, params.ssid[i].band,
                    i, params.ssid[i].low,
                    i, params.ssid[i].high);
            }
            fclose(f_read);
        } else {
            fprintf(stderr, "gscanSendSetSsidHotlistRequest: Failed to open "
            "file /etc/wifi/gscan_set_ssid_hotlist_params.txt  - "
            "use hard-coded defaults\n");

            params.lost_ssid_sample_size = 15;
            params.num_ssid = 3;
            strlcpy(params.ssid[0].ssid, ssids[0], sizeof(params.ssid[0].ssid));
            params.ssid[0].band = WIFI_BAND_BG;
            params.ssid[0].low = -80;
            params.ssid[0].high = -20;

            strlcpy(params.ssid[1].ssid, ssids[1], sizeof(params.ssid[1].ssid));
            params.ssid[1].band = WIFI_BAND_UNSPECIFIED;
            params.ssid[1].low = -77;
            params.ssid[1].high = -47;

            strlcpy(params.ssid[2].ssid, ssids[2], sizeof(params.ssid[2].ssid));
            params.ssid[2].band = WIFI_BAND_A;
            params.ssid[2].low = -69;
            params.ssid[2].high = -58;
        }

        handler.on_hotlist_ssid_found = gscan_on_hotlist_ssid_found;
        handler.on_hotlist_ssid_lost = gscan_on_hotlist_ssid_lost;
        ret = wifi_set_ssid_hotlist(id, ifaceHandle,
                                    params,
                                    handler);
        fprintf(stderr, "gscanSendSetSsidHotlistRequest: Sending GSCAN Set "
            "Ssid Hotlist request completed. Returned value: %d.\n", ret);

        if (ret)
            return;
    }

    void GScanTestSuite::gscanSendResetSsidHotlistRequest(int argc,
                                                                char **argv)
    {
        int ret = 0;
        /* A string listing valid short options letters.  */
        ...
        fprintf(stderr, "%s: Entry - Sending GSCAN Reset Ssid Hotlist "
            "Request.\n", __func__);
                ...
                long_options, &long_index )) != -1)
        {
            switch (opt)
            {
                ...
                default:
                    gscanPrintCmdUsage(argv, GSCAN_CMD,
                        GSCAN_RESET_SSID_HOTLIST, long_options,
                       sizeof(long_options)/sizeof(struct option));
                    return;
            }
        }

        fprintf(stderr, "%s: Reset SSID Hotlist for request_id:%d.\n",
            __FUNCTION__, id);
        ret = wifi_reset_ssid_hotlist(id, ifaceHandle);
        fprintf(stderr, "%s: Sending GSCAN Reset Ssid Hotlist request"
            "completed. Returned value: %d.\n", __FUNCTION__, ret);
    }

    wifi_error GScanTestSuite::gscanSendSSIDWhiteList(int argc, char **argv)
    {
        wifi_error ret;
        int num_networks = 0, i = 0, arg = 4;
        wifi_ssid *ssids;

        if (arg + 1 < argc) {
            id = atoi(argv[arg++]);
            num_networks = atoi(argv[arg++]);
        } else {
            fprintf(stderr, "insufficient args for %s\n", __FUNCTION__);
            return WIFI_ERROR_INVALID_ARGS;
        }

        ssids = (wifi_ssid *)malloc(num_networks * sizeof(wifi_ssid));
        if (!ssids) {
            fprintf(stderr, "Failed to allocate memory for "
                    "ssid white list\n");
            return WIFI_ERROR_OUT_OF_MEMORY;
        }
        for(i=0; i<num_networks && (arg < argc); i++)
            strlcpy(ssids[i].ssid, argv[arg++], sizeof(ssids->ssid));

        fprintf(stderr, "Set white list ssids request_id:%d and Number of ssids"
                " : %d\n", id, num_networks);
        for(i=0; i<num_networks; i++) {
            fprintf(stderr, "SSID %d: %s\n", i+1, ssids[i].ssid);
        }

        ret = wifi_set_ssid_white_list(id, ifaceHandle, num_networks, ssids);
        free(ssids);
        if (ret)
        {
            fprintf(stderr, "%s: Set SSID white list failed.\n", __func__);
        }
        return ret;
    }

    wifi_error GScanTestSuite::gscanSendGscanRoamParams(int argc, char **argv)
    {
        wifi_error ret;
        ...
        if (arg + 7 < argc) {
            id = atoi(argv[arg++]);
            params.A_band_boost_threshold = atoi(argv[arg++]);
            params.A_band_penalty_threshold = atoi(argv[arg++]);
            params.A_band_boost_factor = atoi(argv[arg++]);
            params.A_band_penalty_factor = atoi(argv[arg++]);
            params.A_band_max_boost = atoi(argv[arg++]);
            params.lazy_roam_hysteresis = atoi(argv[arg++]);
            params.alert_roam_rssi_trigger = atoi(argv[arg++]);
        } else {
            fprintf(stderr, "insufficient args for %s\n", __FUNCTION__);
            return WIFI_ERROR_INVALID_ARGS;
        }

        fprintf(stderr, " A_band_boost_threshold : %d \n",
                params.A_band_boost_threshold);
        fprintf(stderr, " A_band_penalty_threshold : %d\n",
                params.A_band_penalty_threshold);
        fprintf(stderr, " A_band_boost_factor : %d\n",
                params.A_band_boost_factor);
        fprintf(stderr, " A_band_penalty_factor : %d\n",
                params.A_band_penalty_factor);
        fprintf(stderr, " A_band_max_boost : %d\n",
                params.A_band_max_boost);
        fprintf(stderr, " lazy_roam_hysteresis: %d\n",
                params.lazy_roam_hysteresis);
        fprintf(stderr, " alert_roam_rssi_trigger : %d\n",
                params.alert_roam_rssi_trigger);


        ret = wifi_set_gscan_roam_params(id, ifaceHandle, &params);
        if (ret)
        {
            fprintf(stderr, "%s: Set Roam params failed.\n", __func__);
        }
        return ret;
    }

    wifi_error GScanTestSuite::gscanSendEnableLazyRoam(int argc, char **argv)
    {
        int enable;
        wifi_error ret;
        int arg = 4;
        ...
        if (arg + 1 < argc) {
            id = atoi(argv[arg++]);
            enable = atoi(argv[arg++]);
        } else {
            fprintf(stderr, "insufficient args for %s\n", __FUNCTION__);
            return WIFI_ERROR_INVALID_ARGS;
        }
        fprintf(stderr, "Lazy roam %s", enable?"Enable":"Disable","\n");

        ret = wifi_enable_lazy_roam(id, ifaceHandle, enable);
        if (ret)
        {
            fprintf(stderr, "%s: Enable/disable lazy roam failed.\n", __func__);
        }
        return ret;
    }

    wifi_error GScanTestSuite::gscanSendSetBssidPrefs(int argc, char **argv)
    {
        wifi_error ret;
        int num_bssid, i, arg = 4;
        char addr[20];
        u32 temp_mac[7];
        ...
        prefs = (wifi_bssid_preference *)
                    malloc(num_bssid * sizeof(wifi_bssid_preference));
        if (!prefs) {
        ...
        for(i=0; i<num_bssid; i++) {
            ...
            memset(&prefs[i].bssid[0], 0, 6 * sizeof(u8));
            ...
            int j = 0;
            while(j<6){
                prefs[i].bssid[j] = (u8)temp_mac[j];
                j++;
            }
            fprintf(stderr, "BSSID %d: "MAC_ADDR_STR"\n", i+1,
                MAC_ADDR_ARRAY(prefs[i].bssid));
            if (arg < argc)
                prefs->rssi_modifier = atoi(argv[arg++]);
            else {
                fprintf(stderr, "insufficient args for %s\n", __FUNCTION__);
                free(prefs);
                return WIFI_ERROR_INVALID_ARGS;
            }
            fprintf(stderr, "Rssi modifier : %d\n", prefs->rssi_modifier);
        }

        ret = wifi_set_bssid_preference(id, ifaceHandle, num_bssid, prefs);
        free(prefs);
        if (ret)
        {
            fprintf(stderr, "%s: Set bssid preference failed.\n", __func__);
        }
        return ret;
    }
    ...
    void GScanTestSuite::gscanSendSetEpnoListRequest(int argc, char **argv)
    {
        ...
        wifi_epno_network *networks = (wifi_epno_network *) malloc
            (sizeof(wifi_epno_network) * MAX_PNO_SSID);
        if (!networks) {
            fprintf(stderr, "Failed to allocate memory for wifi_epno_network "
                "*networks. Exit. ");
            return;
        }
        ...
        if (f_read) {
            if (fscanf(f_read, "%d", &num_networks)) {
                fprintf(stderr, "gscanSendSetEpnoListRequest: Parsed "
                    "number of ePNO list networks:%d\n", num_networks);
            }
            for ( i = 0; i < num_networks; i++ )
            {
                if ((fscanf(f_read, "%32s", networks[i].ssid) &&
                    fscanf(f_read, "%u", (unsigned int *)&networks[i].rssi_threshold) &&
                    fscanf(f_read, "%u", (unsigned int *)&networks[i].flags) &&
                    fscanf(f_read, "%u", (unsigned int *)&networks[i].auth_bit_field)) == 0 ) {
                ...
                fprintf(stderr, "ePNO-Network[%d].SSID: %s \n",
                            i, networks[i].ssid);
                fprintf(stderr, "ePNO-Network[%d].rssi_threshold:%d\n "
                            "ePNO-Network[%d].flags:%d\n "
                            "ePNO-Network[%d].auth_bit_field:%u\n\n",
                            i, (signed char) networks[i].rssi_threshold,
                            i, networks[i].flags,
                            i, networks[i].auth_bit_field);
            }
            fclose(f_read);
        } else {
            ...
            num_networks = 3;
            memcpy(networks[0].ssid, ssids[0], sizeof(networks[0].ssid));
            networks[0].rssi_threshold = 4;
            networks[0].flags = 7;
            networks[0].auth_bit_field = 1;

            memcpy(networks[1].ssid, ssids[1], sizeof(networks[1].ssid));
            networks[1].rssi_threshold = 8;
            networks[1].flags = 9;
            networks[1].auth_bit_field = 1;

            memcpy(networks[2].ssid, ssids[2], sizeof(networks[2].ssid));
            networks[2].rssi_threshold = 7;
            networks[2].flags = 15;
            networks[2].auth_bit_field = 0;
        ...
        ret = wifi_set_epno_list(id, ifaceHandle, num_networks,
                                networks, handler);
        fprintf(stderr, "gscanSendSetEpnoListRequest: Sending GSCAN Set "
            "ePNO List request completed. Returned value: %d.\n", ret);
        free(networks);
    }
    ...
    void GScanTestSuite::gscanSendSetPasspointListRequest()
    {
        ...
        wifi_passpoint_network *networks = (wifi_passpoint_network *) malloc
            (sizeof(wifi_passpoint_network) * MAX_PNO_SSID);
        if (!networks) {
