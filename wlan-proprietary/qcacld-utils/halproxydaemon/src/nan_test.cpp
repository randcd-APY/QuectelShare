...
namespace NAN_TEST
{
        ...
        :wifiHandle_(handle)
    {
        ...
        fprintf(stderr, "%s: Passing UserContext %p\n", __func__, this);
    ...
    void NanTestSuite::nanEventPublishTerminated(NanPublishTerminatedInd *event,
                                                 void *user_context)
    {
        fprintf(stderr, "%s: handle %d reason %d user_context %p\n",
          __FUNCTION__,
         event->header.handle,
         event->reason,
         user_context
                                     ...
                                     void *user_context)
    {
        fprintf(stderr, "%s: handle %d match_handle %08x " MAC_ADDR_STR " rssi:%d "
                "Match Occured Flag:%d Out of Resource Flag:%d user_context %p\n",
          __FUNCTION__,
          event->header.handle,
          event->match_handle,
    ...
    void NanTestSuite::nanEventUnMatch(NanUnmatchInd *event,
                                       void *user_context)
    {
        fprintf(stderr, "%s: handle %d match_handle %08x user_context %p\n",
          __FUNCTION__,
          event->header.handle,
          event->match_handle,
          user_context
    ...
    void NanTestSuite::nanEventDiscEngEvent(NanDiscEngEventInd *event,
                                            void *user_context)
    {
        fprintf(stderr, "%s: handle %d event_id %d user_context %p\n",
          __FUNCTION__,
         event->header.handle,
         event->event_id,
         user_context
                                   ...
                                   void *user_context)
    {
        fprintf(stderr, "%s: handle %d tca_id %d "
                "threshold risen %d threshold fallen %d user_context %p\n",
          __FUNCTION__,
         event->header.handle,
         event->tca_id,
                                          ...
                                          void *user_context)
    {
        fprintf(stderr, "%s: handle %d addr " MAC_ADDR_STR
                "isVsa %d isBeaconSdfPayload %d user_context %p\n",
          __FUNCTION__,
         event->header.handle,
         MAC_ADDR_ARRAY(event->addr),
        ...
        NanFurtherAvailabilityMap* fam)
    {
        ...
        for(int idx = 0; idx < fam->numchans; idx++)
        {
    ...
    {
        if(statsrsp->stats_id == NAN_STATS_ID_DE_PUBLISH)
        {
        ...
        else if(statsrsp->stats_id == NAN_STATS_ID_DE_SUBSCRIBE)
        {
        ...
        else if(statsrsp->stats_id == NAN_STATS_ID_DE_MAC)
        {
        ...
        else if(statsrsp->stats_id == NAN_STATS_ID_DE_TIMING_SYNC)
        {
            ...
            fprintf(stderr, "currTsf:%lu\n"
                    ...
                    "currRole:%u\n"
                    "currClusterId:%u\n"
                    "timeSpentInCurrRole:%lu\n"
                    "totalTimeSpentAsMaster:%lu\n"
                    "totalTimeSpentAsNonMasterSync:%lu\n"
                    "totalTimeSpentAsNonMasterNonSync:%lu\n"
        ...
        else if(statsrsp->stats_id == NAN_STATS_ID_DE_DW ||
                statsrsp->stats_id == NAN_STATS_ID_DE)
        {
        ...
        {
            fprintf(stderr,"Unknown stats_id:%d\n",
                    statsrsp->stats_id);
        }
    }
