    ...
    if (rtt_result) {
        for(u32 i = 0; i < num_results; i++)
        {
            ...
            if (result) {
                ...
                fprintf(stderr, "tx_rate:preamble(%d), nss(%d), bw(%d)"
                ...
                fprintf(stderr, "rx_rate:preamble(%d), nss(%d), bw(%d)"
                                "rateMcsIdx(%d) bitrate(%d Kbps)\n",
        ...
        if (f_read) {
            ...
            for ( i = 0; i < (int)num_rtt_config; i++ )
            {
                if (
                    ...
                    fscanf(f_read, "%d", &rtt_config[i].burst_period) &&
                    fscanf(f_read, "%d", &rtt_config[i].num_burst) &&
                    fscanf(f_read, "%d", &rtt_config[i].num_frames_per_burst) &&
                    fscanf(f_read,
                        "%d", &rtt_config[i].num_retries_per_rtt_frame) &&
                    fscanf(f_read, "%d", &rtt_config[i].num_retries_per_ftmr) &&
                    fscanf(f_read, "%d", &rtt_config[i].LCI_request) &&
                    fscanf(f_read, "%d", &rtt_config[i].LCR_request) &&
                    fscanf(f_read, "%d", &rtt_config[i].burst_duration) &&
                    fscanf(f_read, "%d", &rtt_config[i].preamble) &&
                    fscanf(f_read, "%hhx", &rtt_config[i].bw)) == 0 ) {
                    fprintf(stderr, "%s: Failed parsing RTT Range Request "
                        "params file. Exit\n", __func__);
                    fclose(f_read);
                    return;
