...
void dump_samp_msg(struct spectral_samp_msg_info *msg)
{
	...
	if (is_pwr_format_enabled) {
		for (i = 0; i < msg->samp_data.bin_pwr_count; i++)
			printf("%d ", (int8_t)msg->samp_data.bin_pwr[i]);
	} else {
		for (i = 0; i < msg->samp_data.bin_pwr_count; i++)
			printf("%d ", (uint8_t)msg->samp_data.bin_pwr[i]);
	}
	printf("\n");
...
void recv_and_process_results(void)
{
	...
	if (info->cldctx)
		cld80211_recv(info->cldctx, 5000, true,
...
int spectral_scan_start(int ifidx)
{
	...
	put_params(msg, SCAN_COUNT);
	put_params(msg, SCAN_PERIOD);
	put_params(msg, PRIORITY);
	put_params(msg, FFT_SIZE);
	put_params(msg, GC_ENA);
	put_params(msg, RESTART_ENA);
	put_params(msg, NOISE_FLOOR_REF);
	put_params(msg, INIT_DELAY);
	put_params(msg, NB_TONE_THR);
	put_params(msg, STR_BIN_THR);
	put_params(msg, WB_RPT_MODE);
	put_params(msg, RSSI_RPT_MODE);
	put_params(msg, RSSI_THR);
	put_params(msg, PWR_FORMAT);
	put_params(msg, RPT_MODE);
	put_params(msg, BIN_SCALE);
	put_params(msg, DBM_ADJ);
	put_params(msg, CHN_MASK);
...
#ifdef CONFIG_CLD80211_LIB
		cld80211_recv(info->cldctx, 5000, false, getsample_handler, NULL);
#else
		tv_timeout.tv_sec = QCA_SPECTOOL_NL_TIMEOUT_SEC;
		tv_timeout.tv_usec = QCA_SPECTOOL_NL_TIMEOUT_USEC;
