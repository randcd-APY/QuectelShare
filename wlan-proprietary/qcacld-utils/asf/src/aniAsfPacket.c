                             ...
                             u_int offset)
{
  ...
  *packetPtr = NULL;
  ...
  assert(ANI_CHECK_RANGE(0U, offset, maxHead));
  if (!ANI_CHECK_RANGE(0U, offset, maxHead))
    return ANI_E_ILLEGAL_ARG;
                           ...
                           u_int len)
{
    ...
    if (!ANI_CHECK_RANGE(0U, len, packet->len))
        return ANI_E_FAILED;
                       ...
                       u_int offset)
{
    ...
    assert(ANI_CHECK_RANGE(0U, offset, packet->size));
    if (!ANI_CHECK_RANGE(0U, offset, packet->size))
        return ANI_E_ILLEGAL_ARG;
