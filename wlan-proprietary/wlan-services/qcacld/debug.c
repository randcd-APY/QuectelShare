...
{
    openlog("wlan_services", LOG_PID | LOG_NDELAY, LOG_DAEMON);
    wsvc_debug_syslog++;
}
...
int wsvc_debug_open_file(const char *path)
{
    ...
    if (log_file == NULL) {
        wsvc_printf_err("%s: failed to open the log file: %s", __func__,
                path);
        return -1;
    }
...
static void _wsvc_vprintf(int level, const char *fmt, va_list argp)
{
    ...
    fprintf(file, "%s %2d %02d:%02d:%02d %5s: %s\n", months_abbr[tm.tm_mon],
            tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, msg_abbr[level], buf);
...
void _wsvc_hexdump(int level, const char *title, const void *p, int len)
{
    ...
    while (len > 0) {
        ...
        for (i = 0; i < size && n < sizeof(dump); i++) {
            n += snprintf(&dump[n], sizeof(dump) - n, " %02x", buf[i]);
            ascii[i] = WSVC_ASCII_CHAR(buf[i]);
        }

        ascii[i] = '\0';

        for (i = size; i < WSVC_HEXDUMP_COLUMN && n < sizeof(dump); i++)
