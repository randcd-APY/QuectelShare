...
static void *process_soc_data(void *arg)
{
    ...
    if (wds->mode == MODE_BT_UART || wds->mode == MODE_ANT_UART ||
        ...
        if (wds->mode == MODE_BT_SMD || wds->mode == MODE_ALL_SMD) {
            FD_SET((arr[num] = wds->soc_if.smd.bt_acl_fd), &readfds);num++;
            FD_SET((arr[num] = wds->soc_if.smd.bt_cmd_fd), &readfds);num++;
        }
        if (wds->mode == MODE_ANT_SMD || wds->mode == MODE_ALL_SMD) {
            FD_SET((arr[num] = wds->soc_if.smd.ant_cmd_fd), &readfds);num++;
            FD_SET((arr[num] = wds->soc_if.smd.ant_data_fd), &readfds);num++;
        }

        if (wds->mode == MODE_FM_SMD || wds->mode == MODE_ALL_SMD)
            FD_SET((arr[num] = wds->soc_if.smd.fm_cmd_fd), &readfds);num++;
