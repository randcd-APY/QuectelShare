...
static int parse_options(wdsdaemon *wds, int argc, char *argv[])
{
    ...
    if (argc > 2) {
        ...
        ERROR("Usage %s [-abfunlght]", argv[0]);
        return ret;
    }
    ...
    while ((opt = getopt(argc, argv, "abfunlghstm")) != -1) {
        switch (opt) {
        case 'a':
...
#endif
      case 'l':
          ERROR("Opening Loopback channels\n");
          wds->mode = MODE_LOOPBACK_UART;
          break;
      case 'g':
          ERROR("Opening Loopback channels\n");
          wds->bg_process = READ_FROM_FILE;
          printf("Setting the bg process flag\n");
          break;
      case 'h':
      ...
      if (soc_type == BT_SOC_CHEROKEE) {
          DEBUG("Use -l for HCI-UART Loopback Test\n");
          DEBUG("Use -g for HCI-UART Loopback background\n");
          DEBUG("\tFORMAT of LB Config file: /data/misc/bluetooth/lb_cfg.txt\n");
          DEBUG("\t\tX\tLB Mode\t: 1(BT) / 2(FM) / 3(BT + FM)\n");
          DEBUG("\t\tX\tLB type\t: 0(Continuous) / 1('N' no. of packets)\n");
          DEBUG("\t\tX\tLB cnt\t: Specify no of packets to loopback\n");
          DEBUG("\t\tXXXX\tBT Loopback delay (in ms)\n");
          DEBUG("\t\tXXXX\tFM Loopback delay (in ms)\n");
          DEBUG("\t\t0\tANT Loopback delay (in ms)\n");
                            ...
                            int *dst_fd, int *len, int dir)
{
    ...
    switch(pkt_id) {
        ...
        case BT_ACL_DATA_PKT_ID:
            state = RX_BT_HDR;
            if (wds->mode == MODE_BT_UART  || wds->mode == MODE_LOOPBACK_UART)
...
int process_pc_data_to_soc(wdsdaemon *wds, unsigned char *buf, int src_fd)
{
    ...
    if(wds->mode != MODE_LOOPBACK_UART) {
        ...
        if(retval)
            goto fail;
    } else { /* in UART loopback test mode */
        dst_fd = src_fd;
        switch(buf[0]){
        case BT_CMD_PKT_ID:
        case FM_CMD_PKT_ID:
            n_total = (HCI_PKT_IND_SIZE + HCI_CMD_PKT_HDR_SIZE +
                buf[HCI_CMD_PKT_LEN_OFFSET]);
            break;
        case BT_ACL_DATA_PKT_ID:
            n_total = HCI_PKT_IND_SIZE + HCI_ACL_PKT_HDR_SIZE +
                ((buf[HCI_ACL_PKT_LENGTH_OFFSET + 1] << 8) |
                (buf[HCI_ACL_PKT_LENGTH_OFFSET]));
            break;
        default:
            n_total = (HCI_PKT_IND_SIZE + HCI_CMD_PKT_HDR_SIZE +
                buf[HCI_CMD_PKT_LEN_OFFSET]);
...
int main(int argc, char *argv[])
{
    ...
    if(!wds.pcinit_mask && (wds.mode != MODE_LOOPBACK_UART))
    {
    ...
    switch (wds.mode) {
        ...
        case MODE_LOOPBACK_UART:
            if (hci_lb_core.pkt_type_bt.lb_enabled) {
                printf("Closing the BT WCNSS FILTER SOCKET: %d\n", wds.soc_if.uart.bt_fd);
                port_deinit_libbt(&wds, MODE_BT_UART);
            }
            if (hci_lb_core.pkt_type_fm.lb_enabled) {
                printf("Closing the FM WCNSS FILTER SOCKET: %d\n", wds.soc_if.uart.fm_fd);
                port_deinit_libbt(&wds, MODE_FM_UART);
            }
            if (hci_lb_core.pkt_type_ant.lb_enabled) {
                printf("Closing the ANT WCNSS FILTER SOCKET: %d\n", wds.soc_if.uart.ant_fd);
                port_deinit_libbt(&wds, MODE_ANT_UART);
            }
            lb_deinit(&hci_lb_core);
            break;
    }
