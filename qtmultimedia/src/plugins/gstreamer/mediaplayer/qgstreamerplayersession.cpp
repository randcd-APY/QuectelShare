...
{
	printf("new QGstreamerPlayerSession\n");
    m_playbin = gst_element_factory_make(QT_GSTREAMER_PLAYBIN_ELEMENT_NAME, QT_GSTREAMER_PLAYBIN_ELEMENT_NAME);
    if (m_playbin) {
        ...
        if(envAudioSink.isEmpty())
            printf("I: QGstreamerPlayerSession: Les : chose autoaudiosink \n");
        else
            printf("E: QGstreamerPlayerSession: Les : chose %s \n", (const char*)envAudioSink);
        GstElement *audioSink = gst_element_factory_make(envAudioSink.isEmpty() ? "autoaudiosink" : envAudioSink, "audiosink");
        if (audioSink) {
            printf("I: QGstreamerPlayerSession: Les : audioSink is valid \n");
            m_audioSink = audioSink;
            m_volumeElement = m_playbin;
            /*if (usePlaybinVolume()) {
                printf("V: QGstreamerPlayerSession: Les : usePlaybinVolume is true \n");
                m_audioSink = audioSink;
                m_volumeElement = m_playbin;
            } else {
                m_volumeElement = gst_element_factory_make("volume", "volumeelement");
                if (m_volumeElement) {
                    printf("V: QGstreamerPlayerSession: Les : m_volumeElement is valid \n");
            ...
            printf("E: QGstreamerPlayerSession: Les : audioSink is NULL \n");
        }
    }
	...
	m_nullVideoSink = gst_parse_bin_from_description ("fbdevsink device=/dev/fb0", TRUE, NULL);
    ...
    if (m_playbin != 0) {
        printf("I: QGstreamerPlayerSession: Les : m_playbin is valid\n");
        // Sort out messages
        ...
        if (/*usePlaybinVolume()*/m_audioSink) {
            setVolume(m_volume);  // add by Les
            updateVolume();
            updateMuted();
        ...
        //gdouble nplayervolume = 10.0;  // add by Les
        //g_object_set(G_OBJECT(m_playbin), "volume", nplayervolume, NULL);  // add by Les
#if QT_CONFIG(gstreamer_app)
...
#endif
    printf("I: QGstreamerPlayerSession::setVolume Les : volume = %d, m_volume = %d \n", volume, m_volume);
    if (m_volume != volume) {
        m_volume = volume;
        if (m_volumeElement)
            g_object_set(G_OBJECT(m_volumeElement), "volume", m_volume / 5.0, NULL);  // modified by Les
...
#endif
    printf("I: QGstreamerPlayerSession::setMuted Les : muted = %d\n", muted);
    if (m_muted != muted) {
        m_muted = muted;
...
void QGstreamerPlayerSession::updateVolume()
{
    ...
    printf("I: QGstreamerPlayerSession::updateVolume Les : volume = %d, m_volume = %d \n", (int)volume, m_volume);

    if (m_volume != int(volume*5 + 0.5)) { // modified by Les
        m_volume = int(volume*5 + 0.5); // modified by Les
#ifdef DEBUG_PLAYBIN
